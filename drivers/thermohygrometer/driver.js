/*
 * HomeWizard Cloud Thermo/Hygrometer Driver
 * 
...
class CloudThermoHygroDriver extends Driver {

  /**
   * onInit is called when the driver is initialized.
   */
  async onInit() {
    this.log('CloudThermoHygroDriver has been initialized');
  }
  ...
  async onPair(session) {
    ...
    session.setHandler('list_devices_for_location', async (data) => {
      try {
        ...
        // Filter for Thermo/Hygrometer only
        const p1Devices = (location.devices || []).filter(device => 
          device.type === 'thermohygrometer'
        );

        this.log(`Found ${p1Devices.length} Thermo/Hygro device(s) in location ${location.name}`);

        // Format devices for display
        return p1Devices.map(device => ({
          name: device.name || 'Thermo/Hygrometer',
          data: {
            id: device.device_id
          },
          settings: {
...
module.exports = CloudThermoHygroDriver;