...
// this rode along on every unrelated write. See lib/userdata-store.js.
const CACHE_FILE     = 'solcast-forecast-cache';
...
class SolcastProvider {
  ...
  _saveCache(data) {
    if (!userdata.writeJson(CACHE_FILE, { data, fetchedAt: Date.now() })) {
      this.homey.log('[Solcast] Cache save to /userdata failed');
    }
  }
