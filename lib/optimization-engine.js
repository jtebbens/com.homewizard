...
class OptimizationEngine {
   ...
   */
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0, terminalPvKwhTomorrow = pvKwhTomorrow, pvCloudFactor = 1.0) {
    ...
    const { dp, policy, chargeSocDeltaG, perSlotDischargeSocDeltaG, chargeKwhFull, pvStrongCoverage,
            slot0ChargeSocDeltaG, slot0ChargeKwhFull } =
      this._runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
        ...
        slot0RemainingFrac, pvWPerSlot, terminalPvKwhTomorrow);
    ...
    for (let t = 0; t < N; t++) {
      ...
      // Standby + strong PV surplus: check if _pvStoreWins would apply at runtime.
      // Condition: max future price × RTE > current price (simplified; ignores delay-charge).
      // When true, override action to 'preserve' so buildPlanningSchedule shows zero_charge_only,
      // and update SoC so downstream projections are accurate.
      let pvStoreWins = false;
      if (code === 3 && !hasFutureStrongNeg && pvCoverage[t] >= pvStrongCoverage && price > 0) {
        const pvStoreValue = suffixMaxPrice[t] * effectiveRte;
        if (pvStoreValue > price) {
          action = 'preserve';
          pvStoreWins = true;
        }
      }
      ...
      const trickleWins = pvStoreValue2 > price * (this.exportPriceRatio ?? 1.0);
      ...
      slots.push({ timestamp: prices[t].timestamp, action, price, socProjected: socG / GRID, pvExportWins,
        ...
        pvStoreValue: suffixMaxPrice[t] * effectiveRte,
      ...
      if (code === 1) {
        ...
        if (isToday) {
          const actualDeltaG   = Math.min(perSlotDischargeSocDeltaG[t], socG - minSocG);
          const dischargeKwh   = (actualDeltaG / GRID_TOTAL) * capacityKwh;
          todayProjectedProfit += price * dischargeKwh * effectiveRte;
        }
        socG = Math.max(minSocG, socG - perSlotDischargeSocDeltaG[t]);
      } else if (code === 0 && pvCoverage[t] >= pvStrongCoverage && prices[t].price >= 0) {
      ...
      if (wEnd >= 2) {
        ...
        if (eligible.length >= 2 && !hasCharge) {
          ...
          for (const t of eligible) {
            if (remainSocG <= minSocG) break;
            assigned.add(t);
            remainSocG = Math.max(minSocG, remainSocG - perSlotDischargeSocDeltaG[t]);
          }
          ...
          if (changed) {
            ...
            for (let t = 0; t < wEnd; t++) {
              if (assigned.has(t) && socGw > minSocG) {
              ...
              if (slots[t].action === 'discharge') {
                socGw = Math.max(minSocG, socGw - perSlotDischargeSocDeltaG[t]);
    ...
    this._schedule = { computedAt: Date.now(), projectedProfit, todayProjectedProfit, slots };
  }
  ...
  _runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
                 ...
                 slot0RemainingFrac = 1.0, pvWPerSlot = null, terminalPvKwhTomorrow = pvKwhTomorrow) {
    const GRID_TOTAL = GRID * 100;
    ...
    if (N >= 4) {
      const pvRefill = capacityKwh > 0 ? Math.min(1, terminalPvKwhTomorrow / capacityKwh) : 0;
    ...
    for (let t = N - 1; t >= 0; t--) {
      ...
      // Never flatten during negative-price slots: low SoC has HIGHER future value there
      // (more room to earn at deeper negative slots), so flattening erases the SoC gradient
      // that causes the DP to skip -0.02 in favour of reserving capacity for -0.37.
      // Never flatten when a higher-priced discharge opportunity exists in any upcoming
      // night slot (before PV kicks in): flattening destroys the SoC gradient that makes
      // the DP prefer the better slot over the current one.
      // e.g. 23:00 at €0.272 must not flatten when 01:00 is €0.290 — discharge there instead.
      // Never flatten when the immediately next slot is strong PV (pvCoverage[t+1] ≥ pvStrongCoverage):
      // the SoC gradient at that boundary encodes extra PV export revenue — a higher starting SoC
      // means less PV is stored (because battery is already fuller) and more exported at the morning
      // spot price. Flattening here destroys that gradient and makes the DP blind to the value of
      // holding charge overnight when tomorrow morning prices exceed tonight's discharge price.
      let _bestFutureNightPrice = -Infinity;
      for (let _k = t + 1; _k < N && pvCoverage[_k] < pvStrongCoverage; _k++) {
        if (prices[_k].price > _bestFutureNightPrice) _bestFutureNightPrice = prices[_k].price;
      }
      const _betterSlotAhead = _bestFutureNightPrice > prices[t].price && _bestFutureNightPrice >= minDischargePrice;
      if (!_betterSlotAhead && prices[t].price >= 0 && pvKwhTomorrow >= capacityKwh * 0.8 && pvKwhFromT[t + 1] >= capacityKwh
    ...
    return { dp, policy, chargeSocDeltaG, perSlotDischargeSocDeltaG, chargeKwhFull, pvStrongCoverage, slot0ChargeSocDeltaG, slot0ChargeKwhFull };
  }
