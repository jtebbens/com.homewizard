...
class OptimizationEngine {
  ...
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0) {
    ...
    const pvCoverage = prices.map((p, t) => {
      ...
      const consW = Array.isArray(consumptionWPerSlot) && consumptionWPerSlot[t] != null
        ? consumptionWPerSlot[t] : 50; // 50W baseload floor when slot not yet learned
      pvWPerSlot.push(pvW);
    ...
    for (let t = 0; t < N; t++) {
      ...
      // Skip trickle when PV forecast is sufficient to fill the battery on its own AND
      // storing beats exporting (exportPriceRatio accounts for saldering vs post-2027).
      const pvSufficient = pvKwhTomorrow * effectiveRte >= capacityKwh * (1 - minSocG / GRID_TOTAL);
      const pvStoreValue2 = suffixMaxPrice[t] * effectiveRte;
      const trickleWins = !pvSufficient || pvStoreValue2 > price * (this.exportPriceRatio ?? 1.0);
  ...
  _runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
                 ...
                 slot0RemainingFrac = 1.0, pvWPerSlot = null) {
    const GRID_TOTAL = GRID * 100;
