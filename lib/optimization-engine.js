...
class OptimizationEngine {
  ...
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0, terminalPvKwhTomorrow = pvKwhTomorrow, pvCloudFactor = 1.0, refillConfidence = 1.0) {
    ...
    const { dp, policy, chargeSocDeltaG, perSlotDischargeSocDeltaG, chargeKwhFull, pvStrongCoverage,
            slot0ChargeSocDeltaG, slot0ChargeKwhFull, reserveFloorG } =
      ...
      if (wEnd >= 2) {
        ...
        if (eligible.length >= 2 && !hasCharge) {
          ...
          for (const t of eligible) {
            if (remainSocG <= reserveFloorG[t]) break;
            assigned.add(t);
            remainSocG = Math.max(reserveFloorG[t], remainSocG - perSlotDischargeSocDeltaG[t]);
          }
          ...
          if (changed) {
            ...
            for (let t = 0; t < wEnd; t++) {
              if (assigned.has(t) && socGw > reserveFloorG[t]) {
              ...
              if (slots[t].action === 'discharge') {
                socGw = Math.max(reserveFloorG[t], socGw - perSlotDischargeSocDeltaG[t]);
  ...
  _runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
    ...
    if (reserveAddG > 0) {
      ...
      for (let t = N - 1; t >= 0; t--) {
        ...
        if (strongPvAhead) reserveFloorG[t] = Math.min(maxSocG, minSocG + reserveAddG);
      }
    }
    ...
    return { dp, policy, chargeSocDeltaG, perSlotDischargeSocDeltaG, chargeKwhFull, pvStrongCoverage, slot0ChargeSocDeltaG, slot0ChargeKwhFull, reserveFloorG };
  }
