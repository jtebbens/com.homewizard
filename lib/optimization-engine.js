...
class OptimizationEngine {
  ...
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0, terminalPvKwhTomorrow = pvKwhTomorrow, pvCloudFactor = 1.0, refillConfidence = 1.0, pvTimingRobust = false, maxChargePrice = 0) {
    ...
    // Slot index the suffix max sits on, -1 when no positive price follows. `>=` on a tie
    // keeps the EARLIEST such slot: that is the first moment a stored kWh can be spent, and
    // the deadline the PV-saturation test below has to beat. Same backward scan, so this
    // stays O(N) and outside the forward loop.
    const suffixMaxIdx = new Int32Array(N).fill(-1);
    for (let k = N - 2; k >= 0; k--) {
      const p = prices[k + 1]?.price ?? 0;
      const nextIdx = suffixMaxIdx[k + 1];
      const nextVal = nextIdx >= 0 ? suffixMaxPrice[k + 1] : 0;
      suffixMaxIdx[k] = (p > 0 && p >= nextVal) ? k + 1 : nextIdx;
    }
    ...
    const tcDebug = this._trickleCapDebug = {
      nSlots: 0, nCapZero: 0, nCapZeroSaturating: 0,
    };
    ...
    for (let t = 0; t < N; t++) {
      ...
      // HIGHER the store value it yields. Test the assertion instead.
      //
      // The PV counted must land BEFORE the peak the store value is about. pvKwhFromT[t+1]
      // runs to the end of the 48h horizon, so tomorrow's PV was proving "the pack fills
      // anyway" for a peak TONIGHT — and that reasoning eats itself: zeroing the store value
      // makes export win at every slot of today's block, so the charge the test assumed is
      // never made and the pack enters the peak near empty (live 2026-08-08: 12% through a
      // 2.5kW block, arriving at a €0.363 peak at 38%; property invariant 44). Bounding the
      // window at suffixMaxIdx[t] excludes PV that only arrives after the peak, which times
      // the charge to end exactly full: the cap keeps binding while more PV than room remains
      // ahead, and releases at the point where the rest of the block just fills the pack.
      // pvKwhFromT is a suffix sum, so the window is an O(1) difference — no second loop.
      const roomKwh = Math.max(0, usableSpanKwh - currentKwh);
      const peakIdx = suffixMaxIdx[t];
      const pvBeforePeakKwh = peakIdx > t
        ? Math.max(0, (pvKwhFromT[t + 1] ?? 0) - (pvKwhFromT[peakIdx] ?? 0))
        : (pvKwhFromT[t + 1] ?? 0);   // no positive price ahead → nothing to be early for
      const pvSaturatesAhead = pvBeforePeakKwh >= roomKwh;
      const capBinds         = trickleSuffixMaxPrice[t] <= 0 && pvSaturatesAhead;
      ...
      // Live counters for the trickle-cap saturation test, over the WHOLE horizon — a t=0-only
      // counter cannot see the midday slots this test is about. nCapZero counts slots where the
      // trickle cap zeroes out; nCapZeroSaturating counts the subset where the PV still ahead
      // of the peak really does refill the battery, which is where the cap is honoured instead
      // of falling back to the uncapped suffix max.
      if (tcDebug) {
        tcDebug.nSlots++;
        if (trickleSuffixMaxPrice[t] <= 0) tcDebug.nCapZero++;
        if (capBinds) tcDebug.nCapZeroSaturating++;
      }
  ...
  _runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
    ...
    for (let t = N - 1; t >= 0; t--) {
      ...
      if (flattenGateOpen && !(this.flattenArbGate && arbAhead)) {
        ...
        const pvG = Math.round(pvKwhFromT[t + 1] / capacityKwh * GRID_TOTAL);
