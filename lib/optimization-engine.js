...
class OptimizationEngine {
   ...
   *   Expected house consumption per slot in W. When provided, discharge value
   *   is split: local consumption offset at 100% price, export at 30% price.
   */
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null) {
    ...
    for (let t = N - 1; t >= 0; t--) {
      ...
      for (let soc = 0; soc <= 100; soc++) {
        ...
        if (soc > minSoc) {
          ...
          const consumptionKwh = (Array.isArray(consumptionWPerSlot) && consumptionWPerSlot[t] > 0)
    ...
    for (let t = 0; t < N; t++) {
      ...
      else if (pvCoverage[t] > 0) {
        // Preserve + PV available: firmware runs zero_charge_only, battery charges for free.
        // Reflect this in the projected SoC so the planning view stays accurate.
