...
class OptimizationEngine {
   ...
   */
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW) {
    ...
    const chargeSocDelta    = Math.max(1, Math.round(
      (maxChargePowerW   / 1000) * slotH * this.RTE * 100 / capacityKwh
    ));
    ...
    for (let t = N - 1; t >= 0; t--) {
      ...
      for (let soc = 0; soc <= 100; soc++) {
        ...
        // Charge: SoC rises, grid energy is paid for
        let vCharge = -1e9;
        if (soc < maxSoc) {
          ...
          vCharge = -price * kwh + dp[newSoc];
        }

        // Discharge: SoC falls, house consumption is covered (avoided grid cost)
        let vDischarge = -1e9;
        if (soc > minSoc) {
          ...
          vDischarge = price * kwh + dp[newSoc];
        }
