...
class OptimizationEngine {
  ...
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0) {
    ...
    const perSlotDischargeSocDelta = effectiveDischargePowerW.map(w =>
      Math.max(1, Math.round((w / 1000) * slotH * 100 / capacityKwh))
    );
    ...
    for (let t = N - 1; t >= 0; t--) {
      ...
      for (let soc = 0; soc <= 100; soc++) {
        ...
        let vDischarge = -1e9;
        if (soc > minSoc && pvCoverage[t] <= 0.5 && price >= minDischargePrice) {
          const slotDischargeSocDelta = perSlotDischargeSocDelta[t];
          const slotDischargeKwhFull  = perSlotDischargeKwhFull[t];
          const newSoc      = Math.max(minSoc, soc - slotDischargeSocDelta);
          const socDelta    = soc - newSoc;
          const kwh         = slotDischargeKwhFull * socDelta / slotDischargeSocDelta;
    ...
    for (let t = 0; t < N; t++) {
      ...
      else if (code === 2) soc = Math.max(minSoc, soc - perSlotDischargeSocDelta[t]);
