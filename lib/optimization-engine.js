...
class OptimizationEngine {
  ...
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0, terminalPvKwhTomorrow = pvKwhTomorrow, pvCloudFactor = 1.0, refillConfidence = 1.0, pvTimingRobust = false, maxChargePrice = 0) {
    ...
    for (let t = 0; t < N; t++) {
      ...
      // when free PV ahead actually covers the remaining room.
      ...
      // trickle cap zeroes out; nCapZeroSaturating counts the subset where free PV ahead really
      // does refill the battery, which is where the cap is honoured instead of falling back to
      // the uncapped suffix max. The difference between the two is the fallback that used to
      // fire unconditionally and inverted the cap.
      if (tcDebug) {
        tcDebug.nSlots++;
