...
class OptimizationEngine {
   ...
   */
  static refillConfidenceFromForecast(cv, ratio, pvKwhTomorrow = 0, usableSpanKwh = 0) {
    ...
    // Total energy used in the window stays within the same budget.
    {
      ...
      if (wEnd >= 2) {
        ...
        if (eligible.length >= 2 && !hasCharge) {
          ...
          const dpEndTargetG = wEnd < N ? Math.round(slots[wEnd].socProjected * GRID) : minSocG;
          const assigned = new Set();
          let remainSocG = initialSocG;
          for (const t of eligible) {
            const stopG = Math.max(dpEndTargetG, reserveFloorG[t]);
