...
class OptimizationEngine {
   ...
   */
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0, terminalPvKwhTomorrow = pvKwhTomorrow, pvCloudFactor = 1.0, refillConfidence = 1.0, pvTimingRobust = false) {
    ...
    const { dp, policy, chargeSocDeltaG, perSlotDischargeSocDeltaG, chargeKwhFull, pvStrongCoverage,
      ...
      this._runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
        ...
        slot0RemainingFrac, pvWPerSlot, terminalPvKwhTomorrow, refillConfidence, pvTimingRobust, pvSpreadPerSlot);
    ...
    for (let t = 0; t < N; t++) {
      ...
      const cheaperPvAhead = pvCoverage[t] >= pvStrongCoverage
        && minFuturePvStrongPrice[t] < price
        && minFuturePvStrongPrice[t] < Infinity;
      let pvStoreWins = false;
      if (code === 3 && !hasFutureStrongNeg && pvCoverage[t] >= pvStrongCoverage && price > 0 && pvStoreBeatsExport && !cheaperPvAhead) {
        action = 'preserve';
        pvStoreWins = true;
      }
    ...
    // Total energy used in the window stays within the same budget.
    {
      ...
      if (windowLen >= 2) {
        ...
        if (eligible.length >= 2 && !hasCharge) {
          ...
          if (changed) {
            const snapshot = slots.slice(wStart, wEnd).map(s => ({ action: s.action, socProjected: s.socProjected }));
  ...
  _runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
                 ...
                 slot0RemainingFrac = 1.0, pvWPerSlot = null, terminalPvKwhTomorrow = pvKwhTomorrow, refillConfidence = 1.0, pvTimingRobust = false, pvSpreadPerSlot = null) {
    const GRID_TOTAL = GRID * 100;
