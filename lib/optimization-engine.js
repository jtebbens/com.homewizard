...
const { exportValue, disposalValue, storeValue: storeVal } = require('./price-formulas');
const { fullCurtailSlot } = require('./curtailment');
...
// ── CVaR refill-reserve constants (dp_cvar_reserve) ──────────────────────────────────
// CVaR depth in standard deviations of downside refill. 1.40 = mean of the worst 20%.
const CVAR_K = 1.40;
// Downside spread of the refill itself, relative. Fitted directly on realized-vs-forecast
// storable refill (storablePrefix[lastStrongPv+1]) over the 52 stored dp-input dumps: 35 usable
// runs across 6 distinct days, downside semi-deviation of real/forecast = 0.038 (worst ratio
// 0.843, below forecast in 5/35 runs). Day-collapsed it is 0.012; the run-level number is the
// conservative one and is what is carried here.
//
// This replaces an earlier attempt to derive sigma from the ensemble's own per-slot spread via
// the EMOS fit (sigma^2 = 6725 + 13.6*S^2). Those constants are fitted per HOUR in W; applied to
// an aggregate of ~20 hours in kWh they gave sigma_rel ≈ 1.11, which makes the CVaR haircut
// negative and pins the floor at its cap regardless of coverage — the same inversion the floor
// was rewritten to remove. Only a downside deviation belongs here: the reserve insures the
// refill falling SHORT, and a two-sided sigma on this sample is dominated by one day where the
// forecast under-predicted by 9x (17-08), which is not a risk this floor covers.
//
// Kept as a constant rather than a function of the ensemble spread because the dependence is not
// identifiable on this sample: corr(spreadRel, downside error) = -0.21 over 5 downside points on
// 3 days. Re-fit with scratchpad/refill-sigma-fit.js once more dumps exist, and note that all 6
// days are August — no winter or low-sun regime is represented.
const CVAR_SIGMA_DOWN_REL = 0.038;
...
const ACTION_SRC = {
  ...
  UNREPAYABLE: 'U', // forward-pass override: charge cancelled, the round trip cannot earn it back
...
// PV strength thresholds. Three distinct quantities that happen to share the number 400;
// they are NOT interchangeable, so do not collapse them into one constant.
//   PV_STRONG_SURPLUS_W        net PV surplus (pvW - consumptionW). Above it the DP credits
//                              a free SoC gain on preserve and the planning mapper emits
//                              zero_charge_only.
//   PV_STRONG_RAW_W            raw PV output. Gates the same mapper branch as a second,
//                              independent condition: a house drawing 900 W at 500 W PV
//                              passes this one and fails the surplus one.
//   PV_ONLY_CHARGE_FLOOR_MIN_W lower bound of the "PV alone can sustain a charge" floor,
//                              which is max(this, 0.8 * chargePowerW) = 640 W at 800 W.
// All three originate in tuning commit 03286552 (2026-03-26), not in a firmware spec.
const PV_STRONG_SURPLUS_W = 400;
const PV_STRONG_RAW_W = 400;
const PV_ONLY_CHARGE_FLOOR_MIN_W = 400;

// The fourth quantity, and the only one that IS a hardware fact: the net surplus below which the
// firmware will not charge from PV at all. Live log 2026-09-10 11:00 Amsterdam settles the number
// upward from zero and downward from 400 — a 237 W surplus ran zero_charge_only at 260-340 W with
// SoC 45% -> 47%. See reference_battery_firmware_charge_floor_75w.
const PV_CHARGE_FLOOR_W = 75;
...
class OptimizationEngine {
  constructor(settings) {
    ...
    // Curtailment (chunk 6): when the user's own flow can throttle the inverter, surplus PV
    // that is not stored can be thrown away for free instead of exported. That floors every
    // forgone-export term at 0 — see _disposalValue. Default off: the DP must not plan on a
    // disposal route the install does not have.
    this.curtailmentAvailable = settings.pv_curtailment_enabled === true;
    // Flatten-gate arb guard (default ON). The per-SoC flatten (backward loop) is skipped
    // at slots where a future discharge slot beats the charge-now→discharge-later round-trip
    // cost, so the charge gradient survives instead of being clamped flat.
    // Evidence: replaying the 51 stored dp-input dumps scored it 13 dumps better, 0 worse,
    // sign stable at refPrice 0.25/0.35/0.45; invariant 36 (economic dominance) and the
    // flatten-arb-gate scenario are green; live on 2026-08-31 it turned a suppressed
    // €0.134 → €0.384 evening round trip back into a planned charge.
    // Not user-configurable: whether a real SoC gradient may be flattened away is a
    // correctness question, not a preference.
    this.flattenArbGate   = settings.dp_flatten_arb_gate ?? true;
    ...
    // Trickle-cap saturation test (default on). When on, a zeroed trickleSuffixMaxPrice is
    // only honoured when free PV ahead actually refills the battery; otherwise the later peak
    // is still reachable and the uncapped suffix max stands. Off, the forward pass keeps the
    // unconditional uncapped fallback — and that fallback inverts the cap: a zeroed cap ("the
    // window to the next pvStrong slot is empty") was read as "no cap at all" and reached for
    // the evening peak, so a TIGHTER cap yielded a HIGHER store value. Live 2026-09-10 09:00Z:
    // 11:00 (€0.299, cap 0) stored at €0.556 while 11:15 (€0.278, cap €0.263) exported at
    // €0.118, with the pack full by 13:45 either way — the dearer surplus kept, the cheaper one
    // sold. With trickleCapPos this collapses rawStorePrice to one monotone predicate:
    // pvSaturatesAhead ? trickleSuffixMaxPrice[t] : suffixMaxPrice[t].
    this.trickleCapSat    = settings.dp_trickle_cap_saturation ?? true;
    // Same saturation test on the POSITIVE trickle cap (default on). A cap that lands on a
    // price rather than on zero was taken unconditionally: the "a pvStrong slot ahead refills
    // the battery" assertion was never checked there. Live 2026-09-08 11:00Z, 45% SoC: the cap
    // valued the surplus at the NEXT slot's price (€0.110) instead of the evening peak (€0.261)
    // and the run exported at €0.214, while the same run logged saturating=0 and the pack never
    // got past 55%. Off = the previous unconditional behaviour.
    this.trickleCapPos    = settings.dp_trickle_cap_positive ?? true;
    // Reserve waiver inside the post-DP reorder (default off — shadow first). The overnight
    // refill-reserve waives its floor per slot whose own price beats releasedPeak (:1382-1392),
    // but the reorder's effFloorG re-imposes it through dpEndTargetG, which inherits the floor of
    // the window's LAST slot (a floored morning slot). Live 2026-09-08 20:15: floor0=0% in the
    // flatten line, floor0=21% in the reorder line of the same run — €0.428/€0.416/€0.399 stood
    // still to insure a €0.385 peak. Off = the previous behaviour.
    this.reserveWaiver    = settings.dp_reserve_waiver_reorder ?? false;
    ...
    // Weak PV inside the DP (default off -> shadow only). Above the flag, the whole weak-surplus
    // band is invisible to the backward pass: pvSocGainG is 0 below pvStrongCoverage, so preserve
    // and standby are the same transition on the same dp[] element and the DP cannot choose. The
    // choice was rebuilt outside it (the pvTrickle/weakReachable branch in the forward pass, and
    // _pvStoreWins in policy-engine) — a second decider, which is what CLAUDE.md forbids.
    // On, the SoC gain is credited from PV_CHARGE_FLOOR_W upward, so vPreserve pays the same half
    // cycle and the same forgone export the strong band already pays, and the weak-band overlay
    // steps aside. Off, every line below reduces to the previous behaviour and the shadow pass
    // reports what switching it on would have cost or earned.
    this.weakPvInDp       = settings.dp_weak_pv_in_dp ?? false;
    // Weak-PV shadow measurement (default OFF — opt in per engine). Runs the horizon a second
    // time with dp_weak_pv_in_dp flipped and scores both plans with one scorer, so the flag's
    // € value is known before it is switched on. Diagnostic only. Off by default because this
    // class is also instantiated as a shadow itself (device.js's 2027 and curtailment engines)
    // and by the property suite, and none of those should pay for a second compute(). Only the
    // live engine (device.js:194) asks for it. See _runWeakPvShadow.
    this.weakPvShadow     = settings.dp_weak_pv_shadow   ?? false;
    // Charge repayability test (default off → shadow-log only). The backward pass prices a
    // round trip at half a cycle whenever pvAbundant waives the discharge side (_runBackwardDP,
    // the slotCycleCost line), while the charge side always pays its half — so a grid charge can
    // win on a discount that has no counterpart, and the plan buys energy no later price can earn
    // back (live 2026-08-20 12:45Z, €0.2206 in against a €0.3873 peak at RTE 0.732). On, such a
    // slot is cancelled in the forward roll-out. It filters the rolled-out plan; it does not
    // correct the half-cycle pricing itself — removing the waiver measured worse over 52 dumps.
    this.chargeRepayGate  = settings.dp_charge_repay_gate ?? false;
    // Family-B premise counters (off by default). Diagnostic only, but the B1 day-edge split
    // formats one date per slot per compute() — enough Intl work to blow the property suite's
    // 10s-per-property budget. Only tools/dp-familyB-count.js turns it on.
    this.gateBCount       = settings.dp_gate_b_count      ?? false;
    // Size the refill-reserve floor as the CVaR of the evening shortfall instead of
    // (1 - refillConfidence) * 0.5 * span. The legacy height inverts: more forecast
    // uncertainty raises the floor, which makes it more likely to sit ABOVE the post-peak
    // SoC and never bind at all (measured: all ~64 runs at floor 24-38% held 0.00 kWh).
    // The CVaR height is "evening need minus worst-case refill", so demonstrable PV
    // coverage collapses it to zero while a genuinely uncovered evening raises it.
    // Off by default; no settings-page entry — tools/tests override it via the constructor.
    this.cvarReserve      = settings.dp_cvar_reserve      ?? false;
    this._schedule = null; // { computedAt: number, projectedProfit: number, slots: [{timestamp, action}] }
  }

  /**
   * The settings this engine currently runs on, in constructor shape. Rebuilt from the live
   * fields instead of stashing the constructor argument, so updateSettings() cannot make it
   * drift. Only user: the weak-PV shadow clone in compute().
   */
  _settingsSnapshot() {
    return {
      battery_efficiency: this.RTE,
      min_soc: this.minSoc,
      max_soc: this.maxSoc,
      cycle_cost_per_kwh: this.cycleCostPerKwh,
      export_price_ratio: this.exportPriceRatio,
      tariff_model: this.tariffModel,
      pv_curtailment_enabled: this.curtailmentAvailable,
      dp_flatten_arb_gate: this.flattenArbGate,
      dp_flatten_pv_shift: this.flattenPvShift,
      dp_charge_export_gate: this.chargeExportGate,
      dp_trickle_cap_saturation: this.trickleCapSat,
      dp_trickle_cap_positive: this.trickleCapPos,
      dp_reserve_waiver_reorder: this.reserveWaiver,
      dp_weak_pv_tie_standby: this.weakPvTieStandby,
      dp_weak_pv_in_dp: this.weakPvInDp,
      dp_weak_pv_shadow: false,
      dp_charge_repay_gate: this.chargeRepayGate,
      dp_cvar_reserve: this.cvarReserve,
      // Never cloned: the family-B counters format a date per slot (Intl) and describe the
      // real run only.
      dp_gate_b_count: false,
    };
  }

  /**
   * €/kWh forgone by NOT exporting this slot's surplus — exportValue(), floored at 0 when the
   * install can curtail. The DP books export purely as forgone revenue (effectiveChargeCost,
   * vPreserve), so an unfloored negative export value reads as a payment for charging: the DP
   * would fill the cell to "collect" a penalty it could dodge for free by throttling.
   * Only the PV-surplus sites use this. Battery discharge never exports at all (P1 firmware
   * enforces zero-on-meter, see the note at compute()), so there is no discharge site to floor.
   */
  _disposalValue(priceSlot) {
    return disposalValue(priceSlot, this.tariffModel, this.exportPriceRatio ?? 1.0, this.curtailmentAvailable);
  }
  ...
  /**
   * Refill-reserve floor height as the CVaR of the evening shortfall, in SoC grid units.
   *
   *   shortfall = eveningNeed - refill,  refill ~ (mean, sigma)
   *   reserve   = clamp(eveningNeed - CVaR_worst(refill), 0, maxFrac * usableSpan)
   *
   * Replaces `(1 - refillConfidence) * maxFrac * span`, whose height rises with forecast
   * uncertainty and therefore tends to sit above the post-peak SoC where it never binds.
   * This height instead asks what the evening actually needs that PV will plausibly fail to
   * deliver, so demonstrable coverage collapses it to zero and a genuinely uncovered evening
   * raises it — the "margin instead of binary" waive that a curtailment-blind forecast makes
   * too risky to do as an on/off switch.
   *
   * `k` is the CVaR depth in standard deviations of downside: k=0 is risk-neutral (the point
   * estimate), k≈0.80 is the mean of the worst 50%, k≈1.40 the worst 20%, k≈1.76 the worst 10%.
   * Passed as k rather than α so no inverse-normal is needed; the offline replay sweeps it.
   * `sigmaRel` is the measured downside spread of the refill — see CVAR_SIGMA_DOWN_REL.
   *
   * @returns {number} reserveAddG, ≥ 0
   */
  static cvarReserveAddG({
    eveningNeedKwh, refillMeanKwh, usableSpanKwh,
    capacityKwh, gridTotal, maxFrac,
    k = CVAR_K, sigmaRel = CVAR_SIGMA_DOWN_REL,
  }) {
    if (!(eveningNeedKwh > 0) || !(usableSpanKwh > 0) || !(capacityKwh > 0)) return 0;
    const refillWorstKwh = Math.max(0, refillMeanKwh * (1 - k * sigmaRel));
    const shortfallKwh = Math.min(
      Math.max(0, eveningNeedKwh - refillWorstKwh),
      maxFrac * usableSpanKwh,
    );
    return Math.round((shortfallKwh / capacityKwh) * gridTotal);
  }
  ...
  /**
   * Price a rolled-out plan in €, over an optional [fromMs, untilMs) window.
   *
   * Lives here rather than in the replay harnesses so the in-app weak-PV shadow and
   * tools/dp-replay-weakpvtie.js score the same way — two copies of an accounting rule drift.
   *
   * Two decisions in here are hard-won, do not "simplify" them:
   *  - the energy that entered the pack is read off the plan's OWN SoC path, not from action
   *    labels. A label set (pvStoreWins || trickle || charge) misses the preserve+PV branch,
   *    which charges for free without setting any of them; that PV was then booked as exported,
   *    with revenue, for energy that never left the house — 2.688 kWh on one 08-15 dump.
   *    See feedback_score_energy_from_soc_path_not_labels.
   *  - a charge is costed on its GRID fraction only; the PV fraction is not bought.
   *
   * @param {Array} slots  — plan slots from _schedule.slots
   * @param {object} o
   * @param {number} o.rte, o.cycleCostPerKwh, o.slotH, o.capacityKwh
   * @param {function} o.exportValueFn — priceSlot => €/kWh actually earned by exporting
   * @param {Array} o.priceSlots — the price slots compute() ran on, same order as `slots`
   * @param {number} [o.fromMs], [o.untilMs] — window bounds, defaults to the whole plan
   * @returns {{eur:number, chKwh:number, disKwh:number, expKwh:number, storedKwh:number, n:number}}
   */
  static scorePlanSlots(slots, {
    rte, cycleCostPerKwh, slotH, capacityKwh, exportValueFn, priceSlots,
    fromMs = -Infinity, untilMs = Infinity,
  }) {
    let eur = 0; let chKwh = 0; let disKwh = 0; let expKwh = 0; let storedKwh = 0; let n = 0;
    for (let i = 0; i < slots.length; i++) {
      ...
      if (k) {
        if (s.action === 'charge') {
          eur -= k * s.price * (1 - (s.pvCoverage ?? 0)) + 0.5 * cycleCostPerKwh * k;
          chKwh += k;
        } else if (s.action === 'discharge') {
          eur += k * rte * s.price - 0.5 * cycleCostPerKwh * k;
          disKwh += k;
        }
      }
      ...
      // Whatever the SoC path did not absorb left the house.
      ...
      const toBatt = Math.min(surplusKwh, rise);
      const toGrid = surplusKwh - toBatt;
      storedKwh += toBatt;
      expKwh += toGrid;
      eur -= 0.5 * cycleCostPerKwh * toBatt;
      eur += toGrid * exportValueFn(priceSlots[i]);
    }
    return { eur, chKwh, disKwh, expKwh, storedKwh, n };
  }

  /**
   * Value of the SoC still in the pack at the end of the scored window, at what it can
   * realistically fetch: the mean price over the window, derated by RTE and the discharge half
   * cycle. Without this term a plan that simply ends emptier reads as profitable while it only
   * spent stock — see feedback_replay_must_price_residual_soc.
   */
  static residualEur(slots, { capacityKwh, rte, cycleCostPerKwh, fromMs = -Infinity, untilMs = Infinity }) {
    ...
    const kwh = (last.socProjected / 100) * capacityKwh;
    const priced = inWin.filter(s => s.price != null);
    const meanPrice = priced.reduce((a, s) => a + s.price, 0) / (priced.length || 1);
    return { kwh, eur: kwh * (rte * meanPrice - 0.5 * cycleCostPerKwh), socPct: last.socProjected };
  }
  ...
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0, terminalPvKwhTomorrow = pvKwhTomorrow, pvCloudFactor = 1.0, refillConfidence = 1.0, pvTimingRobust = false, maxChargePrice = 0) {
    ...
    // Diagnostic only (no behaviour): how often does each gate that still assumes
    // "tomorrow's PV arrives for free" actually hold over the horizon? Family B of
    // project_dp_saldering_assumptions_inventory_0824. Filled here, in the post-DP loop
    // below and in _runBackwardDP; read by tools/dp-familyB-count.js.
    this._gateB = !this.gateBCount ? null : {
      N, pvKwhTomorrow, capacityKwh,
      b3_pvHeadroomOpen: pvHeadroomOpen,
      b2_pvSaturatesAhead: 0,
      b8_cheaperPvAhead: 0, b8_blockedStore: 0,
      b5_pvAbundant: 0,
      b6_reserveAddG: 0, b6_reserveSlots: 0,
      b6_refillRawKwh: 0, b6_eveningNeedKwh: 0,
      b1_pvKwhFromT0: 0, b1_pvKwhTomorrowPart: 0,
    };
    ...
    // Slots with a negative import price are planned with the inverter switched off
    // (lib/curtailment.js): the grid pays per kWh drawn, so producing at all costs
    // money. Zeroing pvW here is the single channel for that — every downstream PV
    // aggregate (pvKwhFromT, pvSaturatesAhead, pvAbundant, refillConfidence,
    // pvWForDischarge) reads pvWPerSlot, so none of them counts on PV that will not
    // be there. Runs once per slot in this map, never inside the socG loop.
    const pvIndex = this._buildPvIndex(pvForecast);
    const pvWPerSlot = [];
    const pvCoverage = prices.map((p, t) => {
      const pvW   = (this.curtailmentAvailable && fullCurtailSlot(p))
        ? 0
        : this._getPvForSlot(pvIndex, new Date(p.timestamp).getTime());
    ...
    const { dp, policy, chargeSocDeltaG, perSlotDischargeSocDeltaG, chargeKwhFull, pvStrongCoverage,
            pvGainCoverage, slot0ChargeSocDeltaG, slot0ChargeKwhFull, reserveFloorG, effectiveDischargePowerW, terminalFactor,
    ...
    if (this._gateB) {
      // B1: how much of the free-PV suffix sum the gates read at t=0 sits past the day edge.
      // Derived from the returned suffix array itself, so no second copy of the PV formula.
      // Day edge from slot 0, not the wall clock — replayed dumps must split on their own day.
      const dayOf = p => new Date(p.timestamp)
        .toLocaleString('en-CA', { timeZone: 'Europe/Amsterdam' }).slice(0, 10);
      const day0 = dayOf(prices[0]);
      const firstTomorrow = prices.findIndex(p => dayOf(p) !== day0);
      this._gateB.b1_pvKwhTomorrowPart = firstTomorrow < 0 ? 0 : (pvKwhFromT[firstTomorrow] ?? 0);
    }
    ...
    // Highest price anywhere in the horizon — the reference the terminal value itself is built
    // on (see the terminal block in _runBackwardDP), reused by the charge repayability test so
    // energy meant for after the horizon is not judged against in-horizon prices alone.
    const horizonMaxPrice = Math.max(prices[0]?.price > 0 ? prices[0].price : 0, suffixMaxPrice[0] ?? 0);
    ...
    // Suffix-min of the EXPORT value over future pvStrong slots. Needed only where the store
    // value is clamped to zero or below (trickle cap binding, or wear exceeding a thin cap):
    // storing is then purely about dodging a negative export price, and `storeValue > exportVal`
    // degenerates — every slot with exportVal < 0 clears it, so the gate can say "store" but
    // never "store HERE rather than there", and the forward pass fills at the first PV slot
    // instead of the one carrying the deepest penalty. This restores the ordering.
    // Inert under saldering: there exportVal === price and the gate already requires price > 0,
    // so a clamped (≤ 0) store value can never beat it and the extra term never binds.
    const minFuturePvStrongExport = new Array(N).fill(Infinity);
    for (let k = N - 2; k >= 0; k--) {
      const nextCandidate = pvCoverage[k + 1] >= pvStrongCoverage
        ? this._disposalValue(prices[k + 1]) : Infinity;
      minFuturePvStrongExport[k] = Math.min(nextCandidate, minFuturePvStrongExport[k + 1]);
    }
    // Post-saldering the deferral question below (cheaperPvAhead) changes shape twice over.
    // Storing PV costs the forgone EXPORT revenue, not the retail price — under saldering those
    // are the same number, which is why the retail comparison held. And a PV slot on the far side
    // of the peak is not an alternative at all: the pack discharges INTO that peak and has room
    // for both, so the two kWh never compete for the same cell. Window each slot's candidates to
    // (t, nextPeakIdx[t]] and price them in export value.
    // One backward pass: a new maximum at k+1 becomes the window edge and resets the running min.
    const nextPeakIdx = new Int32Array(N);
    const minPvStrongExportToPeak = new Array(N).fill(Infinity);
    if (N > 0) {
      nextPeakIdx[N - 1] = N - 1;
      let peak = N - 1;
      let runMin = Infinity;
      for (let k = N - 2; k >= 0; k--) {
        if ((prices[k + 1]?.price ?? 0) >= (prices[peak]?.price ?? 0)) {
          peak = k + 1;
          runMin = Infinity;
        }
        nextPeakIdx[k] = peak;
        if (pvCoverage[k + 1] >= pvStrongCoverage) {
          runMin = Math.min(runMin, this._disposalValue(prices[k + 1]));
        }
        minPvStrongExportToPeak[k] = runMin;
      }
    }
    ...
    const tcDebug = this._trickleCapDebug = {
      ...
      posFlag: this.trickleCapPos, nCapPos: 0, nCapPosDropped: 0, firstDropT: null,
    };

    const repayDebug = this._chargeRepayDebug = {
      flag: this.chargeRepayGate, wouldFire: 0, kwh: 0, worstShortfall: 0, worstAt: null,
    };
    ...
    for (let t = 0; t < N; t++) {
      ...
      // on) — with the flag off this reduces to the previous unconditional fallback, which is
      // the non-monotone shape described at the flag's declaration.
      ...
      // A POSITIVE cap carries the same unchecked assertion, and there the fallback is not even
      // available: the capped price is taken as-is, so a peak beyond the reset slot is dropped
      // however unreachable that reset actually makes it. Same test, same answer — free PV ahead
      // must really cover the remaining room before the cap is honoured. Behind trickleCapPos;
      // off, this reduces to the previous unconditional `trickle > 0 → trickle`.
      const capHonoured = !this.trickleCapPos || pvSaturatesAhead;
      const rawStorePrice = (trickleSuffixMaxPrice[t] > 0 && capHonoured)
                          ? trickleSuffixMaxPrice[t]
                          : ((trickleSuffixMaxPrice[t] <= 0 && capBinds) ? 0 : suffixMaxPrice[t]);
      const storeValue  = storeVal(rawStorePrice, effectiveRte, cycleCostPerKwh);
      const exportValT  = this._disposalValue(prices[t]);
      const pvStoreBeatsExport = storeValue > exportValT;
      // Store value at or below zero means the kWh is not worth holding on its own merits —
      // the only gain left is dodging a negative export price. That gain is largest at the
      // slot with the deepest penalty, so defer to a later pvStrong slot that beats this one,
      // but only while free PV ahead still fills the room the deferral leaves open
      // (pvSaturatesAhead, same test the trickle cap uses) — otherwise deferring costs volume.
      const deeperExportPenaltyAhead = storeValue <= 0
        && minFuturePvStrongExport[t] < exportValT - 1e-9
        && pvSaturatesAhead;
      ...
      // Under asymmetric_2027 the same question is asked in export value, and only of the slots
      // that actually compete for this cell: those at or before the peak that prices the store
      // value (see nextPeakIdx above). The third leg is the honest version of the
      // eveningCoverageAtRisk net below — not "does the evening work out", but "does the PV left
      // in that window still fill the room the deferral leaves open". Live 2026-08-24 14:00Z:
      // deferring to the NEXT DAY's PV block cost the pack 22pp that the same plan then drained
      // to 6.6% by 22:45 and bought back at EUR0.30-0.33.
      const deferWindowPvKwh = this.tariffModel === 'asymmetric_2027'
        ? Math.max(0, (pvKwhFromT[t + 1] ?? 0) - (pvKwhFromT[nextPeakIdx[t] + 1] ?? 0))
        : 0;
      const cheaperPvAhead = this.tariffModel === 'asymmetric_2027'
        ? (pvCoverage[t] >= pvStrongCoverage
          && minPvStrongExportToPeak[t] < Infinity
          && exportValT > minPvStrongExportToPeak[t] * 1.30
          && exportValT - minPvStrongExportToPeak[t] >= 0.03
          && deferWindowPvKwh >= roomKwh)
        : (pvCoverage[t] >= pvStrongCoverage
      ...
      if (this._gateB) {
        if (pvSaturatesAhead) this._gateB.b2_pvSaturatesAhead++;
        if (cheaperPvAhead) this._gateB.b8_cheaperPvAhead++;
        // The one that costs money: every other leg of the store test passed, and only
        // cheaperPvAhead (unblocked by the evening net) turned the store into an export.
        if (cheaperPvAhead && !eveningCoverageAtRisk && !pvStoreWins
            && code === 3 && !hasFutureStrongNeg && pvCoverage[t] >= pvStrongCoverage
            && price > 0 && pvStoreBeatsExport && !deeperExportPenaltyAhead) {
          this._gateB.b8_blockedStore++;
        }
      }
      ...
      // With weakPvInDp on the backward pass prices this band itself (pvGainCoverage drops to the
      // firmware floor, so vPreserve carries the wear and the forgone export), and this override
      // would be a second decider on a question already answered — the exact divergence CLAUDE.md
      // §Data Source Discipline is about. Suppressed only for the weak band: the strong band's
      // pvStoreWins above is untouched, and below the firmware floor nothing charges either way.
      const weakOwnedByDp = this.weakPvInDp && pvCoverage[t] < pvStrongCoverage;
      // Same degeneracy as pvStoreBeatsExport above: at a negative export price a zeroed
      // pvStoreValue2 clears `> _exportVal` on every slot of the block, so without the
      // ordering guard the trickle branch simply inherits the store the gate just declined.
      const trickleWins = (pvStoreValue2 > _exportVal || weakReachable) && !deeperExportPenaltyAhead;
      const pvTrickle = !pvStoreWins && !weakOwnedByDp && trickleWins && code === 3 && pvCoverage[t] > 0;
      ...
      // Repayability on a CHARGE slot. Putting 1 kWh of SoC in costs price + half a cycle; the
      // best that kWh can fetch later is the highest price still ahead × RTE, itself minus the
      // other half. Energy meant for after the horizon is covered by the same terminal ceiling
      // the DP priced dp[] with, so a legitimate hoard for tomorrow is not cancelled. Predicate
      // is identical to invariant 52 in test/optimizer-properties.test.js.
      // This filters the rolled-out plan; it does not correct the half-cycle round-trip pricing
      // in the backward pass — the pvAbundant waiver stays, removing it measured worse over 52
      // dumps (−€0.036/horizon at the DP's own residual value, −13.4 kWh discharge).
      const repayCost  = price + 0.5 * cycleCostPerKwh;
      const repayValue = suffixMaxPrice[t] * effectiveRte - 0.5 * cycleCostPerKwh;
      const repayCeil  = horizonMaxPrice * 0.8 * effectiveRte * terminalFactor;
      const chargeUnrepayable = code === 1 && price > 0
        && repayCost > Math.max(repayValue, repayCeil) + 0.002;
      if (chargeUnrepayable && this.chargeRepayGate) {
        action = 'standby'; actionSrc = ACTION_SRC.UNREPAYABLE;
      }

      // Either test can cancel the charge. Shared so the actionKwh and SoC-advance blocks below
      // stay in step with the action stamped above.
      const chargeCancelled = chargeExportLoses || (chargeUnrepayable && this.chargeRepayGate);
      ...
      if (tcDebug) {
        ...
        // Positive-cap counters. nCapPosDropped is the set of slots where the cap named a price
        // but the PV ahead did not cover the room — the slots this gate re-values upward.
        // Flag-independent in the same sense as wouldBind: pvSaturatesAhead does not read it.
        if (trickleSuffixMaxPrice[t] > 0) {
          tcDebug.nCapPos++;
          if (!pvSaturatesAhead) {
            tcDebug.nCapPosDropped++;
            if (tcDebug.firstDropT === null) tcDebug.firstDropT = t;
          }
        }
      }
      ...
      slots.push({ timestamp: prices[t].timestamp, action, actionSrc, price, exportPrice: prices[t].exportPrice ?? null, socProjected: socG / GRID, pvExportWins,
        ...
        pvTrickleCapHonoured: capHonoured,
        pvCoverage: pvCoverage[t],
        // Which layer owns the store-vs-export choice on this slot. True only with
        // dp_weak_pv_in_dp on and a weak surplus: then the backward pass priced the band itself,
        // so policy-engine must follow `action` here instead of re-deriving the choice
        // (CLAUDE.md: one decider). Flag off -> always false -> mapper unchanged.
        pvWeakOwnedByDp: weakOwnedByDp,
      ...
      if (code === 1 && chargeCancelled) {
        // Charge cancelled by the export or repayability test: the PV goes to the grid, so neither SoC nor the
        ...
        socG = Math.min(maxSocG, socG + slotDeltaG);
      } else if (code === 0 && prices[t].price >= 0
                 && (this.weakPvInDp
                   ? pvCoverage[t] >= pvGainCoverage
                   : (pvCoverage[t] >= pvStrongCoverage && pvStoreBeatsExport))) {
        // Preserve + PV: the battery charges for free. Which coverage counts is the same
        // threshold the backward pass credited the gain at (pvGainCoverage), so the projected
        // SoC follows the valued path — socProjected has one writer, the DP.
        // With the flag OFF the threshold stays pvStrongCoverage and the store-vs-export test is
        // repeated here, because there the backward pass never priced it: without that repeat the
        // runtime exports the surplus (standby) while the plan projects a charge.
        // With the flag ON vPreserve already carries the forgone export, so repeating the test
        // would be a second decider overruling the one that valued the plan.
    ...
    // Total energy used in the window stays within the same budget.
    {
      ...
      if (windowLen >= 2) {
        ...
        if (eligible.length >= 2 && !hasCharge) {
          ...
          const reserveEndG = wEnd > 0 ? reserveFloorG[wEnd - 1] : minSocG;
          const dpEndTargetG = Math.max(rawEndG, reserveEndG);
          // Slots the reserve waived (own price > releasedPeak → reserveFloorG[t] === minSocG)
          // must not inherit the window-end reserve level: that re-imposes exactly the hoarding
          // the waiver exists to prevent. rawEndG is not an independent second cause — it IS the
          // SoC the floor froze (measured on the real 2026-09-08 22:32 dump: reserve on → SoC path
          // 11,11,11…; reserve off → 11,5,5…), so the reserve's own contribution comes off it too.
          const reserveHoldG = Math.max(0, reserveEndG - minSocG);
          const waivedEndG = Math.max(minSocG, rawEndG - reserveHoldG);
          const anyWaived = eligible.some(t => reserveFloorG[t] <= minSocG);
          const waiverOn = !!this.reserveWaiver && anyWaived && reserveHoldG > 0;
          const effFloorG = t => (reserveFloorG[t] > minSocG
            ? Math.max(reserveFloorG[t], dpEndTargetG)
            : Math.max(minSocG, waiverOn ? waivedEndG : dpEndTargetG));
          ...
          this._reorderDebug = {
            ...
            // Reserve-waiver shadow. Counts only — how many eligible slots the reserve waived and
            // how much budget the window floor withholds from them. No € claim: the number cannot
            // come out negative, so it measures exposure, not gain.
            waiverOn,
            waivedN: eligible.filter(t => reserveFloorG[t] <= minSocG).length,
            waiverKwh: +((Math.max(0, dpEndTargetG - waivedEndG) / GRID_TOTAL) * capacityKwh).toFixed(3),
            trace: _rdTrace,
          };
          let changed = false;
          ...
          if (changed) {
            ...
            // Undershoot rollback must use the SAME target the budget was built on. Left at
            // dpEndTargetG it reverts every waived-slot rewrite (the plan legitimately ends below
            // the reserve level) and the waiver reads as inert — the reorder-rollback trap.
            const dpEndSocG = wEnd < N ? (waiverOn ? waivedEndG : dpEndTargetG) : socGw;
    ...
    // Shadow counters for the repayability test, over the WHOLE horizon and over the FINAL plan.
    // Counting inside the forward loop instead overstates it: that loop sees `code === 1`, and
    // the post-DP passes above rewrite part of those slots afterwards (04-08 dumps: 4 hits in the
    // loop, zero charge slots left in the plan). Flag-independent — with the gate off these
    // numbers say what switching it on would change; with it on they must read zero.
    for (let t = 0; t < slots.length; t++) {
      const s = slots[t];
      if (s.action !== 'charge' || !(s.actionKwh > 0) || !(s.price > 0)) continue;
      const cost  = s.price + 0.5 * cycleCostPerKwh;
      const value = Math.max(suffixMaxPrice[t] * effectiveRte - 0.5 * cycleCostPerKwh,
        horizonMaxPrice * 0.8 * effectiveRte * terminalFactor);
      if (cost > value + 0.002) {
        repayDebug.wouldFire++;
        repayDebug.kwh += s.actionKwh;
        if (cost - value > repayDebug.worstShortfall) {
          repayDebug.worstShortfall = cost - value;
          repayDebug.worstAt = s.timestamp;
        }
      }
    }

    this._schedule = { computedAt: Date.now(), projectedProfit, todayProjectedProfit, slots, terminalFactor, terminalPvKwh: terminalPvKwhTomorrow, chargeRepay: repayDebug };
    ...
    this._runWeakPvShadow({
      slots, prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast,
      rte, consumptionWPerSlot, minDischargePrice, consumptionMargin, pvKwhTomorrow,
      terminalPvKwhTomorrow, pvCloudFactor, refillConfidence, pvTimingRobust, maxChargePrice,
      pvCoverage, pvStrongCoverage, slotH, effectiveRte, cycleCostPerKwh, N,
    });
  }

  /**
   * Shadow measurement for dp_weak_pv_in_dp: what would the other setting of that flag have
   * planned, and what is the difference worth? Diagnostic only — nothing here feeds a decision.
   *
   * Two rules this obeys, both the hard way:
   *  - It clones the engine and re-runs the FULL compute(), not a second _runBackwardDP. Scoring
   *    needs socProjected, and that only exists after the forward pass.
   *  - Both plans go through the SAME scorer (scorePlanSlots + residualEur). Comparing the two
   *    dp[] values instead would be meaningless: with the flag on the backward pass prices the
   *    weak band, so the on-run's own dp[] is higher by construction.
   *
   * Skipped when the horizon holds no weak-PV slot (every night run) — nothing could differ —
   * and inside the shadow run itself (_isShadow), which is what stops the recursion. Costs one
   * extra compute() on day runs (~206 ms for the backward pass,
   * project_scenario_dp_cost_measured_0828).
   */
  _runWeakPvShadow(ctx) {
    this._weakPvShadow = null;
    if (this._isShadow || !this.weakPvShadow) return;
    const { slots, prices, pvCoverage, pvStrongCoverage, slotH, effectiveRte, cycleCostPerKwh, N } = ctx;
    const isWeak = c => c > 0 && c < pvStrongCoverage;
    if (!pvCoverage.some(isWeak)) return;

    const shadow = new OptimizationEngine({
      ...this._settingsSnapshot(),
      dp_weak_pv_in_dp: !this.weakPvInDp,
    });
    shadow._isShadow = true;
    shadow.compute(
      prices, ctx.currentSoc, ctx.capacityKwh, ctx.maxChargePowerW, ctx.maxDischargePowerW,
      ctx.pvForecast, ctx.rte, ctx.consumptionWPerSlot, ctx.minDischargePrice,
      ctx.consumptionMargin, ctx.pvKwhTomorrow, ctx.terminalPvKwhTomorrow, ctx.pvCloudFactor,
      ctx.refillConfidence, ctx.pvTimingRobust, ctx.maxChargePrice,
    );
    const shadowSlots = shadow._schedule?.slots || [];
    if (shadowSlots.length !== slots.length) return;

    const scoreOpts = {
      rte: effectiveRte, cycleCostPerKwh, slotH, capacityKwh: ctx.capacityKwh,
      exportValueFn: p => this._disposalValue(p), priceSlots: prices,
    };
    const resOpts = { capacityKwh: ctx.capacityKwh, rte: effectiveRte, cycleCostPerKwh };
    const sBase = OptimizationEngine.scorePlanSlots(slots, scoreOpts);
    const sShad = OptimizationEngine.scorePlanSlots(shadowSlots, scoreOpts);
    const rBase = OptimizationEngine.residualEur(slots, resOpts);
    const rShad = OptimizationEngine.residualEur(shadowSlots, resOpts);

    // Decision scope, measured on the realised plans: the two runs differ in one setting, so a
    // slot that differs IS the decision. nDiff = 0 means this run could not move, and reporting
    // its 0.000 as evidence would be the flat-log trap.
    //
    // Counted on BOTH the action label and the SoC path, because neither alone is the decision:
    //  - 'preserve' covers two different outcomes here. A weak surplus that the battery absorbs
    //    and one that leaves the house are both 'preserve'; only socProjected separates them
    //    (feedback_score_energy_from_soc_path_not_labels). Measured: at export ratio 0.30 the two
    //    plans are label-identical while the shadow ends 7.6 pp higher.
    //  - and the reverse happens too: at ratio 0.90 'preserve' (flat SoC) vs 'standby' is two
    //    labels for the same physics, 0 kWh apart. Label churn is not a decision.
    let nDiffAction = 0;
    let nDiffSoc = 0;
    let nDiff = 0;
    let firstDiffT = null;
    const socStep = (arr, t) => (arr[t + 1]?.socProjected ?? arr[t].socProjected) - arr[t].socProjected;
    for (let t = 0; t < slots.length; t++) {
      const dAct = slots[t].action !== shadowSlots[t].action;
      // 0.05 pp ~ 1.3 Wh on a 2.7 kWh pack: below it the paths are the same plan.
      const dSoc = Math.abs(socStep(slots, t) - socStep(shadowSlots, t)) > 0.05;
      if (dAct) nDiffAction++;
      if (dSoc) nDiffSoc++;
      if (dAct || dSoc) {
        nDiff++;
        if (firstDiffT === null) firstDiffT = t;
      }
    }

    const eurBase = sBase.eur + rBase.eur;
    const eurShadow = sShad.eur + rShad.eur;
    const cons = ctx.consumptionWPerSlot || [];
    const r4 = v => Math.round(v * 1e4) / 1e4;
    this._weakPvShadow = {
      // Which side is which: base is the plan that shipped, shadow is the flag flipped.
      flagOn: this.weakPvInDp,
      nWeakSlots: pvCoverage.filter(isWeak).length,
      nDiff,
      nDiffAction,
      nDiffSoc,
      firstDiffT,
      action0Base: slots[0]?.action ?? null,
      action0Shadow: shadowSlots[0]?.action ?? null,
      eurBase: r4(eurBase),
      eurShadow: r4(eurShadow),
      dEur: r4(eurShadow - eurBase),
      residualBase: r4(rBase.eur),
      residualShadow: r4(rShad.eur),
      socEndBase: rBase.socPct,
      socEndShadow: rShad.socPct,
      // Input coverage. Without it a € difference is not evidence: a flat consumption default
      // has flipped a replay's sign before (2026-07-24/25). And the surplus this flag decides
      // over comes from a Math.max with PREDICTED consumption
      // (project_avg_consumption_w_is_prediction_not_measurement), so the measurement can only
      // exceed the forecast, never contradict it.
  ...
  computeExpectedProfit(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0) {
    ...
    const pvWPerSlot = prices.map(p => ((this.curtailmentAvailable && fullCurtailSlot(p))
      ? 0 // planned inverter-off slot — same mask as compute(), see the note there
      : this._getPvForSlot(pvIndex2, new Date(p.timestamp).getTime())));
  ...
  _runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
                 ...
                 slot0RemainingFrac = 1.0, pvWPerSlot = null, terminalPvKwhTomorrow = pvKwhTomorrow, refillConfidence = 1.0, pvTimingRobust = false, pvSpreadPerSlot = null, maxChargePrice = 0, currentSoc = null, weakPvInDp = this.weakPvInDp) {
    const GRID_TOTAL = GRID * 100;
    ...
    // pvStrong threshold, expressed as a coverage fraction of maxChargePowerW.
    // The quantity is NET PV SURPLUS, because pvCoverage is (pvW - consumptionW) /
    // maxChargePowerW. policy-engine's `pvStrong` gates the same mapper branch on RAW
    // pv (PV_STRONG_RAW_W) — a different quantity that shares the number, not a mirror.
    const pvStrongCoverage = maxChargePowerW > 0 ? PV_STRONG_SURPLUS_W / maxChargePowerW : 0.15;
    // Below which coverage does the firmware not charge from PV at all. Only meaningful with
    // weakPvInDp on; off, the SoC gain still starts at pvStrongCoverage and this is unused.
    const pvChargeFloorCoverage = maxChargePowerW > 0 ? PV_CHARGE_FLOOR_W / maxChargePowerW : 0.09;
    const pvGainCoverage = weakPvInDp ? pvChargeFloorCoverage : pvStrongCoverage;

    // Last strong-PV slot: boundary between the PV window and the evening/night tail.
    // Shared by the reserve floor and by eveningNeedKwh below — one definition of
    // "where the evening tail starts," not two.
    let lastStrongPv = -1;
    for (let t = N - 1; t >= 0; t--) {
    ...
    // below — feeds the forward-pass safety net that stops cheaperPvAhead from deferring
    // charging past the point where this need can no longer be covered, and (when
    // dp_cvar_reserve is on) sizes the reserve floor itself.
    ...
    // Usable SoC span in kWh — one definition for the whole function (the reserve floor,
    // the terminal-value block below and compute():495 all mean the same quantity).
    const usableSpanKwh = ((maxSocG - minSocG) / GRID_TOTAL) * capacityKwh;

    // ── Overnight refill-reserve floor (per-slot) ─────────────────────────────
    // When the PV forecast is uncertain, hold a SoC buffer through overnight non-PV slots
    // that PRECEDE a strong-PV refill window, so the evening peak stays served even if the
    // next day's PV under-delivers vs forecast. Slots after the last strong-PV refill
    // (evening peak) keep minSocG so discharge there is unconstrained. When nothing needs
    // insuring the floor collapses to minSocG → no change.
    const RESERVE_MAX_FRAC = 0.5; // cap: reserve at most 50% of usable span
    const reserveAddG = this.cvarReserve
      ? OptimizationEngine.cvarReserveAddG({
        eveningNeedKwh,
        refillMeanKwh: Math.min(storablePrefix[lastStrongPv + 1], usableSpanKwh),
        usableSpanKwh,
        capacityKwh,
        gridTotal: GRID_TOTAL,
        maxFrac: RESERVE_MAX_FRAC,
      })
      : Math.round((1 - refillConfidence) * RESERVE_MAX_FRAC * (maxSocG - minSocG));
    ...
    if (this._gateB) {
      this._gateB.b6_reserveAddG = reserveAddG;
      // Ongecapt, zodat tools/scratchpad de spreiding van de bijvulling zelf kan meten:
      // min(.., usableSpan) hieronder maakt voorspeld en gerealiseerd anders per constructie gelijk.
      this._gateB.b6_refillRawKwh = storablePrefix[lastStrongPv + 1];
      this._gateB.b6_eveningNeedKwh = eveningNeedKwh;
      for (let t = 0; t < N; t++) if (reserveFloorG[t] > minSocG) this._gateB.b6_reserveSlots++;
      this._gateB.b1_pvKwhFromT0 = pvKwhFromT[0];
    }
    ...
    // Scratch copy of dp for the flatten shift, which reads ahead while overwriting dp.
    // Allocated once for the same reason as dpSufMax.
    const dpSrc = new Float64Array(GRID_TOTAL + 1);
    ...
    for (let t = N - 1; t >= 0; t--) {
      ...
      const exportVal = this._disposalValue(prices[t]);
      // Same reason: the discharge-wear waiver below reads only t. Counted once per slot.
      const pvAbundant = pvKwhTomorrow >= capacityKwh * 1.5 && pvCoverage[t] < 0.1;
      if (pvAbundant && this._gateB) this._gateB.b5_pvAbundant++;
      ...
      //   (exportValue() × pvCoverage).
      // Under saldering exportValue() === price, so this simplifies to `price`, making vCharge
      ...
      // Charging from PV forgoes exporting it (worth exportValue() this slot); the grid
      // fraction costs the retail price. Under saldering exportValue() === price, so this
      ...
      // Depends on t only — hoisted out of the socG loop (see the discharge block below).
      const effectiveMinDischarge = Array.isArray(minDischargePrice)
        ? (minDischargePrice[t] ?? 0) : minDischargePrice;

      for (let socG = 0; socG <= GRID_TOTAL; socG++) {

        // Preserve: credit the free SoC gain from pvGainCoverage upward. Where that threshold
        // sits is the whole weakPvInDp question. Off it is pvStrongCoverage, and the comment
        // here used to justify that with the firmware running zero_charge_only only above 400 W.
        // That is refuted: live log 2026-09-10 11:00 Amsterdam shows zero_charge_only at a 237 W
        // surplus, charging 260-340 W, SoC 45% -> 47%. PV_STRONG_SURPLUS_W is a tuning constant,
        // not a hardware limit, so with the flag off the weak band is standby by choice — and,
        // because the gain is 0 there, a choice the DP cannot even express. On, the threshold
        // drops to the real firmware floor and the terms below make preserve and standby differ.
        ...
        const pvSocGainG  = (!isNegativePrice && pvCoverage[t] >= pvGainCoverage)
        ...
        // shows 'standby' for those slots. (effectiveMinDischarge is hoisted above the loop.)
        let vDischarge = -1e9;
    ...
    return { dp, policy, chargeSocDeltaG, perSlotDischargeSocDeltaG, chargeKwhFull, pvStrongCoverage, pvGainCoverage, slot0ChargeSocDeltaG, slot0ChargeKwhFull, reserveFloorG, effectiveDischargePowerW, terminalFactor, pvKwhFromT, eveningNeedKwh, lastStrongPv, topupFiringSlots };
  }
  ...
  updateSettings(newSettings) {
    ...
    if (newSettings.pv_curtailment_enabled != null) this.curtailmentAvailable = newSettings.pv_curtailment_enabled === true;
    ...
    if (newSettings.dp_trickle_cap_positive != null) this.trickleCapPos = newSettings.dp_trickle_cap_positive;
    if (newSettings.dp_reserve_waiver_reorder != null) this.reserveWaiver = newSettings.dp_reserve_waiver_reorder;
    if (newSettings.dp_weak_pv_tie_standby != null) this.weakPvTieStandby = newSettings.dp_weak_pv_tie_standby;
    if (newSettings.dp_weak_pv_in_dp != null) this.weakPvInDp = newSettings.dp_weak_pv_in_dp;
    if (newSettings.dp_weak_pv_shadow != null) this.weakPvShadow = newSettings.dp_weak_pv_shadow;
    if (newSettings.dp_charge_repay_gate != null) this.chargeRepayGate = newSettings.dp_charge_repay_gate;
    if (newSettings.dp_cvar_reserve != null) this.cvarReserve = newSettings.dp_cvar_reserve;
    this._schedule = null; // invalidate — will recompute on next policy run
  }
}

OptimizationEngine.ACTION_SRC = ACTION_SRC;
OptimizationEngine.PV_STRONG_SURPLUS_W = PV_STRONG_SURPLUS_W;
OptimizationEngine.PV_STRONG_RAW_W = PV_STRONG_RAW_W;
OptimizationEngine.PV_CHARGE_FLOOR_W = PV_CHARGE_FLOOR_W;
OptimizationEngine.PV_ONLY_CHARGE_FLOOR_MIN_W = PV_ONLY_CHARGE_FLOOR_MIN_W;
