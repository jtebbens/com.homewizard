...
class OptimizationEngine {
  ...
  _runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
    ...
    for (let t = N - 1; t >= 0; t--) {
      ...
      // Guard: only when overall pvKwhTomorrow >= capacityKwh (strong PV day confirmed).
      // Only flatten during low/no-PV slots (pvCoverage < pvStrongCoverage). During
      // strong PV hours the vPreserve free-SoC-gain already naturally flattens dp via
      // backward induction, so explicit flattening is redundant. More importantly, if
      // we flatten during strong PV hours the DP loses sight of the evening peak value
      // and may incorrectly recommend discharge at any price > half-cycle-cost.
      if (pvKwhTomorrow >= capacityKwh && pvKwhFromT[t + 1] >= capacityKwh
