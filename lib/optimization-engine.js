...
class OptimizationEngine {
   ...
   */
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0) {
    ...
    const { dp, policy, chargeSocDeltaG, perSlotDischargeSocDeltaG, pvStrongCoverage } =
      this._runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
        ...
        minDischargePrice, maxSocG, minSocG, consumptionMargin);
   ...
   */
  computeExpectedProfit(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0) {
    ...
    for (let t = 0; t < N; t++) {
      ...
      const pvW   = pvCoverage[t] * maxChargePowerW; // raw PV watts
      const code  = policy[t][socG];
      ...
      if (code === 1) {
        ...
        // Preserve with strong PV: zero_charge_only — PV partially charges battery
        const pvToBatteryW = pvCoverage[t] * maxChargePowerW;
  ...
  _runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
                 ...
                 minDischargePrice, maxSocG, minSocG, consumptionMargin = 1.0) {
    const GRID_TOTAL = GRID * 100;
    ...
    // Instead of dp[socG] = 0 (battery worthless at end-of-horizon), assign a
    // residual value to stored energy. This prevents the DP from aggressively
    // dumping battery in the last hours of the horizon and improves multi-day
    // decisions (e.g. keeping battery full over a rainy weekend when sun returns Monday).
    //
    // Terminal value = (average of top-quartile prices in the horizon) × kWh at that SoC.
    // This conservatively estimates what stored energy could earn in the next cycle.
    // Capped at 80% of the actual horizon peak to avoid over-valuation.
    ...
    if (N >= 4) {
      ...
      if (terminalPricePerKwh > 0) {
        for (let socG = 0; socG <= GRID_TOTAL; socG++) {
          ...
          dp[socG] = terminalPricePerKwh * kwhStored;
        }
      }
    }
