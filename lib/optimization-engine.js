...
class OptimizationEngine {
  ...
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0) {
    ...
    for (let t = 0; t < N; t++) {
      ...
      // Trickle only when storing beats exporting: max future discharge value > current export value.
      // pvSufficient (tomorrow PV fills battery) was previously a separate condition but caused
      // trickle to fire when tomorrow data was missing (pvKwhTomorrow=0), bypassing the economic check.
