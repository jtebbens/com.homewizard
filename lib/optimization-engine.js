...
class OptimizationEngine {
  ...
  compute(prices, currentSoc, capacityKwh, maxChargePowerW, maxDischargePowerW, pvForecast, rte = null, consumptionWPerSlot = null, minDischargePrice = 0, consumptionMargin = 1.0, pvKwhTomorrow = 0) {
    ...
    for (let t = 0; t < N; t++) {
      ...
      slots.push({ timestamp: prices[t].timestamp, action, price, socProjected: socG / GRID,
      ...
      if (code === 1) {
        if (isToday) {
          const actualDeltaG  = Math.min(chargeSocDeltaG, maxSocG - socG);
          const gridChargeKwh = (actualDeltaG / chargeSocDeltaG) * chargeKwhFull * (1 - pvCoverage[t]);
          todayProjectedProfit -= price * gridChargeKwh;
        }
        socG = Math.min(maxSocG, socG + chargeSocDeltaG);
      } else if (code === 2) {
        if (isToday) {
        ...
        // pvStoreWins: simulate free PV charging for accurate SoC projection in planning.
        socG = Math.min(maxSocG, socG + Math.round(pvCoverage[t] * chargeSocDeltaG));
      }
      // code === 3 (standby, no pvStoreWins): PV exports to grid, battery SoC unchanged
    }
  ...
  _runBackwardDP(N, prices, pvCoverage, consumptionWPerSlot, slotH,
    ...
    for (let t = N - 1; t >= 0; t--) {
      ...
      if (prices[t].price >= 0 && pvKwhTomorrow >= capacityKwh * 0.8 && pvKwhFromT[t + 1] >= capacityKwh
      ...
      const price = prices[t].price;
      // Negative price: you earn money by importing from grid — use full price, not reduced by PV.
      // Positive price: PV covers part of the charge power, so reduce effective grid import cost.
      const isNegativePrice = price < 0;
      const effectiveChargeCost = isNegativePrice ? price : price * (1 - pvCoverage[t]);
      ...
      for (let socG = 0; socG <= GRID_TOTAL; socG++) {
        ...
        if (pvSocGainG > 0 && preserveSocG < socG + pvSocGainG && price > 0) {
          const surplusKwh = (socG + pvSocGainG - preserveSocG) / chargeSocDeltaG * chargeKwhFull;
        ...
        if (price > 0 && pvCoverage[t] > 0) {
          const pvGainKwh = pvCoverage[t] * chargeKwhFull;
