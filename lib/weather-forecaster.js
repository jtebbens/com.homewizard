...
class WeatherForecaster {
  ...
  getSatPanelWAt(ms, gtiOverGhi) {
    ...
    const coords = this._satCoords();
    const elevDeg = coords ? WeatherForecaster._solarElevAz(new Date(ms), coords.lat, coords.lon).elev : null;
    return this.satGhiToPanelW(ghi, utcH, gtiOverGhi, elevDeg);
  }
  ...
  satGhiToPanelW(ghi, utcHour, gtiOverGhi, elevDeg = null) {
    if (ghi == null) return null;
    if (elevDeg != null && elevDeg < 15) return null;
  ...
  // Parses lat/lon from this._satUrl's query string — same pattern as _fetchSatelliteNowcast's
  // stale-issue elevation check. Returns null if _satUrl is unset or unparsable.
  _satCoords() {
    const params = new URLSearchParams((this._satUrl || '').split('?')[1] || '');
    const lat = parseFloat(params.get('lat'));
    const lon = parseFloat(params.get('lon'));
