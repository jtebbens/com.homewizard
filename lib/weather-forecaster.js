...
class WeatherForecaster {
  ...
  async _fetchStandardHourly(lat, lon) {
    const params = new URLSearchParams({
      latitude: lat.toString(),
      longitude: lon.toString(),
      hourly: 'shortwave_radiation,sunshine_duration,temperature_2m,cloud_cover,cloud_cover_low,precipitation_probability,precipitation,weather_code,windspeed_10m,winddirection_10m',
  ...
  _processForecast(rawData) {
    ...
    for (let i = 0; i < maxHours; i++) {
      ...
      const ensRadiationWm2 = (() => {
        ...
        if (sumGhi > 0) gtiOverGhi = Math.max(0.3, Math.min(2.5, sum / sumGhi));
  ...
  async fetchUpwindData(lat, lon, windFromDeg) {
    if (!this._satUrl) return null;
    // Derive API base: always use /api path regardless of _satUrl path (e.g. /msgcpp/latest.json → /api)
    const baseUrl = new URL(this._satUrl).origin + '/api';
    const R = 6371, dist = 40;
    const rad = d => d * Math.PI / 180;
    // windFromDeg may be null (no OM wind yet); in that case skip upwind point fetches
    const hasWind = windFromDeg != null;
    const upLat = hasWind ? lat + (dist / R) * Math.cos(rad(windFromDeg)) * (180 / Math.PI) : null;
    const upLon = hasWind ? lon + (dist / R) * Math.sin(rad(windFromDeg)) / Math.cos(rad(lat)) * (180 / Math.PI) : null;
    try {
      const homeQgP    = fetchWithTimeout(`${baseUrl}/qg?lat=${lat.toFixed(4)}&lon=${lon.toFixed(4)}`, {}, 5000).then(r => r.ok ? r.json() : null);
      const homePointP = fetchWithTimeout(`${baseUrl}/point?lat=${lat.toFixed(4)}&lon=${lon.toFixed(4)}`, {}, 5000).then(r => r.ok ? r.json() : null);
      const upQgP    = hasWind ? fetchWithTimeout(`${baseUrl}/qg?lat=${upLat.toFixed(4)}&lon=${upLon.toFixed(4)}`, {}, 5000).then(r => r.ok ? r.json() : null)    : Promise.resolve(null);
      const upPointP = hasWind ? fetchWithTimeout(`${baseUrl}/point?lat=${upLat.toFixed(4)}&lon=${upLon.toFixed(4)}`, {}, 5000).then(r => r.ok ? r.json() : null) : Promise.resolve(null);
      const [thisQg, thisPoint, upQg, upPoint] = await Promise.all([homeQgP, homePointP, upQgP, upPointP]);
      const thisCot    = Math.max(thisPoint?.cot ?? 0, 0);
      const upwindCot  = hasWind ? Math.max(upPoint?.cot ?? 0, 0) : null;
      const station    = hasWind ? (upQg?.station ?? null) : null;
      // thisQg: dd array [{t,deg}], ff array [{t,ms}] — take latest entry
      ...
      const thisFf = Array.isArray(_ffArr) && _ffArr.length ? _ffArr[_ffArr.length - 1].ms  : null;
      const dcot = (upwindCot != null) ? Math.round((upwindCot - thisCot) * 10) / 10 : null;
      const result = { station, upwindCot, thisCot, dcot, thisStation, thisDd, thisFf };
      this.log(`[Upwind] thuis=${thisStation ?? '?'} dd=${thisDd}° ff=${thisFf}m/s · verklikker=${station ?? '?'} COT thuis=${thisCot} up=${upwindCot ?? '-'} Δ=${dcot ?? '-'}`);
      return result;
    } catch (e) {
  ...
  async _fetchSatelliteNowcast(url) {
    try {
      ...
      if (issueAge > 60 * 60 * 1000 || issueAge < -5 * 60 * 1000) { this.log(`[SAT] reject: stale issue age=${Math.round(issueAge/60000)}min (issue=${data.issue})`); return null; }
  ...
  _applySatelliteOverlay(satData) {
    ...
    for (const slot of slots) {
      ...
      const satYf = this.learningEngine?.getSatYieldFactor?.(utcH) ?? SAT_YIELD_FACTORS[utcH];
      slot.satPanelW = satYf ? Math.round(satGhi * satYf) : null;
      this.log(`[SAT] h=${slot.time.getUTCHours()} sat=${satGhi} om=${slot.radiationWm2} satPanelW=${slot.satPanelW}`);
    }
  ...
  // Satellite GHI × sat-YF → panel W for the 15-min bucket containing `ms`.
  getSatPanelWAt(ms) {
    const ghi = this.getSatGhiAt(ms);
    if (ghi == null) return null;
    const utcH = new Date(ms).getUTCHours();
    const yf = this.learningEngine?.getSatYieldFactor?.(utcH) ?? SAT_YIELD_FACTORS[utcH];
    return yf ? Math.round(ghi * yf) : null;
  }
  ...
  // First contiguous dip in SAT curve within 2h of nowMs.
  ...
  getNextSatDip(nowMs, dipThresholdFrac = 0.15, maxChargePowerW = 800) {
    const dipThresholdW = dipThresholdFrac * maxChargePowerW;
    const curve = this.getSat15minCurve(nowMs);
  ...
  static getSatYieldFactor(utcHour) {
    return SAT_YIELD_FACTORS[utcHour] || 0;
  }
