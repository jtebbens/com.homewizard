...
class WeatherForecaster {
  ...
  async _doFetch(latitude, longitude, tilt, azimuth) {
    try {
      ...
      if (this._satUrl) {
        try {
          const sat = await this._fetchSatelliteNowcast(this._satUrl, this._satToken);
      ...
      if (sampleAvg.length > 0) {
        ...
        for (let i = 0; i < times.length; i++) {
          ...
          if (leadH >= -0.5 && leadH <= 5) {
            this.log(`[OMGHI] issue=${new Date(_nowMs).toISOString()} t=${times[i]} ghi=${shortwave_radiation[i]}`);
          }
        }
      }
    } else {
  ...
  async _fetchSatelliteNowcast(url, token) {
    try {
      const headers = {};
      if (token) headers['X-PV-Token'] = token;
  ...
  _applySatelliteOverlay(satData) {
    ...
    for (const slot of slots) {
      ...
      const satYf = SAT_YIELD_FACTORS[slot.time.getUTCHours()];
  ...
  getSatPanelWAt(ms) {
    ...
    const yf = SAT_YIELD_FACTORS[new Date(ms).getUTCHours()];
  ...
  startSatelliteLoop(url, token, onOverlay) {
    this.stopSatelliteLoop();
    this._satUrl = url;
    this._satToken = token || '';
