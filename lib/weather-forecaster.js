...
class WeatherForecaster {
  ...
  _applySatelliteOverlay(satData) {
    ...
    for (const slot of slots) {
      ...
      const satYf = this.resolveSatYieldFactor(utcH);
  ...
  getSatPanelWAt(ms) {
    ...
    const yf = this.resolveSatYieldFactor(utcH);
    return yf ? Math.round(ghi * yf) : null;
  }

  // Learned (EMA) sat yield factor for this UTC hour, falling back to the
  // hardcoded SAT_YIELD_FACTORS table when not yet learned. Single source of
  // truth — every satGhi→panelW conversion (live overlay, accuracy sampling,
  // chart history) must go through this, or the surfaces silently diverge.
  resolveSatYieldFactor(utcHour) {
    return this.learningEngine?.getSatYieldFactor?.(utcHour) ?? SAT_YIELD_FACTORS[utcHour];
  }
