  ...
  async fetchUpwindData(lat, lon, windFromDeg = null) {
    ...
    try {
      ...
      const [thisQg, thisPoint, upQg, upPoint] = await Promise.all([
        ...
        upLat != null ? fetchWithTimeout(`${baseUrl}/qg?lat=${upLat.toFixed(4)}&lon=${upLon.toFixed(4)}`, _ag, 5000).then(r => r.ok ? r.json() : null) : Promise.resolve(null),
        upLat != null ? fetchWithTimeout(`${baseUrl}/point?lat=${upLat.toFixed(4)}&lon=${upLon.toFixed(4)}`, _ag, 5000).then(r => r.ok ? r.json() : null) : Promise.resolve(null),
      ]);
