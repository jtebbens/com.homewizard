...
class WeatherForecaster {
   ...
   * clear-sky = 1000*sin(elev) — so this lands on the same scale as the KNMI kt and the existing
   * 0.30 / 0.65 bucket thresholds carry over untouched. That also makes the two directly
   ...
   * @private
   */
  _computeSatKt(dateStr, nowMs = Date.now()) {
   ...
   */
  getTodaySatKtInfo(dateStr = new Date().toISOString().slice(0, 10), nowMs = Date.now()) {
    return this._computeSatKt(dateStr, nowMs);
  }
