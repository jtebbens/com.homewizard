...
class WeatherForecaster {
  ...
  _mergeApiResponses(ensembleData, standardData, tiltedData, lat, lon) {
    ...
    if (ensembleData) {
      // Average shortwave_radiation across all models; apply conservative spread discount
      // when models disagree significantly (std > 30 W/m²): use mean − 0.3×std.
      let spreadSlotsAdjusted = 0;
      shortwave_radiation = times.map((_, i) => {
        const vals = ENSEMBLE_MODELS
          .map(m => ensembleData.hourly[`shortwave_radiation_${m}`]?.[i])
          .filter(v => typeof v === 'number');
        if (vals.length === 0) return 0;
        const mean = vals.reduce((a, b) => a + b, 0) / vals.length;
        if (vals.length < 2) return Math.round(mean);
        const std = Math.sqrt(vals.reduce((s, v) => s + (v - mean) ** 2, 0) / vals.length);
        if (std > 80) {
          spreadSlotsAdjusted++;
          return Math.round(Math.max(0, mean - 0.5 * std));
        }
        if (std > 30) {
          spreadSlotsAdjusted++;
          return Math.round(Math.max(0, mean - 0.3 * std));
        }
        return Math.round(mean);
      });
      const sampleAvg = shortwave_radiation.slice(0, 24).filter(v => v > 0);
      if (sampleAvg.length > 0) {
        this.log(`Ensemble radiation averaged from ${ENSEMBLE_MODELS.length} models (sample avg: ${Math.round(sampleAvg.reduce((a, b) => a + b, 0) / sampleAvg.length)} W/m²${spreadSlotsAdjusted > 0 ? `, spread-adjusted ${spreadSlotsAdjusted} slots` : ''})`);
      }
    } else {
