  ...
  async _learnFromYesterday(rawData) {
    ...
    try {
      ...
      if (lat != null && lon != null) {
        const knmiHourly = this.learningEngine.data?.knmi_hourly_actuals?.[yDate];
        if (knmiHourly) {
          ...
          for (const [hourStr, ghi] of Object.entries(knmiHourly)) {
            ...
            const d = new Date(`${yDate}T${String(hour).padStart(2, '0')}:30:00Z`);
          ...
          if (ksCount >= 4 && ksClear > 0) {
            knmiKt = ksActual / ksClear;
            this.log(`[KNMI] kt=${knmiKt.toFixed(2)} from ${ksCount} daylight hours for ${yDate}`);
          }
        }
      }
