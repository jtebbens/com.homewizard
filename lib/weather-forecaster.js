...
const { fetchKnmiObservations, fetchKnmiCloudObservations } = require('./knmi-stations');
...
class WeatherForecaster {
  ...
  async _recordKnmiActual(lat, lon) {
    ...
    try {
      ...
      await this._recordOktaActual(apiKey, lat, lon, result, dateStr, hour)
   ...
   * @private
   */
  async _recordOktaActual(apiKey, lat, lon, qgResult, dateStr, hour) {
    const okta = await fetchKnmiCloudObservations(apiKey, lat, lon, {
      knownStationId: qgResult.stationId,
      knownN: qgResult.n,
    });
    if (!okta || okta.oktaFrac == null) return;
