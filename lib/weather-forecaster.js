...
const SAT_YIELD_FACTORS = {
  4: 2.553, 5: 4.815, 6: 5.924, 7: 5.062, 8: 4.204, 9: 3.806,
...
class WeatherForecaster {
  ...
  _mergeApiResponses(ensembleData, standardData, tiltedData, lat, lon, modelWeights = null, tilt = null, azimuth = null) {
    ...
    // TEMP verification log — compares old (single best_match) vs new (ensemble-blended) cloud%
    // for the current hour. Remove once a few live cycles confirm sane values.
    if (ensembleData) {
      const nowIdx = times.findIndex(t => new Date(t + 'Z') > new Date()) - 1;
      if (nowIdx >= 0) {
        this.log(`[CLOUDBLEND] old(best_match)=${standardData.hourly.cloud_cover?.[nowIdx]}% new(ensemble)=${cloud_cover[nowIdx]}%`);
      }
    }
    ...
    if (ensembleData) {
      ...
      const wxNowIdx = times.findIndex(t => new Date(t + 'Z') > new Date()) - 1;
      if (wxNowIdx >= 0) {
        const oldWx = WeatherForecaster._weatherAttenuation(
          standardData.hourly.weather_code?.[wxNowIdx],
          standardData.hourly.precipitation_probability?.[wxNowIdx]);
        this.log(`[WXBLEND] old(best_match)=${oldWx?.toFixed(2)} new(ensemble)=${wxFactorEnsemble[wxNowIdx]?.toFixed(2)}`);
      }
    }
  ...
  async fetchUpwindData(lat, lon, windFromDeg = null) {
    ...
    try {
      ...
      const result = { station, upwindCot, upwindKt, thisCot, dcot, thisStation, thisDd, thisFf };
