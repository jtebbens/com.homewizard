...
// Must not exceed the device's 55min weather-age threshold (battery-policy device.js
// _maybeRefreshWeatherOnly / _gatherInputs). At 60min the device asked for fresh weather
// at ~60min, got this still-valid cache back, and the real fetch slipped a full 15-min
// tick to 75min anyway — while each cached hit still dropped the DP schedule.
...
class WeatherForecaster {
  ...
  async _fetchStandardHourly(lat, lon) {
    const params = new URLSearchParams({
      latitude: lat.toString(),
      longitude: lon.toString(),
      hourly: 'shortwave_radiation,sunshine_duration,temperature_2m,cloud_cover,cloud_cover_low,cloud_cover_mid,cloud_cover_high,cape,convective_inhibition,wind_gusts_10m,freezing_level_height,uv_index,precipitation_probability,precipitation,weather_code,windspeed_10m,winddirection_10m',
   ...
   *
   * ONE implementation for every kt on the app side — used by `_computeKnmiKt` so the 0.30/0.65
   * buckets keep the same meaning regardless of which model computes the denominator.
   *
  ...
  async fetchUpwindData(lat, lon, windFromDeg = null) {
    ...
    try {
      // Round to ~1km grid before it leaves the device (GDPR minimisation, same as the
      // /sat call in device.js) — nearest-station/pixel lookups don't need address-level
      // (4-decimal) precision.
      ...
      if (windFromDeg != null) {
        upLat = Number((lat + (dist / R) * Math.cos(rad(windFromDeg)) * (180 / Math.PI)).toFixed(2));
        upLon = Number((lon + (dist / R) * Math.sin(rad(windFromDeg)) / Math.cos(rad(lat)) * (180 / Math.PI)).toFixed(2));
      }
      const [thisQg, thisPoint, upQg, upPoint] = await Promise.all([
        fetchWithTimeout(`${baseUrl}/qg?lat=${lat.toFixed(2)}&lon=${lon.toFixed(2)}`, _ag, 10000).then(r => r.ok ? r.json() : null),
        fetchWithTimeout(`${baseUrl}/point?lat=${lat.toFixed(2)}&lon=${lon.toFixed(2)}`, _ag, 10000).then(r => r.ok ? r.json() : null),
        upLat != null ? fetchWithTimeout(`${baseUrl}/qg?lat=${upLat.toFixed(2)}&lon=${upLon.toFixed(2)}`, _ag, 10000).then(r => r.ok ? r.json() : null) : Promise.resolve(null),
        upLat != null ? fetchWithTimeout(`${baseUrl}/point?lat=${upLat.toFixed(2)}&lon=${upLon.toFixed(2)}`, _ag, 10000).then(r => r.ok ? r.json() : null) : Promise.resolve(null),
      ]);
