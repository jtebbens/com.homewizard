...
class WeatherForecaster {
  ...
  async fetchUpwindData(lat, lon) {
    ...
    try {
      // Phase 1: home station — need KNMI measured dd before we know upwind coords
      const [thisQg, thisPoint] = await Promise.all([
        fetchWithTimeout(`${baseUrl}/qg?lat=${lat.toFixed(4)}&lon=${lon.toFixed(4)}`, _ag, 5000).then(r => r.ok ? r.json() : null),
        fetchWithTimeout(`${baseUrl}/point?lat=${lat.toFixed(4)}&lon=${lon.toFixed(4)}`, _ag, 5000).then(r => r.ok ? r.json() : null),
      ]);
      ...
      // Phase 2: upwind coords from KNMI measured dd (NAAR → FROM = +180°)
      let station = null, upwindCot = null, dcot = null;
      if (thisDd != null) {
        const fromDeg = (thisDd + 180) % 360;
        const upLat = lat + (dist / R) * Math.cos(rad(fromDeg)) * (180 / Math.PI);
        const upLon = lon + (dist / R) * Math.sin(rad(fromDeg)) / Math.cos(rad(lat)) * (180 / Math.PI);
        const [upQg, upPoint] = await Promise.all([
          fetchWithTimeout(`${baseUrl}/qg?lat=${upLat.toFixed(4)}&lon=${upLon.toFixed(4)}`, _ag, 5000).then(r => r.ok ? r.json() : null),
          fetchWithTimeout(`${baseUrl}/point?lat=${upLat.toFixed(4)}&lon=${upLon.toFixed(4)}`, _ag, 5000).then(r => r.ok ? r.json() : null),
        ]);
