...
class WeatherForecaster {
  ...
  _processForecast(rawData) {
    ...
    const todaySunrise    = parseDailyTime(daily.sunrise?.[0]);
    const todaySunset     = parseDailyTime(daily.sunset?.[0]);
    const tomorrowSunrise = parseDailyTime(daily.sunrise?.[1]);
    const tomorrowSunset  = parseDailyTime(daily.sunset?.[1]);
