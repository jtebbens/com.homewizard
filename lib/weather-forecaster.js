...
const { fetchKnmiObservations } = require('./knmi-stations');

// Single retry on transient errors (TIMEOUT, 5xx). Open-Meteo overloads at peak.
...
class WeatherForecaster {
  ...
  async _fetchEnsembleRadiation(lat, lon) {
    const params = new URLSearchParams({
      ...
      models: 'meteofrance_arpege_europe,gfs_seamless,icon_seamless,knmi_harmonie_arome_netherlands',
      past_days: '1',
      forecast_days: '2',
      timezone: 'UTC'
    });
  ...
  async _fetchStandardHourly(lat, lon) {
    const params = new URLSearchParams({
      latitude: lat.toString(),
      longitude: lon.toString(),
      hourly: 'shortwave_radiation,sunshine_duration,temperature_2m,cloud_cover,cloud_cover_low,precipitation_probability,precipitation,weather_code',
  ...
  _mergeApiResponses(ensembleData, standardData, tiltedData, lat, lon, modelWeights = null, tilt = null, azimuth = null) {
    const ENSEMBLE_MODELS = ['meteofrance_arpege_europe', 'gfs_seamless', 'icon_seamless', 'knmi_harmonie_arome_netherlands'];
    const times = standardData.hourly.time;

    let shortwave_radiation;
    if (ensembleData) {
      // Weighted blend across models; spread discount when models disagree (std > 30 W/m²).
      ...
      shortwave_radiation = times.map((_, i) => {
        ...
        // Spread discount on unweighted std to detect model disagreement
        ...
        if (std > 80) { spreadSlotsAdjusted++; return Math.round(Math.max(0, wMean - 0.5 * std)); }
        if (std > 30) { spreadSlotsAdjusted++; return Math.round(Math.max(0, wMean - 0.3 * std)); }
      ...
      if (sampleAvg.length > 0) {
        const wLog = modelWeights
          ...
          : 'equal';
        this.log(`Ensemble radiation blended from ${ENSEMBLE_MODELS.length} models [${wLog}] (sample avg: ${Math.round(sampleAvg.reduce((a, b) => a + b, 0) / sampleAvg.length)} W/m²${spreadSlotsAdjusted > 0 ? `, spread-adjusted ${spreadSlotsAdjusted} slots` : ''})`);
      }
    } else {
    ...
    // Per-model GHI daily average for today — used by _learnFromYesterday for snapshot/accuracy.
    // Must use GHI (not GTI) so it's comparable to KNMI ground-truth (also GHI).
    // Computed here so ensembleData can be freed immediately after _mergeApiResponses returns.
    const todayDate = new Date().toISOString().slice(0, 10);
    const perModelGhiAvgToday = {};
    if (ensembleData) {
      for (const m of ENSEMBLE_MODELS) {
        let mSum = 0, mCnt = 0;
        for (let i = 0; i < times.length; i++) {
          ...
          if (typeof v === 'number' && v > 10) { mSum += v; mCnt++; }
        }
        if (mCnt > 0) perModelGhiAvgToday[m] = Math.round(mSum / mCnt);
      }
    }
    ...
    return {
      ...
      hourly: {
        ...
        cloud_cover_low: standardData.hourly.cloud_cover_low,
      ...
      perModelGhiAvgToday: Object.keys(perModelGhiAvgToday).length > 0 ? perModelGhiAvgToday : null,
    };
  }
  ...
  _processForecast(rawData) {
    ...
    // Observe-only: ensemble-GTI as alternative radiation source. Operational forecast uses
    // best_match GTI (rawRadiation below); this blends the 4 per-model GTI values by learned
    // accuracy weights so we can compare which source predicts actual PV better. Diagnostic only.
    ...
    for (let i = 0; i < maxHours; i++) {
      ...
      // PV ~2× in afternoon; tilt users were on single best_match model — see [GTI compare]).
      const ensRadiationWm2 = (() => {
        if (!hourly.perModelRadiation) return null;
        let sum = 0, wTot = 0;
        for (const [m, arr] of Object.entries(hourly.perModelRadiation)) {
          const v = arr?.[idx];
          if (typeof v !== 'number') continue;
          const w = _ensWeights?.[m] ?? 0.25;
          sum += w * v; wTot += w;
        }
        return wTot > 0 ? Math.round((sum / wTot) * biasFactor * wxFactor) : null;
      })();
      ...
      hourlyForecast.push({
        ...
        radiationEnsWm2: ensRadiationWm2,
  ...
  async _learnFromYesterday(rawData) {
    ...
    try {
      ...
      await this.learningEngine.recordDailyPvBiasFromPredictions(yDate, avgCloudPct, knmiKt);
      ...
      // Per-model: save today's forecast avg per model for tomorrow's accuracy comparison.
      // Pre-computed in _mergeApiResponses using GHI (not GTI) so it's comparable to KNMI ground-truth.
      const todayPerModel = rawData.perModelGhiAvgToday ?? null;
      if (todayCount > 0) {
        const pmLog = todayPerModel ? Object.entries(todayPerModel).map(([m,v]) => `${m.replace('meteofrance_arpege_europe','mf').replace('_seamless','').replace('knmi_harmonie_arome_netherlands','knmi')}=${v}`).join(' ') : 'null';
        this.log(`[Snapshot] ${todayDate} rad=${Math.round(todaySum/todayCount)} perModel=${pmLog} hourlyLen=${hourly.time.length}`);
        await this.learningEngine.saveForecastSnapshot(todayDate, todaySum / todayCount, todayPerModel);
      }

      // Per-model accuracy: compare yesterday's per-model snapshot vs actual radiation.
      const ySnapshot = this.learningEngine.getForecastSnapshot(yDate);
      this.log(`[ModelAccuracy check] ${yDate}: perModel=${ySnapshot?.perModel ? Object.keys(ySnapshot.perModel).join(',') : 'null'} actualCount=${actualCount}`);
      if (ySnapshot?.perModel && actualCount > 0) {
        await this.learningEngine.recordModelAccuracy(ySnapshot.perModel, actualAvg, yDate);
      }
  ...
  async _recordKnmiActual(lat, lon) {
    ...
    try {
      ...
      // Update today's partial kt for use by getDailyPvBiasFactor
      const todayKt = this._computeKnmiKt(dateStr, lat, lon);
      if (todayKt != null) {
        this._todayKnmiKt = todayKt;
        this.log(`[KNMI] today kt=${todayKt.toFixed(2)} (partial)`);
        // Observe-only: compare Cabauw clearness vs local-PV-derived clearness to
        // surface spatial decorrelation (station 14km away may see different clouds).
        // No policy effect — diagnostic log line only.
        const local = this._computeLocalPvKt(dateStr, lat, lon);
        if (local && local.kt != null) {
          const delta = local.kt - todayKt;
          this.log(`[KNMI] decorrelation: Cabauw kt=${todayKt.toFixed(2)} PV-kt=${local.kt.toFixed(2)} Δ=${delta >= 0 ? '+' : ''}${delta.toFixed(2)} (PV n=${local.n} clip-skip=${local.clipped})`);
        }
      }
    } catch (err) {
   ...
   * @private
   */
  _computeKnmiKt(dateStr, lat, lon) {
    ...
    for (const [hourStr, ghi] of Object.entries(knmiHourly)) {
      ...
      if (elev < 5) continue;
      const ghiClear = 1000 * Math.sin(elev * Math.PI / 180);
  ...
  /**
   * Observe-only local clearness index from PV output: kt_pv = Σ actual_PV / Σ clear-sky_PV,
   * where clear-sky_PV ≈ pvCapacityW × sin(elev). Mirrors _computeKnmiKt but uses the panels
   * as a (confounded) local irradiance proxy. Hours where PV sits at/near nameplate are skipped
   * because inverter clipping (Steca curtails) caps output and would understate true clearness.
   * Returns { kt, n, clipped } or null. Diagnostic only — never feeds policy/learning.
   * @private
   */
  _computeLocalPvKt(dateStr, lat, lon) {
    const cap = this.pvCapacityW || 0;
    if (cap <= 0) return null;
    const preds = this.learningEngine?.data?.pv_predictions;
    if (!Array.isArray(preds)) return null;

    // Group max actual PV per UTC hour for the target date.
    const hourMax = {};
    for (const p of preds) {
      if (!p || typeof p.actual !== 'number') continue;
      const d = new Date(p.timestamp);
      if (d.toISOString().slice(0, 10) !== dateStr) continue;
      const h = d.getUTCHours();
      if (hourMax[h] == null || p.actual > hourMax[h]) hourMax[h] = p.actual;
    }

    const clipW = 0.92 * cap; // at/above this, output is likely clipped → skip
    let sumActual = 0, sumClear = 0, n = 0, clipped = 0;
    for (const [hourStr, actualW] of Object.entries(hourMax)) {
      const hour = parseInt(hourStr);
      const dMid = new Date(`${dateStr}T${String(hour).padStart(2, '0')}:30:00Z`);
      const { elev } = WeatherForecaster._solarElevAz(dMid, lat, lon);
      if (elev < 5) continue;
      const clearPV = cap * Math.sin(elev * Math.PI / 180);
      if (clearPV <= 50) continue;
      if (actualW >= clipW) { clipped++; continue; }
      sumActual += actualW;
      sumClear  += clearPV;
      n++;
    }
    return n >= 4 && sumClear > 0 ? { kt: sumActual / sumClear, n, clipped } : { kt: null, n, clipped };
  }

  /** Returns today's KNMI clearness index (partial day, updated each KNMI fetch). */
  getTodayKt() { return this._todayKnmiKt ?? null; }
