...
class WeatherForecaster {
  ...
  _applySatelliteOverlay(satData) {
    ...
    for (const slot of slots) {
      ...
      if (lead < -3600_000 || lead >= maxLeadMs) continue;
      ...
      const wxFactor = WeatherForecaster._weatherAttenuation(slot.weatherCode ?? 0, slot.precipProb ?? 0);
      const omRadiation = slot.radiationWm2;
      slot.satGhiWm2 = satGhi;
      slot.radiationWm2 = Math.round(satGhi * biasFactor * wxFactor);
      slot.satRadWm2 = slot.radiationWm2;
      slot.radiationSpreadFrac = 0.05;
      this.log(`[SAT] h=${slot.time.getUTCHours()} sat=${satGhi} om=${omRadiation} → ${slot.radiationWm2} spread→0.05`);
    }
  }
