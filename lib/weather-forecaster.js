...
const { fetchKnmiRadiation } = require('./knmi-stations');
...
class WeatherForecaster {
  ...
  async _doFetch(latitude, longitude, tilt, azimuth) {
    try {
      ...
      this._lastEnsembleData = ensembleData;
      const modelWeights = this.learningEngine?.getModelWeights?.() ?? null;
      const rawData = this._mergeApiResponses(ensembleData, standardData, tiltedData, lat, lon, modelWeights);
   ...
   * @private
   */
  _mergeApiResponses(ensembleData, standardData, tiltedData, lat, lon, modelWeights = null) {
    ...
    // Per-model shortwave_radiation arrays, aligned to `times` (standardData indices).
    // Used in device.js to build per-model pvForecast without a separate time-map lookup.
    const perModelRadiation = {};
    if (ensembleData) {
      const ENSEMBLE_MODELS = ['meteofrance_arpege_europe', 'gfs_seamless', 'icon_seamless', 'knmi_harmonie_arome_netherlands'];
      for (const m of ENSEMBLE_MODELS) {
        perModelRadiation[m] = times.map((_, i) => {
  ...
  async _learnFromYesterday(rawData) {
    ...
    try {
      ...
      await this.learningEngine.recordDailyPvBiasFromPredictions(yDate, avgCloudPct);
      ...
      let todayPerModel = null;
      const ensData = this._lastEnsembleData;
      if (ensData) {
        ...
        for (const m of MODELS) {
          let mSum = 0, mCnt = 0;
          for (let i = 0; i < hourly.time.length; i++) {
            if (!hourly.time[i].startsWith(todayDate)) continue;
            const v = ensData.hourly[`shortwave_radiation_${m}`]?.[i];
            if (typeof v === 'number' && v > 10) { mSum += v; mCnt++; }
          }
          if (mCnt > 0) todayPerModel[m] = Math.round(mSum / mCnt);
        }
      }
      if (todayCount > 0) {
        ...
        this.log(`[Snapshot] ${todayDate} rad=${Math.round(todaySum/todayCount)} perModel=${pmLog} ensLen=${ensData?.hourly?.time?.length ?? 'null'} hourlyLen=${hourly.time.length}`);
  ...
  async _recordKnmiActual(lat, lon) {
    ...
    try {
      const result = await fetchKnmiRadiation(apiKey, lat, lon);
      if (!result || result.qg == null) return;
      const now = new Date();
      const dateStr = now.toISOString().slice(0, 10);
      const hour = now.getUTCHours();
      this.learningEngine.recordKnmiHourlyActual(result.qg, dateStr, hour);
      this.log(`[KNMI] ${result.stationName} (${result.distKm}km): qg=${result.qg} W/m²`);
    } catch (err) {
