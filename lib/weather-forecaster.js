...
class WeatherForecaster {
  ...
  async fetchForecast(latitude, longitude, tilt = null, azimuth = null) {
    ...
    try {
      ...
      this.cache = this._processForecast(rawData);
  ...
  /**
   * Fetch shortwave_radiation from 3-model ensemble (ECMWF, GFS, ICON) in parallel.
  ...
  _mergeApiResponses(ensembleData, standardData, tiltedData, lat, lon) {
    const ENSEMBLE_MODELS = ['ecmwf_ifs04', 'gfs_seamless', 'icon_seamless'];
  ...
  _processForecast(rawData) {
    ...
    for (let i = 0; i < maxHours; i++) {
      ...
      const radiationWm2 = Math.round(rawRadiation * biasFactor * fogFactor); // Open-Meteo GHI already cloud-corrected

      hourlyForecast.push({
        time: new Date(`${hourly.time[idx]}Z`), // Append Z — Open-Meteo returns UTC ISO without tz offset
        sunshine: (rawSunshineSec * cloudFactor * fogFactor) / 3600, // seconds → hours, cloud+fog-corrected
        cloudCover,
    ...
    for (let i = 0; i < hourly.time.length; i++) {
      ...
      dailyProfiles.push({
        time: new Date(`${t}Z`),
        cloudCover,
        sunshine: (rawSunshineSec * cloudFactor * fogF) / 3600,
  ...
  invalidateCache() {
    this.cache = null;
    this.cacheExpiry = null;
    this.homey.settings.unset('weather_forecast_cache').catch(() => {});
  ...
  _loadCache() {
    try {
      ...
      // Revive Date objects (JSON serialization turns them into ISO strings)
      const c = stored.cache;
      ...
      return { cache: c, expiry: stored.expiry };
    } catch (e) {
