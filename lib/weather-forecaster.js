...
class WeatherForecaster {
  ...
  _buildApiUrl(latitude, longitude, tilt = null, azimuth = null) {
    ...
    const params = new URLSearchParams({
      ...
      hourly: `sunshine_duration,temperature_2m,cloud_cover,precipitation_probability,${radiationVar}`,
  ...
  _processForecast(rawData) {
    ...
    for (let i = 0; i < maxHours; i++) {
      ...
      const radiationWm2 = Math.round(rawRadiation * biasFactor); // Open-Meteo GHI already cloud-corrected
    ...
    for (let i = 0; i < hourly.time.length; i++) {
      ...
      dailyProfiles.push({
        time: new Date(`${t}Z`),
        cloudCover,
        sunshine: (rawSunshineSec * cloudFactor) / 3600,
        radiationWm2: Math.round(rawRadiation * biasFactor)
      });
    }
