...
class XadiProvider {
  ...
  async fetchPrices(force = false) {
    ...
    try {
      // 1. Fetch TODAY's prices (from midnight to midnight)
      const todayUrl = `https://dap.xadi.eu/api/nl/today?markup=${markup}&vat=${vat}`;
      this.log(`Fetching Xadi /today: ${todayUrl}`);
      ...
      // 2. Fetch NEXT24H (from current hour onwards)
      const next24hUrl = `https://dap.xadi.eu/api/nl/next24h?markup=${markup}&vat=${vat}`;
      this.log(`Fetching Xadi /next24h: ${next24hUrl}`);
      ...
      try {
        ...
        if (next24hResponse.ok) {
          ...
          if (next24hData.status === 'success' && next24hData.data && next24hData.data.length > 0) {
            ...
            this.log(`✅ Fetched ${next24hPrices.length} prices from /next24h (${next24hPrices.length - todayPrices.filter(tp => next24hPrices.some(np => np.timestamp.getTime() === tp.timestamp.getTime())).length} new)`);
          }
        }
      } catch (err) {
  ...
  getTop3Cheapest() {
    ...
    // Use all available hours to show the day's actual price pattern
    return [...this.cache]
