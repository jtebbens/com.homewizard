...
const { exportPrice } = require('./price-formulas');

// Built once — constructing an Intl.DateTimeFormat per call is expensive.
// h23 keeps midnight as "00", not "24".
const AMS_HOUR_FMT = new Intl.DateTimeFormat('en-GB', {
...
class XadiProvider {
  ...
  /** Amsterdam local hour of an instant. @param {Date} d - injectable clock, for tests */
  _amsHour(d = new Date()) {
    return Number(AMS_HOUR_FMT.format(d));
  }
   ...
   */
  _mapItem(item) {
    // Server applies markup + VAT — use price directly
    ...
    return {
      ...
      price,
      exportPrice: exportPrice(spot, this.exportAddon, this.exportMultiplier),
  ...
  _separateIntervals(allPrices) {
    ...
    const hourlyPrices = Object.entries(hourlyBuckets).map(([key, bucket]) => {
      ...
      const spot = (avgPrice / 1.21) - this.markup; // Reverse: (spot + markup) × 1.21
  ...
  /**
   * Fetch prices from /today and /next24h.
   *
   * /day/{date} and /tomorrow are NOT available on this API — both return 404 for every
   * date (verified 2026-08-01). /next24h is therefore the only forward source, and it is a
   * ROLLING window: Xadi alone can never complete tomorrow. The tail gap is filled by
   * MergedPriceProvider's ENTSOE tomorrow-fill.
   *
  ...
  async fetchPrices(force = false) {
    const now = Date.now();
    // The app process on the hub runs in UTC — verified 2026-08-02 against device.js's
    // hour-boundary log line, which printed the same hour as its UTC timestamp on 8/8
    // consecutive boundaries. getHours() therefore yielded UTC, not CET as an earlier
    // comment here claimed, and this window fired at 17:00-18:00 Amsterdam instead of 15:00.
    const hour = this._amsHour();
    ...
    // Server applies both markup and VAT — use item.price directly, no client-side calculation
    const baseParams = `markup=${this.markup}&vat=0.21`;
    const allPrices = [];
    const seenTimestamps = new Set();

    try {
      const endpoints = [
        { label: '/today',   url: `https://dap.xadi.eu/api/nl/today?${baseParams}` },
        { label: '/next24h', url: `https://dap.xadi.eu/api/nl/next24h?${baseParams}` },
      ];

      // Parallel fetch — each endpoint independent, dedup via seenTimestamps below.
      // Worst case ~10s total instead of 30s sequential.
      const results = await Promise.allSettled(
        endpoints.map(e => fetchWithTimeout(e.url, {}, 10000)
          .then(async res => ({ endpoint: e, res, data: res.ok ? await res.json() : null })))
      );

      for (let i = 0; i < results.length; i++) {
        const r = results[i];
        const e = endpoints[i];
        if (r.status === 'rejected') { this.log(`Failed to fetch ${e.label}: ${r.reason.message}`); continue; }
        const { res, data } = r.value;
        if (!res.ok) { this.log(`${e.label} HTTP ${res.status}`); continue; }
        if (!data || data.status !== 'success' || !data.data || data.data.length === 0) continue;
        ...
        this.log(`✅ Fetched ${prices.length} intervals from ${e.label} (${newCount} new)`);
      }
