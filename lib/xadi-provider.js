...
class XadiProvider {
  ...
  getCurrentRate() {
    ...
    const now = new Date();
    const currentHour = now.getHours();   // LOCAL TIME (correct!)

    let currentPrice = this.cache.find(p => p.hour === currentHour);

    if (!currentPrice) {
      const first = this.cache[0];
      this.log(
        `getCurrentRate: no exact match for hour ${currentHour}, using first entry (${first.hour})`
      );
      currentPrice = first;
    }
  ...
  getCurrentPrice() {
    ...
    if (!currentPrice) {
      const first = this.cache[0];
      this.log(
        `getCurrentPrice: no exact match for hour ${currentHour}, using first entry (${first.hour})`
      );
      currentPrice = first;
    }
  ...
  getCheapestHours(count = 3, lookAhead = 24) {
    ...
    for (let i = 0; i < lookAhead; i++) {
      ...
      const priceData = this.cache.find(p => p.hour === futureHour);
      if (priceData) {
  ...
  getTop3Cheapest() {
    ...
    return [...this.cache].sort((a, b) => a.price - b.price).slice(0, 3).map(p => p.price);
  }

  getTop3MostExpensive() {
    if (!this.cache) return [];
    return [...this.cache].sort((a, b) => b.price - a.price).slice(0, 3).map(p => p.price);
  }
}
