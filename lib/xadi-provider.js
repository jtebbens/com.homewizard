...
class XadiProvider {
   ...
   */
  _mapItem(item) {
    // Server applies markup; multiply by 1.21 for VAT — matches KwhPrice formula exactly
    const price = item.price * 1.21;
  ...
  async fetchPrices(force = false) {
    ...
    // Let Xadi apply markup server-side; VAT (21%) applied client-side to match KwhPrice formula
