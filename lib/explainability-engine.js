...
class ExplainabilityEngine {
  ...
  _generateDpSummary(recommendation, inputs) {
    ...
    // preserve
    if (dp.pvExportWins) {
      // Mapper overrides standby→to_full when SoC is critically low and price is
      // cheap (lowSocGridTopUp in policy-engine.js _mapPolicyToHwMode). Reflect
      // that override so the summary doesn't say "bewaren" while grid-charging.
      if (recommendation.hwMode === 'to_full') {
        return `Advies: opladen. Reden: SoC ${soc}% is laag${priceStr ? ` en prijs ${priceStr} is gunstig` : ''} — batterij wordt vanaf net bijgeladen, ondanks dat PV-export normaal rendabeler is.`;
      }
