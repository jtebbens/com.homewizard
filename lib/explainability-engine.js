...
const WeatherForecaster = require('./weather-forecaster');
const { exportValue, disposalValue, storeValue, effectiveRte } = require('./price-formulas');
const { fullCurtailSlot } = require('./curtailment');

// HW modes in which the battery takes nothing in, so a "charge" plan did not survive the mapper.
const NON_CHARGING_HW_MODES = new Set(['standby', 'zero_discharge_only', 'zero']);
...
class ExplainabilityEngine {
  ...
  generateExplanation(recommendation, inputs, scores) {
    ...
    this._addUpwindReasons(reasons, inputs);
    ...
    const MODE_SUPPORTED = {
      zero_charge_only: 'charge', pv_trickle: 'charge', to_full: 'charge',
  ...
  _generateDpSummary(recommendation, inputs) {
    ...
    // "verwacht" when the peak sits on an estimated slot (price_forecast_fill): the explanation
    // may not present a price the day-ahead auction has not published as a known one.
    const futStr      = dp.maxFuturePrice != null
      ? `€${dp.maxFuturePrice.toFixed(3)}${dp.maxFuturePriceEstimated ? ' (schatting)' : ''}` : null;
    // Export figure shown to the user: retail under saldering, lower per-slot export
    // price under asymmetric_2027, so the displayed number matches what export actually credits.
    // Deliberately NOT disposalValue(): this prints what the grid pays, not the value of the
    // best disposal route. Curtailment runs in the user's own flow, so the app cannot claim the
    // surplus was thrown away rather than exported. Decision sites use disposalValue().
    // Mirrored at _buildDpReasons below.
    const _exportV    = price == null ? null : exportValue(
      { price, exportPrice: inputs.tariff?.currentExportPrice },
      inputs.settings?.tariff_model ?? 'saldering',
      inputs.settings?.export_price_ratio ?? 1.0,
    );
    const exportPriceStr = _exportV != null ? `€${_exportV.toFixed(3)}` : null;
    ...
    if (exception === 'delay_charge') {
      return `Advies: standby. Reden: PV exporteert nu naar net${exportPriceStr ? ` (${exportPriceStr})` : ''} — laden uitgesteld naar goedkoper moment.`;
    }
    ...
    if (exception === 'min_price_override') {
      const minVal = inputs.settings?.min_discharge_price;
      const minStr = minVal != null ? `€${Number(minVal).toFixed(3)}` : null;
      return `Advies: ontladen. Reden: DP wilde bewaren, maar prijs${priceStr ? ` ${priceStr}` : ''} ligt boven je ingestelde minimum${minStr ? ` ${minStr}` : ''} en geen PV dit blok — ontladen boven de drempel.`;
    }
    ...
    if (action === 'discharge') {
      ...
      // Reported, not claimed as cleared: break-even is the sunk cost price of the stored
      // energy and no longer gates discharging (the DP's floor does), so "boven break-even"
      // would be false whenever the battery was filled at a higher price than it sells at.
      if (beStr)    parts.push(`kostprijs ${beStr}`);
      return `Advies: ontladen. Reden: ${parts.join(', ')}.`;
    }
    if (action === 'charge') {
      if (price != null && price < 0) {
        return `Advies: opladen. Reden: negatieve prijs ${priceStr} — laden levert geld op, terugleveren kost geld.`;
      }
      // The mapper can still veto a DP charge that the DP itself let through: its export test
      // runs on live PV, the DP's on forecast coverage. Reporting "Advies: opladen" next to
      // mode 'standby' is what the 2026-08-20 complaint opened with — say what the battery
      // does, not what the DP asked for.
      if (NON_CHARGING_HW_MODES.has(recommendation.hwMode)) {
        return `Advies: niets doen. Reden: DP wilde opladen${priceStr ? ` op ${priceStr}` : ''}, `
             + `maar terugleveren levert nu meer op dan opslaan — PV gaat naar het net.`;
      }
      let parts = [`DP gepland: opladen`];
      if (priceStr) parts.push(`goedkope stroom ${priceStr}`);
      const upwind = inputs.upwindData;
      const upMod  = WeatherForecaster.getUpwindModulation(upwind);
      if (upMod.active) {
        parts.push(`wolken in aantocht over ~${upMod.leadMin} min (upwind kt=${upMod.upwindKt.toFixed(2)})`);
      }
      return `Advies: opladen. Reden: ${parts.join(', ')}.`;
    }
    // preserve
    // Mapper overrides preserve/standby→to_full when SoC is critically low and price
    // is cheap (lowSocGridTopUp in policy-engine.js _mapPolicyToHwMode). Reflect that
    // override so the summary doesn't say "bewaren" while grid-charging.
    if (recommendation.hwMode === 'to_full') {
      return `Advies: opladen. Reden: SoC ${soc}% is laag${priceStr ? ` en prijs ${priceStr} is gunstig` : ''} — batterij wordt vanaf net bijgeladen${dp.pvExportWins ? ', ondanks dat PV-export normaal rendabeler is' : ''}.`;
    }
    if (dp.pvExportWins) {
      const exportStr = exportPriceStr ? ` (${exportPriceStr}/kWh)` : '';
      // pvExportWins means the DP decided NOT to store — so the battery is explicitly not
      // being filled. Saying otherwise contradicts the decision this line is explaining, and
      // reads as a fault when the user sees SoC sitting at 0% all day (reported 2026-07-17).
      return `Advies: niets doen. Reden: PV-export${exportStr} rendeert meer dan opslaan — overschot gaat naar het net.`;
    }
    let reason = 'DP gepland: bewaren';
    if (futStr && priceStr && dp.maxFuturePrice > (price ?? 0) * 1.05) {
      if (soc <= minSoc + 5) {
        reason = `Wacht op goedkopere laadkans — hoge prijs verwacht ${futStr} vs nu ${priceStr}`;
      } else {
  ...
  _generateDpShortSummary(recommendation, inputs) {
    ...
    const socPart = typeof soc === 'number' ? `${soc}%` : null;

    const exc = dp.exception;
    let reasonPart = null;
    if (exc === 'no_optimizer')       reasonPart = 'geen planning';
    else if (exc === 'soc_too_low')   reasonPart = 'SoC te laag';
    else if (exc === 'soc_full')      reasonPart = 'vol';
    else if (exc === 'delay_charge')  reasonPart = 'PV→net';
    else if (exc === 'peak_shaving')  reasonPart = 'piek';
    // The mapper can now bank surplus on a slot the DP planned for export (store-vs-export
    // re-tested on the live price). Saying 'export>opslaan' while the battery charges would
    // put the explanation at odds with the decision.
    else if (dp.pvExportWins)         reasonPart = (hw === 'to_full') ? 'SoC laag, net-laden'
                                                 : (hw === 'zero_charge_only' || hw === 'pv_trickle') ? 'opslaan>export'
                                                 : 'export>opslaan';
    else if (dp.finalAction === 'preserve' && hw === 'to_full') reasonPart = 'SoC laag, net-laden';
    else if (dp.finalAction === 'preserve' && soc <= 10 && hw !== 'zero_charge_only' && hw !== 'to_full' && hw !== 'pv_trickle') {
      reasonPart = 'laden uitgesteld';
    } else if (dp.finalAction === 'preserve' && dp.maxFuturePrice && price && hw !== 'zero_charge_only' && hw !== 'to_full' && hw !== 'pv_trickle') {
      reasonPart = `wacht op €${dp.maxFuturePrice.toFixed(3)}`;
    } else if (dp.finalAction === 'discharge' && breakEven != null && price != null) {
      const margin = price - breakEven;
      reasonPart = `${margin >= 0 ? '+' : ''}€${margin.toFixed(3)}/kWh`;
    }

    return [label, socPart, reasonPart].filter(Boolean).join(' - ');
  }
  ...
  _buildDpReasons(recommendation, inputs) {
    ...
    const priceStr  = price     != null ? `€${price.toFixed(3)}`     : null;
    const beStr     = breakEven != null ? `€${breakEven.toFixed(3)}` : null;
    const futStr    = dp.maxFuturePrice != null                       // '(schatting)': see line 139
      ? `€${dp.maxFuturePrice.toFixed(3)}${dp.maxFuturePriceEstimated ? ' (schatting)' : ''}` : null;
    // Display figure, not a decision — unfloored on purpose, see _generateDpSummary above.
    const _exportV  = price == null ? null : exportValue(
      { price, exportPrice: inputs.tariff?.currentExportPrice },
      inputs.settings?.tariff_model ?? 'saldering',
      inputs.settings?.export_price_ratio ?? 1.0,
    ...
    if (exception === 'no_optimizer') {
      ...
      reasons.push({ impact: 'high', category: 'pv', supportedMode: 'preserve', icon: '⏸️', text: `PV exporteert nu naar net${exportPriceStr ? ` (${exportPriceStr})` : ''} — laden uitgesteld.` });
      ...
      if (beStr)    text += `, kostprijs ${beStr}`;
      reasons.push({ impact: 'critical', category: 'strategy', supportedMode: 'discharge', icon: '🧠', text });
    } else if (action === 'charge') {
      const text = (price != null && price < 0)
        ? `DP gepland: negatieve prijs ${priceStr} — laden levert geld op, terugleveren kost geld`
        : `DP gepland: opladen${priceStr ? `, goedkope stroom ${priceStr}` : ''}`;
      reasons.push({ impact: 'critical', category: 'strategy', supportedMode: 'charge', icon: '🧠', text });
    } else if (inputs.refillReserveActive && price != null && soc > minSoc
               && price >= (inputs.effectiveMinDischarge ?? inputs.settings?.min_discharge_price ?? 0.22)) {
      // Battery would normally discharge now (price ≥ min-discharge) but the overnight
      // refill-reserve floor holds charge back because tomorrow's PV is uncertain.
      const confPct = Math.round((inputs.refillConfidence ?? 1) * 100);
      reasons.push({ impact: 'high', category: 'pv', supportedMode: 'preserve', icon: '🛡️',
        text: `Reserve aangehouden voor morgenochtend — PV‑voorspelling onzeker (vertrouwen ${confPct}%). Batterij wordt niet leeggetrokken zodat de ochtend‑ en avondpiek gedekt blijven als de zon tegenvalt.` });
    } else if (dp.pvExportWins && recommendation.hwMode === 'to_full') {
      reasons.push({ impact: 'high', category: 'battery', supportedMode: 'charge', icon: '🔋',
        text: `SoC ${soc}% is laag${priceStr ? ` en prijs ${priceStr} is gunstig` : ''} — batterij wordt vanaf net bijgeladen, ondanks dat PV-export normaal rendabeler is.` });
    } else {
      let text = `DP gepland: bewaren`;
      if (futStr && priceStr && dp.maxFuturePrice > (price ?? 0) * 1.05) {
        if (soc <= minSoc + 5) {
          text = `DP gepland: wacht op goedkopere laadkans — hoge prijs verwacht ${futStr} vs nu ${priceStr}`;
        } else {
  ...
  // Shared by _addBatteryReasons (soc=0) and _addPVReasons (PV overschot): the
  // best future discharge price and what storing-now would be worth against it.
  _maxFutureStoreValue(inputs, eff) {
    const pricesArr = inputs.tariff?.allPrices || inputs.tariff?.next24Hours || [];
    const now = new Date();
    const futurePrices = pricesArr
      .filter(h => h.timestamp ? new Date(h.timestamp) > now : (h.index ?? 0) >= 1)
      .map(h => h.price).filter(p => typeof p === 'number' && p > 0);
    const maxFuture = futurePrices.length ? Math.max(...futurePrices) : null;
    const cycleCost = inputs.settings?.cycle_cost_per_kwh ?? 0.075;
    return { maxFuture, cycleCost, storeValue: storeValue(maxFuture, eff, cycleCost) };
  }
  ...
  _addBatteryReasons(reasons, warnings, battery, settings, tariffType, inputs) {
    ...
    if (soc === 0) {
      ...
      // Same measured round-trip efficiency the policy gate and the DP use — set on inputs by
      // calculatePolicy(); recomputed here only when the explanation runs without it.
      const _eff   = inputs.effectiveRte ?? effectiveRte(inputs.batteryEfficiency, settings.battery_efficiency);
      const { maxFuture: _maxFuture, cycleCost: _cycleCost, storeValue: _storeValue } = this._maxFutureStoreValue(inputs, _eff);
      ...
      if (_exportWins) {
        reasons.push({
          icon: '⚡',
          category: 'battery',
          text: `Batterij op 0% — laden nu niet rendabel (laadprijs €${_price.toFixed(3)} > verwachte ontlaadwaarde €${_maxFuture.toFixed(3)} × ${_eff.toFixed(2)} − cycluskosten €${_cycleCost.toFixed(3)} = €${_storeValue.toFixed(3)}). Standby aanbevolen.`,
          impact: 'critical',
          sentiment: 'positive',
    ...
    if (soc > 10 && soc <= 30) {
      // _pvDeferToCheaperPv: storing beats exporting here, but the DP holds out for a pvStrong
      // slot before the same peak where the forgone export is lower, so nothing is charging yet.
      // Claiming "wordt geladen" there contradicts the mode the mapper actually returns.
      if (inputs._pvStoreWins && !inputs._pvDeferToCheaperPv) {
  ...
  _addTariffReasons(reasons, tariff, settings, tariffType, inputs) {
    ...
    const price = inputs.effectivePrice;
    // Static cap (not the dynamic peak ceiling): this block reasons about profitability against
    // the min_discharge FLOOR (price × 1.25 vs minDischargePrice). That claim only holds at low
    // charge prices; widening to the peak-arbitrage ceiling would falsely call €0.18 "winstgevend
    // bij ontladen op €0.25". Peak-arbitrage charging is explained by the sun-forecast + RTE-loss
    // reasons instead.
    ...
    if (price > breakEven + 0.01) {
      if (effectiveDischargeFloor > 0 && price < effectiveDischargeFloor) {
        const blockReason = respectMinMax
          ...
          : `onder opportunistische drempel (${fmt(effectiveDischargeFloor)})`;
        reasons.push({
          icon: '⚖️',
          category: 'arbitrage',
          impact: 'medium',
  ...
  _addPVReasons(reasons, inputs) {
    ...
    // Negative import price: the inverter is throttled to 0W and everything — house
    // load and battery charge — is drawn from the grid, which pays per kWh. There is
    // no surplus to weigh against export here, so this case comes before the
    // PV-overschot branch below (the meter reads import, not export, on these slots).
    if (inputs.settings?.pv_curtailment_enabled === true
        && fullCurtailSlot({ price: inputs.effectivePrice ?? null })) {
      reasons.push({
        icon: '🚫',
        category: 'pv',
        text: `Importprijs €${inputs.effectivePrice.toFixed(3)}/kWh is negatief — omvormer naar 0W: elke kWh van het net levert geld op, dus eigen PV kost die vergoeding. Huis én accu laden vol van het net.`,
        impact: 'high',
        sentiment: 'positive',
        supportedMode: 'charge'
      });
    ...
    if (gridPower < -150) {
      ...
      const _price = inputs.effectivePrice ?? null;
      // Same measured round-trip efficiency the policy gate and the DP use (see line ~424).
      const _eff   = inputs.effectiveRte ?? effectiveRte(inputs.batteryEfficiency, inputs.settings?.battery_efficiency);
      // Value of NOT storing this surplus — retail under saldering, lower per-slot
      // exportPrice under asymmetric_2027, floored at 0 when the inverter can be throttled
      // (free disposal) — so the reason matches the DP/runtime store-vs-dispose verdict.
      const _exportVal = _price === null ? null : disposalValue(
        { price: _price, exportPrice: inputs.tariff?.currentExportPrice },
        inputs.settings?.tariff_model ?? 'saldering',
        inputs.settings?.export_price_ratio ?? 1.0,
        inputs.settings?.pv_curtailment_enabled === true,
      );
      const { maxFuture: _maxFuture, cycleCost: _cycleCost, storeValue: _rawStoreValue } = this._maxFutureStoreValue(inputs, _eff);
      // One implementation per formula: the policy gate has already decided what this surplus is
      // worth stored (_pvStoreValue — trickle cap honoured or dropped, see _computePvFlags).
      // Recomputing an uncapped value here explains a different number than the one the decision
      // ran on. Fall back to the local computation only on paths that never reach _computePvFlags.
      const _storeValue = typeof inputs._pvStoreValue === 'number' ? inputs._pvStoreValue : _rawStoreValue;
      // The `maxFuture × eff − cycle` breakdown only describes the uncapped value; where the gate
      // ran on a capped one, show the value alone instead of a sum that does not add up.
      const _storeStr = (_maxFuture !== null && _storeValue !== null
        && Math.abs(_storeValue - _rawStoreValue) < 1e-9)
        ? `€${_maxFuture.toFixed(3)} × ${_eff.toFixed(2)} − cycluskosten €${_cycleCost.toFixed(3)} = €${_storeValue.toFixed(3)}`
        : `opslagwaarde €${_storeValue !== null ? _storeValue.toFixed(3) : '?'}`;
      const _exportMargin = (_exportVal !== null && _storeValue !== null) ? _exportVal - _storeValue : null;
      const _soc = inputs.battery?.stateOfCharge ?? 50;
      const _lowSocMarginRequired = _soc < 50 ? 0.02 : 0;
      // With dp_weak_pv_in_dp on and a weak surplus, the backward pass owns this slot's
      // store-vs-export choice and the mapper follows it (policy-engine _computePvFlags). Deriving
      // a verdict from the margins here would let the explanation contradict the plan — the exact
      // divergence the three-file rule exists to stop. Flag off → field absent → unchanged.
      const _dpOwnsPv = inputs._pvWeakOwnedByDp === true;
      // Mirror policy-engine: export only wins if margin exceeds low-SoC threshold
      const _exportWins = _dpOwnsPv
        ? inputs._pvStoreWins === false
        : (_exportMargin !== null && _exportMargin > 0 && _exportMargin >= _lowSocMarginRequired);

      if (_exportWins) {
        const delayNote = inputs._delayCharge ? `, batterij laadt later bij goedkopere prijzen` : '';
        const dpNote = _dpOwnsPv ? ' [DP-besluit]' : '';
        reasons.push({
          icon: '⚡',
          category: 'pv',
          text: `PV-overschot (${exportPower}W export) — export naar net winstgevender (€${_exportVal.toFixed(3)} > ${_storeStr})${delayNote}${dpNote}.`,
          impact: 'high',
          sentiment: 'positive',
          supportedMode: 'preserve'
        });
      } else if (!_dpOwnsPv && _exportMargin !== null && _exportMargin > 0 && _exportMargin < _lowSocMarginRequired) {
        ...
        reasons.push({
          icon: '☀️',
          category: 'pv',
          text: `PV-overschot gedetecteerd (${exportPower}W export) — opslaan winstgevender dan exporteren nu${_dpOwnsPv ? ' [DP-besluit]' : ''}.`,
          impact: 'high',
          sentiment: 'positive',
          supportedMode: 'charge'
        });
  ...
  _addUpwindReasons(reasons, inputs) {
    const upwind = inputs.upwindData;
    const upMod  = WeatherForecaster.getUpwindModulation(upwind);
    if (!upMod.active) return;
    const stationStr = upwind.station ? ` bij ${upwind.station}` : '';
    const ktPct = Math.round(upMod.upwindKt * 100);
    reasons.push({
      icon: '🌥️',
      category: 'pv',
      impact: 'high',
      sentiment: 'negative',
      supportedMode: 'charge',
      text: `Wolken in aantocht${stationStr} (transmissie ${ktPct}%) — PV-opbrengst verwacht lager over ~${upMod.leadMin} min. DP laadt eerder om goedkope stroom veilig te stellen.`,
    });
  }
  ...
  _addSunForecastReasons(reasons, inputs) {
    ...
    const maxChargePrice = inputs.dynamicMaxChargePrice ?? (inputs.settings?.max_charge_price || 0);
  ...
  _buildMappingReason(normalized, inputs, scores) {
    ...
    // Discharge planned, but live PV surplus + headroom + storing beats exporting:
    // bank the surplus toward the upcoming higher-priced slot instead of exporting now.
    // Charge-only (zero_charge_only) → battery never discharges here, no RTE churn.
    if (policyMode === 'discharge' && hwMode === 'zero_charge_only') {
      const pvW = Math.round(inputs.p1?.pv_power_estimated ?? 0);
      const gridPower = inputs.p1?.resolved_gridPower ?? 0;
      const surplus = gridPower < 0 ? Math.abs(Math.round(gridPower)) : 0;
      const pvLabel = surplus > 0 ? `${surplus}W overschot van ${pvW}W` : (pvW > 0 ? `${pvW}W productie` : '');
      return {
        icon: '🔋',
        category: 'mapping',
        text: `Zon-laden i.p.v. ontladen — live PV-overschot${pvLabel ? ` (${pvLabel})` : ''} laadt batterij richting hogere prijs straks (opslaan loont meer dan nu exporteren). Geen ontlading → geen RTE-verlies.`,
        impact: 'critical',
        sentiment: 'positive',
        supportedMode: 'charge'
      };
  ...
  _generateSummary(recommendation, reasons, inputs) {
    ...
    const MODE_LABELS_NL = {
      ...
      standby: 'batterij standby te zetten',
