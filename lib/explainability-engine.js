...
class ExplainabilityEngine {
  ...
  _addArbitrageReasons(reasons, inputs) {
    ...
    const oppDischargeFloor = inputs.settings?.opportunistic_discharge_floor ?? 0.20;
    const effectiveDischargeFloor = respectMinMax ? minDischargePrice : oppDischargeFloor;
    ...
    if (hwMode === 'zero' && inputs._pvStoreWins) {
      ...
      if (!hasChargeReason) {
        reasons.push({
          icon: '⚡',
          category: 'mode',
          text: `Modus Net-0 actief — batterij laadt uit PV-overschot én ontlaadt bij verbruikspieken. Net blijft ~0W.`,
          impact: 'high',
          sentiment: 'positive',
          supportedMode: 'charge'
        });
      ...
      if (!hasChargeReason) {
        ...
        if (effectiveDischargeFloor > 0 && typeof price === 'number' && price < effectiveDischargeFloor) {
          const blockLabel = respectMinMax
            ...
            : `opportunistische drempel ${fmt(oppDischargeFloor)}`;
  ...
  _buildMappingReason(normalized, inputs, scores) {
    ...
    if (policyMode === 'discharge' && hwMode === 'standby') {
      const respectMinMax = (inputs.settings?.policy_mode === 'balanced-dynamic')
        ...
        : inputs.settings?.respect_minmax !== false;
      const oppDischargeFloor = inputs.settings?.opportunistic_discharge_floor ?? 0.20;
      const effectiveFloor = respectMinMax ? minDischarge : oppDischargeFloor;
