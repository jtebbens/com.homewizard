...
class ExplainabilityEngine {
  ...
  _generateDpSummary(recommendation, inputs) {
    ...
    if (exception === 'delay_charge') {
      return `Advies: standby. Reden: PV exporteert nu naar net${priceStr ? ` (${priceStr})` : ''} — laden uitgesteld naar goedkoper moment.`;
    }
    ...
    if (action === 'charge') {
      ...
      if (upwind?.upwindKt != null && upwind.upwindKt < 0.98 && (upwind.thisFf ?? 0) > 1) {
        const leadMin = Math.round((40_000 / upwind.thisFf) / 60);
        parts.push(`wolken in aantocht over ~${leadMin} min (upwind kt=${upwind.upwindKt.toFixed(2)})`);
      }
    ...
    if (dp.pvExportWins) {
      ...
      const exportStr = priceStr ? ` (${priceStr}/kWh)` : '';
  ...
  _addUpwindReasons(reasons, inputs) {
    const upwind = inputs.upwindData;
    if (!upwind || upwind.upwindKt == null || upwind.upwindKt >= 0.98 || (upwind.thisFf ?? 0) <= 1) return;
    const leadMin = Math.round((40_000 / upwind.thisFf) / 60);
    const stationStr = upwind.station ? ` bij ${upwind.station}` : '';
    const ktPct = Math.round(upwind.upwindKt * 100);
    reasons.push({
      icon: '🌥️',
      category: 'pv',
      impact: 'high',
      sentiment: 'negative',
      supportedMode: 'charge',
      text: `Wolken in aantocht${stationStr} (transmissie ${ktPct}%) — PV-opbrengst verwacht lager over ~${leadMin} min. DP laadt eerder om goedkope stroom veilig te stellen.`,
    });
  }
