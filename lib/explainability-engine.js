...
class ExplainabilityEngine {
  ...
  _addTariffReasons(reasons, tariff, settings, tariffType, inputs) {
    ...
    const isTop3Cheap = Array.isArray(tariff.top3Lowest) &&
      tariff.top3Lowest.some(p => Math.abs(p - price) < 0.00001);
    
    const isTop3Expensive = Array.isArray(tariff.top3Highest) &&
      tariff.top3Highest.some(p => Math.abs(p - price) < 0.00001);
    ...
    if (isTop3Cheap) {
      ...
      if (profit > 0.05) {
        reasons.push({
          icon: '⚡',
          category: 'tariff',
          text: `TOP-3 GOEDKOOPSTE UUR (${fmt(price)}) — winst €${profit.toFixed(3)}/kWh na conversieverlies (20%).`,
          impact: 'critical',
          sentiment: 'positive'
        });
      } else {
    ...
    if (typeof price === 'number' && price <= maxChargePrice) {
      const minProfitable = price * 1.25; // Need 25% higher to break even
    ...
    if (typeof price === 'number' && price >= minDischargePrice) {
      reasons.push({
        icon: '💰',
        category: 'tariff',
        text: `Stroomprijs hoog (${fmt(price)}) — ontladen bespaart geld.`,
        impact: 'medium',
        sentiment: 'positive'
      });
    }
    ...
    if (policyMode === 'charge' || (price && price <= maxChargePrice)) {
      ...
      if (minDischargePrice < minProfitable) {
        ...
        reasons.push({
          icon: '⚠️',
          category: 'efficiency',
          text: `Conversieverlies 20% (€${loss.toFixed(3)}/kWh) — spread te klein voor winst.`,
          impact: 'medium',
          sentiment: 'negative'
        });
      }
