...
class ExplainabilityEngine {
  ...
  generateExplanation(recommendation, inputs, scores) {
    ...
    return {
      ...
      confidence: recommendation.confidence,
      reasons: reasons.slice(0, 8), // Increased from 6 to show more details
      warnings,
      scores,
      summary,
      shortSummary,
  ...
  _addBatteryReasons(reasons, warnings, battery, settings, tariffType, inputs) {
    ...
    // 2. Bij 0% moet BMS calibratie mogelijk lopen
    if (soc === 0) {
      reasons.push({
        icon: '🔧',
        category: 'battery',
        text: `Batterij op 0% — HomeWizard BMS calibratie actief, laden prioriteit.`,
        impact: 'critical',
        sentiment: 'negative'
      });
      warnings.push(`Batterij op 0% (BMS calibratie)`);
      return;
    }

    // 3. Onder ingestelde minimum SOC
    ...
    // 4. Boven ingestelde maximum SOC
    if (soc >= maxSoc) {
      reasons.push({
        icon: '🔋',
        category: 'battery',
...
_addTariffReasons(reasons, tariff, settings, tariffType, inputs) {
  ...
  // FIX: use the same price as the planning view
  ...
  // PROFITABILITY CHECK
  // ⭐ Use battery cost model if available to determine if we're in a profitable charge/discharge scenario
  if (inputs.batteryCost?.avgCost > 0) {
    ...
    if (price > breakEven + 0.01) {
      ...
      reasons.push({
        ...
        impact: 'high',
        sentiment: 'positive',
        text: `Huidige prijs (${fmt(price)}) ligt onder jouw kostprijs (${fmt(inputs.batteryCost.avgCost)}) — laden is goedkoop.`
      });
    } else {
      reasons.push({
        icon: '⚖️',
  ...
  // Top-3 cheap hour
  if (isTop3Cheap) {
    const EFFICIENCY = inputs.batteryEfficiency ?? 0.80;
  ...
  // Regular expensive price
  if (typeof price === 'number' && price >= minDischargePrice) {
    reasons.push({
      icon: '💰',
      category: 'tariff',
      text: `Stroomprijs hoog (${fmt(price)}) — ontladen voorkomt dure netstroom, maar levert geen arbitragewinst op.`,
      impact: 'high',
      sentiment: 'positive'
    });
  }
  ...
  if (sunTomorrow >= 4.0 && soc > 50) {
    ...
    const effectiveMinDischarge = minDischargePrice * (currentHour >= eveningStartHour || currentHour < 1 ? 0.75 : 0.9);

    if (typeof price === 'number' && price >= effectiveMinDischarge && price < minDischargePrice) {
      const timeWindow = currentHour >= eveningStartHour || currentHour < 1
        ? `avond (${eveningStartHour}:00-00:59)`
        : `ochtend vroeg (01:00-${preDawnEndHour}:00)`;
      const threshold = effectiveMinDischarge.toFixed(3);

      reasons.push({
        icon: '🌅',
        category: 'tariff',
        text: `Weerbewust ontladen: ${sunTomorrow.toFixed(1)}h zon morgen, ${timeWindow}, verlaagde drempel €${threshold} — batterij wordt morgen gratis geladen.`,
        impact: 'high',
        sentiment: 'positive'
      });
    }
  ...
  if (typeof price === 'number' && price <= 0.05) {
    reasons.push({
      icon: '⚡',
      category: 'tariff',
      text: `Extreem lage prijs (${fmt(price)}) — zelfs na 20% verlies winstgevend.`,
      impact: 'critical',
      sentiment: 'positive'
    });
  }
...
_addArbitrageReasons(reasons, inputs) {
  ...
  // Geen kostprijs bekend → niets uitleggen
  if (avg <= 0) return;
  ...
  if (price < breakEven - 0.01) {
    reasons.push({
      ...
      impact: 'high',
      sentiment: 'positive',
      text: `Laden is goedkoop: huidige prijs (${fmt(price)}) ligt onder jouw gemiddelde kostprijs (${fmt(avg)}).`
    });
    return;
  }
  ...
  _addPVReasons(reasons, inputs) {
    ...
    if (gridPower < -150) {
      ...
      reasons.push({
        icon: '☀️',
        category: 'pv',
        text: `PV-overschot gedetecteerd (${exportPower}W export) — gratis laden mogelijk.`,
        impact: 'high',
        sentiment: 'positive'
      });
      return;
    ...
    if (sunTomorrow >= 4.0 && soc > 50) {
      ...
      reasons.push({
        icon: '🌥️',
        category: 'pv',
        text: `Geen PV-opwek — grid laden kost 20% conversieverlies.`,
        impact: 'medium',
        sentiment: 'neutral'
      });
    }
  ...
  _addSunForecastReasons(reasons, inputs) {
    ...
    // Only explain sun decision if we're in a cheap hour
  ...
  _addEfficiencyReasons(reasons, inputs) {
    ...
    // Only show efficiency explanation when charging from grid
    if (policyMode === 'charge' || (price && price <= maxChargePrice)) {
      const EFFICIENCY = inputs.batteryEfficiency ?? 0.80;
      const minProfitable = price * 1.25;

      if (minDischargePrice < minProfitable) {
        const loss = price * 0.20;
        reasons.push({
          icon: '⚠️',
          category: 'RTE',
          text: `RTE-verlies 20% (€${loss.toFixed(3)}/kWh) — spread te klein voor winst.`,
          impact: 'medium',
          sentiment: 'negative'
        });
      }
  ...
  _addConfidenceReason(reasons, recommendation) {
    ...
    if (c >= 90) {
      reasons.push({
        icon: '🎯',
        category: 'confidence',
        text: `Zeer hoge zekerheid (${c}%) — duidelijke beste keuze.`,
        impact: 'low',
        sentiment: 'positive'
      });
    } else if (c >= 70) {
      reasons.push({
        icon: '🎯',
        category: 'confidence',
        text: `Hoge zekerheid (${c}%) — goede onderbouwing.`,
        impact: 'low',
        sentiment: 'positive'
      });
    } else if (c >= 50) {
      reasons.push({
        icon: '🎯',
        category: 'confidence',
        text: `Redelijke zekerheid (${c}%) — meerdere factoren in balans.`,
        impact: 'low',
        sentiment: 'neutral'
      });
    } else {
      reasons.push({
        icon: '⚠️',
        category: 'confidence',
        text: `Lage zekerheid (${c}%) — scores lagen dicht bij elkaar.`,
        impact: 'low',
        sentiment: 'neutral'
      });
    }
  ...
  _generateSummary(recommendation, reasons, inputs) {
    ...
    // Get top 2-3 most important reasons
  ...
  const rte = inputs?.batteryEfficiency;
