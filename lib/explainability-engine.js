...
class ExplainabilityEngine {
  ...
  _addTariffReasons(reasons, tariff, settings, tariffType, inputs) {
    ...
    if (inputs.batteryCost?.avgCost > 0) {
      ...
      const minDischargePriceTariff = settings.min_discharge_price ?? 0;

      if (price > breakEven + 0.01) {
        if (minDischargePriceTariff > 0 && price < minDischargePriceTariff) {
          reasons.push({
            icon: '⚖️',
            category: 'tariff',
            impact: 'medium',
            sentiment: 'neutral',
            text: `Huidige prijs (${fmt(price)}) ligt boven break‑even (${fmt(breakEven)}) — ontladen winstgevend, maar geblokkeerd door ingestelde min. ontlaadprijs (€${minDischargePriceTariff.toFixed(3)}).`,
            supportedMode: 'preserve'
          });
        } else {
          reasons.push({
