    ...
    if (soc === 0) {
      ...
      if (_exportWins) {
        reasons.push({
          icon: '⚡',
          category: 'battery',
          text: `Batterij op 0% — laden nu niet rendabel (laadprijs €${_price.toFixed(3)} > verwachte ontlaadwaarde €${_maxFuture.toFixed(3)} × ${_eff.toFixed(2)} = €${_storeValue.toFixed(3)}). Standby aanbevolen.`,
          impact: 'critical',
          sentiment: 'positive',
    ...
    if (gridPower < -150) {
      ...
      if (_exportWins) {
        ...
        reasons.push({
          icon: '⚡',
          category: 'pv',
          text: `PV-overschot (${exportPower}W export) — export naar net winstgevender (€${_exportVal.toFixed(3)} > €${_maxFuture.toFixed(3)} × ${_eff.toFixed(2)} = €${_storeValue.toFixed(3)})${delayNote}.`,
          impact: 'high',
          sentiment: 'positive',
          supportedMode: 'preserve'
        });
