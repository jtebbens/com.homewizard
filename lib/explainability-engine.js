...
class ExplainabilityEngine {
    ...
    if (soc === 0) {
      ...
      if (_exportWins) {
        reasons.push({
          icon: '⚡',
          category: 'battery',
          text: `Batterij op 0% — export naar net winstgevender (€${_price.toFixed(3)} > max €${_maxFuture.toFixed(3)} × ${_eff.toFixed(2)} = €${_storeValue.toFixed(3)}). Stand-by aanbevolen.`,
          impact: 'critical',
          sentiment: 'positive',
  ...
  _addLoadAwarenessReasons(reasons, inputs) {
    ...
    const maxDischarge = battery.maxDischargePowerW ||
                        (battery.totalCapacityKwh
                          ? Math.max(1, Math.round(battery.totalCapacityKwh / 2.688)) * 800
                          : 800);
    ...
    if (gridPower < -150) {
      ...
      if (_exportWins) {
        reasons.push({
          icon: '⚡',
          category: 'pv',
          text: `PV-overschot (${exportPower}W export) — export naar net winstgevender (€${_price.toFixed(3)} > €${_maxFuture.toFixed(3)} × ${_eff.toFixed(2)} = €${_storeValue.toFixed(3)}). Stand-by aanbevolen.`,
          impact: 'high',
          sentiment: 'positive',
          supportedMode: 'preserve'
        });
  ...
  _addWeatherReasons(reasons, weather) {
    ...
    const pvKwh = weather.pvKwhToday != null ? `, ~${weather.pvKwhToday} kWh vandaag` : '';

    if (sun4h >= 2) {
      reasons.push({
        icon: '☀️',
        category: 'weather',
        text: `Sterke zon komende 4 uur (${sun4h.toFixed(1)}h zonuren${pvKwh}) — batterij kan gratis geladen worden.`,
        impact: 'high',
        sentiment: 'positive',
        supportedMode: 'charge'
      });
      ...
      reasons.push({
        icon: '🌤️',
        category: 'weather',
        text: `Matige zon komende 4 uur (${sun4h.toFixed(1)}h zonuren${pvKwh}).`,
        impact: 'medium',
        sentiment: 'neutral',
        supportedMode: 'charge'
      });
    ...
    if (sun8h >= 3 && sun4h < 2) {
      ...
      // Zon verwacht later op de dag maar buiten het 8-uurs venster
      const pvKwh = weather.pvKwhToday != null ? `, ~${weather.pvKwhToday} kWh verwacht` : '';
      reasons.push({
        icon: '⛅',
        category: 'weather',
        text: `Zon verwacht later vandaag (${sunToday.toFixed(1)}h zonuren resterend${pvKwh}).`,
        impact: 'medium',
        sentiment: 'positive',
        supportedMode: 'preserve'
      });
      ...
      const pvKwhLabel = weather.pvKwhToday != null ? `~${weather.pvKwhToday} kWh` : 'weinig PV';
      const sunLabel = sunToday > 0 ? `${sunToday.toFixed(1)}h zon resterend` : 'geen zonuren verwacht';
      reasons.push({
        icon: '🌧️',
        category: 'weather',
        text: `Bewolkt/regen vandaag — ${pvKwhLabel} PV verwacht (${sunLabel}).`,
        impact: 'low',
        sentiment: 'neutral',
        supportedMode: 'preserve'
      });
  ...
  _addModeSpecificReasons(reasons, inputs, recommendation) {
    ...
    if (hwMode === 'zero_charge_only') {
      ...
      if (!hasChargeReason) {
        ...
        const parts = [];
        if (minDischargePrice > 0 && typeof price === 'number' && price < minDischargePrice) {
          parts.push(`ontladen geblokkeerd (prijs ${fmt(price)} < min. ontlaadprijs ${fmt(minDischargePrice)})`);
        }
  ...
  _buildMappingReason(normalized, inputs, scores) {
    ...
    if (policyMode === 'charge'   && (hwMode === 'to_full' || hwMode === 'zero_charge_only')) return null;
    ...
    if (policyMode === 'discharge' && hwMode === 'standby') {
      const parts = [];
      if (typeof price === 'number' && price < minDischarge) {
        parts.push(`prijs ${fmt(price)} < min ontlaadprijs ${fmt(minDischarge)}`);
      } else {
