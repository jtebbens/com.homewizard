...
// Cached formatters — constructing a fresh Intl.DateTimeFormat per call inside a
// per-slot loop was a confirmed CPU hotspot (232 samples in one profiled second).
...
function buildModeChartBody(modeHistory) {
  ...
  const labels = slots.map(s => {
    ...
    const h = _amsHourFormatter.format(new Date(s + ':00Z'));
 ...
 * @param {Object} pvData - { pvActual, pvForecast, pvForecastOM, pvForecastSC, pvForecastDayStart, pvScDayStart, pvCapacityW }
 * @returns {Object|null} chart config for quickchart.io, or null if no data
 */
function buildPvChartConfig(pvData) {
  const { pvActual, pvForecast, pvForecastOM, pvForecastSC, pvForecastSAT, pvForecastDayStart, pvScDayStart, pvCapacityW } = pvData || {};
  const todayStr = _amsDayKeyFormatter.format(new Date());
  ...
  const scTodaySnap = (() => {
    ...
    for (const slot of pvScDayStart.data) {
      const h = parseInt(_amsHourFormatter.format(new Date(slot.timestamp)), 10);
  ...
  const forecastData = hours.map(h => {
    ...
    if (h < nowAmsHour) {
      const dpPast = pvForecast?.[0]?.[h];
      if (dpPast != null) return dpPast;
  ...
  // Raw p50 only — pvForecastSCEffective (blend weight, sometimes p10-substituted)
  // is diag-only and must not appear on this chart under the "Solcast" label.
  const scToday    = scTodayFull;
  ...
  const satToday    = pvForecastSAT?.[0] || {};
  const satTomorrow = pvForecastSAT?.[1] || {};
  const hasSAT = Object.keys(satToday).length > 0 || Object.keys(satTomorrow).length > 0;
  const satData = hasSAT ? hours.map(h => {
    if (h < 24) return satToday[h] ?? null;
    return satTomorrow[h - 24] ?? null;
  }) : null;
  ...
  const pvMax = Math.max(
    ...
    ...Object.values(satToday).map(v => v || 0),
    ...Object.values(satTomorrow).map(v => v || 0),
    1
  );
