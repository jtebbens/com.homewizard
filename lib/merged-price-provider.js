...
class MergedPriceProvider {
  constructor(homey, options = {}) {
    ...
    this.entsoe = new EntsoeFallbackProvider(homey, { markup: this.markup });
  ...
  async fetchPrices(force = false) {
    ...
    // If primary sources only cover today, try ENTSOE for tomorrow
    if (this.cache.length <= 24) {
      try {
        const entsoePrices = await this.entsoe.fetchPrices(true);
        if (entsoePrices?.length > 0) {
          const existingKeys = new Set(this.cache.map(p =>
          ...
          for (const ep of entsoePrices) {
            ...
            if (!existingKeys.has(ts.toISOString())) {
