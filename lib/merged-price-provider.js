  ...
  async _saveCache() {
    try {
      await this.homey.settings.set('merged_price_cache', {
        prices: this.cache,
        // Persist sub-provider 15-min caches so they survive restarts.
        // Both sub-providers use a no-op _saveCache(), so this is the only place
        // their cache15min data is written to persistent storage.
