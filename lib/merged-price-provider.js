...
const PbthProvider = require('./pbth-provider');
...
// retry window while tomorrow is still incomplete — PBTH is often late publishing
 ...
 * MergedPriceProvider
 *
 * Single primary source (PBTH, via the paired com.gruijter.powerhour device) with an ENTSOE
 * fallback: used whenever PBTH returns nothing (no device paired, app not installed, fetch
 * failure) and to fill any tail PBTH doesn't yet cover (e.g. tomorrow not published yet).
 *
 * Typical coverage:
 *   Before ~13:15 CET  → today 24h
 *   After  ~13:15 CET  → today + tomorrow up to 48h
 *
 * Exposes the same public interface as the sub-providers so it is a drop-in for
 * TariffManager.dynamicProvider.
 */
class MergedPriceProvider {
  constructor(homey, options = {}) {
    ...
    this.pbth = new PbthProvider(homey, { ...subProviderOptions, deviceId: options.deviceId || null });
  ...
  async _loadCache() {
    try {
      ...
      if (cached && cached.expiry > Date.now()) {
        ...
        // A cache saved while the primary source was fully down (emergency
        ...
        // retry PBTH soon, same as the normal <48h emergency case already does.
        ...
        if (cached.prices15min_pbth?.length > 0) {
          this._cached15min_pbth = cached.prices15min_pbth.map(p => ({
        ...
        this.log(
          ...
          `15-min: ${this._cached15min_pbth?.length || 0} pbth` +
  ...
  async _saveCache() {
    ...
    this._saveCacheTimer = setTimeout(() => {
      ...
      const ok = userdata.writeJson(CACHE_FILE, {
        prices: this.cache,
        prices15min_pbth:   this.pbth.cache15min   || null,
  ...
  /**
   * Does the merged table reach back to today 00:00 Amsterdam?
   *
   * The mirror image of _coversThroughTomorrow(), for the other end of the table. PBTH on a
   * 'dap15' device only serves slots ahead of now, so a table built from it alone starts at
   * the current slot — today's elapsed hours are simply absent, and nothing later re-supplies
   * them (the coverage brake sees a complete forward horizon and skips the round). ENTSOE
   * serves whole-day files, so it is the only source that can fill them back in.
   *
   * A table anchored on an earlier calendar day also counts: _dropPastDays() trims those, and
   * until it runs they already cover today from its first hour.
   *
   * @param {Date} now - injectable clock, for tests
   */
  _coversFromTodayStart(now = new Date()) {
    if (!this.cache?.length) return false;

    const first = this.cache[0].timestamp;
    const firstTs = first instanceof Date ? first : new Date(first);
    if (Number.isNaN(firstTs.getTime())) return false;

    const firstLocal = this._amsParts(firstTs);
    const todayDate = this._amsParts(now).date;

    return firstLocal.date < todayDate || firstLocal.hour === 0;
  }
   ...
   */
  _restore15MinCaches() {
    if (!this.pbth.cache15min && this._cached15min_pbth?.length > 0) {
      this.pbth.cache15min = this._cached15min_pbth;
      this.log(`♻️ Restored ${this.pbth.cache15min.length} PBTH 15-min prices from merged cache`);
    }
   ...
   * complete while tomorrow has no native 15-min data at all — PBTH on a 'dap' (hourly) device
   * serves today's hours only and ENTSOE fills tomorrow's. TariffManager then expands those hours
   * to four identical quarters and the DP plans tomorrow on averaged-away peaks (observed 2026-08-06).
   */
  _covers15MinThroughTomorrow(now = new Date()) {
    ...
    const tomorrowDate = this._amsParts(new Date(now.getTime() + 24 * 3600 * 1000)).date;
    const sources = [this.pbth.cache15min, this.entsoe.cache15min];
  ...
  async fetchPrices(force = false) {
    ...
    // each sub-provider's own cache and re-fetches. Once the merged table already spans
    // through tomorrow 23:00 Amsterdam there is nothing left to learn this side of
    // midnight, so skip the whole round — force included.
    ...
    const pbthRaw = await this.pbth.fetchPrices(force);
    ...
    const pbthPrices = (pbthRaw || []).filter(isValidPrice);
    if ((pbthRaw || []).length !== pbthPrices.length) {
      this.log(`⚠️ PBTH returned ${pbthRaw.length - pbthPrices.length} invalid-price slots, dropped`);
    }

    if (pbthPrices.length === 0) {
      // PBTH unavailable (no device paired, app not installed, or fetch failure) — try ENTSOE
      this.log('⚠️ PBTH unavailable, trying ENTSOE fallback...');
      try {
      ...
      throw new Error('MergedPriceProvider: all sources (PBTH, ENTSOE) returned no data');
    }

    this.lastFetchSources = ['pbth'];
    this.cache = pbthPrices.map(p => ({ ...p, _source: 'pbth' })).sort((a, b) => a.timestamp - b.timestamp);

    // If the table has a gap at either end, try ENTSOE to fill it. Ahead: PBTH hasn't
    // published tomorrow's slots yet, or the paired device is 'dap' and only serves hours.
    // Behind: PBTH only serves slots ahead of now, so today's elapsed hours are missing and
    // the assignment above just dropped whatever the previous table held of them. Testing
    // only the far end left that gap permanent — the coverage brake skips every later round,
    // so the table stayed truncated until midnight (live 2026-08-10: 48h → 33h, 0h ENTSOE).
    if (!this._coversThroughTomorrow() || !this._coversFromTodayStart()) {
      try {
        const entsoePrices = this._dropPastDays(await this.entsoe.fetchPrices(true));
        if (entsoePrices.length > 0) {
          const existingIdx = new Map(this.cache.map((p, i) => [
          ...
          let added = 0, addedTomorrow = 0, checked = 0, maxDelta = 0, repaired = 0;
          for (const ep of entsoePrices) {
            ...
            if (idx != null) {
              ...
              checked++;
              const delta = Math.abs(ep.price - existing.price);
              if (delta > maxDelta) maxDelta = delta;

              // Gap-fill has one exception: an hour PBTH averaged over fewer quarters than
              // ENTSOE did. That happens every round on the running hour — PBTH serves only
              // slots ahead of now, and the assignment above replaces yesterday's complete
              // value with the truncated one. Both slotCounts present means both sides folded
              // 15-min data, so more quarters is strictly the better average.
              if (existing.slotCount != null && ep.slotCount != null && ep.slotCount > existing.slotCount) {
                this.cache[idx] = { ...ep, _source: 'entsoe' };
                repaired++;
              }
            } else {
          ...
          if (checked > 0) {
            this.log(
              `🔍 ENTSOE cross-check: ${checked} overlapping slots, max Δ€${maxDelta.toFixed(4)}` +
              (repaired > 0 ? `, ${repaired} partial-hour slot(s) replaced` : '')
            );
          }
          if (added > 0) {
    ...
    const pbthSlots    = this.cache.filter(p => p._source === 'pbth').length;
