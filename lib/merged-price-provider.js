...
class MergedPriceProvider {
  ...
  async fetchPrices(force = false) {
    ...
    // SHADOW ONLY — logs, does not act yet. Once proven, this becomes an early
    // `return this.cache` that also skips the forced refresh from the policy run
    // (device.js calls fetchPrices(true) every run, which bypasses kWhPrice's own cache
    // and re-fetches 53 kB of HTML even when today+tomorrow are already complete).
    if (this._coversThroughTomorrow()) {
      ...
      this.log(`[PRICE-BRAKE] shadow: would skip fetch (cache covers through ${localTs} Amsterdam, ${this.cache.length}h, force=${force})`);
    }
