  ...
  async _selectBestProvider() {
    try {
      ...
      if (xadiHasPrices) {
        this.dynamicProvider = this.xadiProvider;
        this.activeProvider = 'xadi';
        this.log('✅ Using Xadi provider (primary source)');
        return;
      }
      
      this.log('⚠️ Xadi returned no prices, falling back to EnergyZero');
    } catch (error) {
      this.log('⚠️ Xadi fetch failed, falling back to EnergyZero:', error.message);
    }
    ...
    try {
      ...
      this.log('✅ Using EnergyZero provider (fallback)');
    } catch (error) {
      this.log('❌ Both providers failed:', error.message);
      this.dynamicProvider = null;
    }
  }
