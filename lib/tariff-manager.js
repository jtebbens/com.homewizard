    ...
    try {
      ...
      if (count > 0) {
        ...
        if (all15min.length > 0) {
          ...
          this.log(`💾 Initial save: ${all15min.length} 15-min prices after full fetch (${hasNative15Min ? 'native' : 'expanded hourly'})`);
        }
        return;
      }
    ...
    if (!this._lastPriceSettingsSave || _now - this._lastPriceSettingsSave > 300_000) {
      ...
      (async () => {
        try {
          ...
          // Save 15-min prices for planning view — always, even when expanded from hourly.
          // Stale/empty data is worse than fresh expanded data.
          if (allPrices15min && allPrices15min.length > 0) {
            ...
            const hasNative = !!(this.kwhpriceProvider?.cache15min?.length > 0 ||
                                 this.xadiProvider?.cache15min?.length > 0);
            this.log(`💾 Saved ${allPrices15min.length} 15-min prices to settings (${hasNative ? 'native' : 'expanded hourly'})`);
          }
        } catch (err) {
