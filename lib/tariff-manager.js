...
class TariffManager {
  ...
  _initializeDynamicProvider() {
    ...
    // For now, we'll use Xadi as it has the cleanest API
    // In the future, this could be made configurable
    this.dynamicProvider = new XadiProvider(this.homey);
    
    // Fetch prices immediately
    this.dynamicProvider.fetchPrices().catch(err => {
      this.log('Initial price fetch failed:', err.message);
    });
    
    this.log('Dynamic pricing enabled with Xadi provider');
  }
  ...
  // Apply user's markup if configured
  const markup = this.settings.dynamic_price_markup || 0;
  const finalPrice = currentPrice ? currentPrice + markup : null;
