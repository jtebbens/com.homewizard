...
class TariffManager {
  ...
  async _selectBestProvider() {
    ...
    try {
      ...
      const xadiHasPrices = this.xadiProvider.hasPrices();
      
      if (xadiHasPrices) {
        this.dynamicProvider = this.xadiProvider;
        this.activeProvider = 'xadi';
        const priceCount = this.xadiProvider.cache?.length || 0;
        this.log(`✅ Using Xadi provider (${priceCount} prices available)`);
        return;
      }
      
      this.log('⚠️ Xadi returned no prices, trying EnergyZero fallback...');
    } catch (error) {
    ...
    try {
      ...
      const ezHasPrices = this.energyzeroProvider.hasPrices();
      
      if (ezHasPrices) {
        this.dynamicProvider = this.energyzeroProvider;
        this.activeProvider = 'energyzero';
        const priceCount = this.energyzeroProvider.cache?.length || 0;
        this.log(`✅ Using EnergyZero provider (${priceCount} prices available, fallback active)`);
        return;
      }
