  ...
  async _selectBestProvider() {
    ...
    try {
      ...
      if (count > 0) {
        ...
        // Guard: only save if sub-providers have native 15-min cache — otherwise
        // getAll15MinPrices() returns _expandHourlyTo15Min() (4 identical prices/hour)
        // which would overwrite any valid 15-min data from the previous session.
        ...
        if (hasNative15Min) {
          ...
          this.log('⚡ 15-min native cache not yet available — keeping existing policy_all_prices_15min');
        }
        return;
      }
    ...
    if (!this._lastPriceSettingsSave || _now - this._lastPriceSettingsSave > 300_000) {
      ...
      (async () => {
        try {
          ...
          // Save 15-min prices for planning view — only when native data is available.
          // If sub-providers have no cache15min, getAll15MinPrices() returns expanded
          // hourly (4 identical prices/hour); saving that would overwrite valid data.
          const hasNative = !!(this.kwhpriceProvider?.cache15min?.length > 0 ||
                               this.xadiProvider?.cache15min?.length > 0);
          if (hasNative && allPrices15min && allPrices15min.length > 0) {
