...
class TariffManager {
  ...
  _initializeDynamicProvider() {
    ...
    this.mergedProvider   = new MergedPriceProvider(this.homey, {
      ...
      getResolution: () => this.settings.price_resolution
    });
    ...
    try {
      ...
      if (count > 0) {
        ...
        if (all15min.length > 0) {
          const hasNative15Min = !!(this.kwhpriceProvider?.cache15min?.length > 0 ||
                                    this.xadiProvider?.cache15min?.length > 0);
  ...
  _getDynamicTariff(gridPower, now) {
    ...
    const force1h      = this.settings.price_resolution === '1h';
    ...
    if (!this._lastPriceSettingsSave || _now - this._lastPriceSettingsSave > 300_000) {
      ...
      const hasNative = !!(this.kwhpriceProvider?.cache15min?.length > 0 ||
                           this.xadiProvider?.cache15min?.length > 0);
  ...
  getAll15MinPrices() {
    ...
    try {
      ...
      if (this.kwhpriceProvider && typeof this.kwhpriceProvider.getAll15MinPrices === 'function') {
        ...
        nativeCount += addPrices(kwhPrices, 'kwhprice');
      }
      ...
      if (this.xadiProvider && typeof this.xadiProvider.getAll15MinPrices === 'function') {
        ...
        nativeCount += addPrices(xadiPrices, 'xadi');
      }
  ...
  getCurrent15MinPrice() {
    ...
    try {
      // Try kwhprice first (consistent with getAll15MinPrices priority)
