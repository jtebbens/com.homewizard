...
class TariffManager {
  ...
  _getDynamicTariff(gridPower, now) {
    ...
    return {
      ...
      top3Highest,
      allPrices,
      allPrices15min,     // ✅ NEW: Include 15-min prices
      next24Hours,
      timestamp:          now,
    ...
    try {
      ...
      if (this.kwhpriceProvider && typeof this.kwhpriceProvider.getAll15MinPrices === 'function') {
        ...
        if (kwhPrices && kwhPrices.length > 0) {
          ...
          foundNative = true;
          this.log(`📊 Got ${kwhPrices.length} native 15-min prices from KwhPrice`);
        }
      }
      ...
      if (this.xadiProvider && typeof this.xadiProvider.getAll15MinPrices === 'function') {
        ...
        if (xadiPrices && xadiPrices.length > 0) {
          ...
          foundNative = true;
          this.log(`📊 Got ${xadiPrices.length} native 15-min prices from Xadi`);
        }
      }
      ...
      if (foundNative && all15min.length > 0) {
        ...
        this.log(`📊 Merged ${deduplicated.length} total 15-min intervals from providers`);
        return deduplicated;
      }
