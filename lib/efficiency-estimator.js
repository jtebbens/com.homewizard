  ...
  update(p1, battery, activeMode = null) {
    ...
    if (
      ...
      if (newEff >= 0.65 && newEff <= 0.85) {
        ...
        this.state.totalChargeKwh = 0;
        this.state.totalDischargeKwh = 0;
      } else if (newEff < 0.65 && this.state.totalChargeKwh > 2.0) {
        // Implausibly low — likely biased by standby leak or partial cycle, discard
        this.homey.log(
          `[Efficiency] ⚠️ RTE measurement ${(newEff * 100).toFixed(1)}% implausibly low after ${(this.state.totalChargeKwh * 1000).toFixed(0)}Wh → discarding`
        );
        this.state.totalChargeKwh = 0;
        this.state.totalDischargeKwh = 0;
