...
class EfficiencyEstimator {
  ...
  update(p1, battery, activeMode = null) {
    ...
    if (
      ...
      // Threshold 1.45: allows measurements down to 1/1.45 = 69% true RTE,
      // consistent with the 70% floor below. Filters out badly skewed windows
      // where charge far outpaces discharge (charge:discharge > 1.45:1).
      const balanceRatio = this.state.totalChargeKwh / this.state.totalDischargeKwh;
      if (balanceRatio > 1.45) {
