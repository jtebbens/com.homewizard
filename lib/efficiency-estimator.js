...
class EfficiencyEstimator {
  constructor(homey) {
    ...
    this.state = this.homey.settings.get('efficiency_state') || {
      totalChargeKwh: 0,
      totalDischargeKwh: 0,
      efficiency: 0.75,  // Default 78% RTE (conservative start, learning engine corrects this)
      lastTimestamp: null,
      lastSoc: null,
  ...
  update(p1, battery, activeMode = null) {
    ...
    if (
      ...
      // Stale counter guard: reset if counters grow too large without completing a cycle.
      // Must run BEFORE balance-ratio check to avoid infinite stalling when PV charges
      // far more than the battery discharges (ratio stays > 1.45 indefinitely).
      if (this.state.totalChargeKwh > 10.0 || this.state.totalDischargeKwh > 10.0) {
        this.homey.log(
      ...
      // Balance check: prevent partial-cycle bias where PV charges much more
      // than the discharge window captured (e.g. 1.5kWh charge / 1.0kWh discharge = 67% RTE).
      // Threshold 1.40: allows measurements down to 1/1.40 = 71.4% true RTE,
      // just above the 70% floor below so guard passage always yields a valid measurement.
      // (Old threshold 1.45 → minimum 68.9% → always discarded at the 70% floor.)
      const balanceRatio = this.state.totalChargeKwh / this.state.totalDischargeKwh;
      if (balanceRatio > 1.40) {
        // Discharge hasn't caught up yet — wait for more discharge before measuring
        return;
      }
