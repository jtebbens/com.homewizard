 ...
 *
 * A bonus (positive addon and/or multiplier > 1) is paid on positive prices
 * only — Zonneplan and NextEnergy both gate it that way — so at spot ≤ 0 the
 * bare spot stands and the exporter pays per kWh. Without this gate a €0.02
 * addon masks the sign for the whole -0.02..0 band, and the DP reads a payment
 * as revenue. A NEGATIVE addon is a feed-in fee, not a bonus: it keeps applying
 * at negative spot, so the gate must not swallow it.
 */
function exportPrice(spotEur, addon, multiplier) {
  const hasBonus = addon > 0 || multiplier > 1;
  if (hasBonus && spotEur <= 0) return spotEur;
...
/**
 * Effective round-trip efficiency: the measured estimate when it is usable, else the
 * configured setting. Single source so the policy gate, the DP and the explanation can
 * never pick different numbers for the same decision — the [0.50, 0.97] window is the
 * one device.js already applies before handing learnedRte to optimization-engine.
 */
function effectiveRte(learnedRte, configuredRte = 0.75) {
  const l = Number(learnedRte);
  return (Number.isFinite(l) && l >= 0.50 && l <= 0.97) ? l : configuredRte;
}

module.exports = { importPrice, exportPrice, exportValue, disposalValue, storeValue, effectiveRte };
