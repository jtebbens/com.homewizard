...
// Cached formatters — constructing a fresh Intl.DateTimeFormat per slot inside
// _process15MinSlots/_aggregateToHourly was the single biggest CPU hotspot found
// in this app (473 samples in one profiled second, ~96 slots/day × 2 calls each).
...
class KwhPriceProvider {
  constructor(homey, options = {}) {
    ...
    this.log(`KwhPrice provider initialized with markup: €${this.markup}/kWh, export ×${this.exportMultiplier} +€${this.exportAddon}/kWh`);
   ...
   *
   * The page may contain two datasets: today and tomorrow. Both are parsed.
  ...
  _parseHtml(html, todayAms) {
    ...
    const toUtcForDay = (date, offset) => (hh, mm) =>
      new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), hh, mm, 0) - offset);

    const toUtcToday    = toUtcForDay(todayUtcMidnight, offsetToday);
    const toUtcTomorrow = toUtcForDay(tomorrowUtcMidnight, offsetTomorrow);

    // One shared labels array, two data arrays (datasets[0]=today, datasets[1]=tomorrow)
    const labelsMatch = html.match(/labels:\s*\[([\s\S]*?)\]/);
    const dataMatches = [...html.matchAll(/\bdata:\s*\[([-\d.,\s\n]+)\]/g)];

    if (!labelsMatch || dataMatches.length === 0) {
      this.log('kWhPrice: no Chart.js labels/data arrays found in page');
      return [];
    }

    // Shared time labels for both today and tomorrow
    const times = [...labelsMatch[1].matchAll(/"(\d{2}):(\d{2})-\d{2}:\d{2}"/g)]
      .map(m => ({ hh: parseInt(m[1], 10), mm: parseInt(m[2], 10) }));

    const datasets = [
      { dataMatch: dataMatches[0], toUtc: toUtcToday,    label: 'today' },
      ...(dataMatches.length > 1
        ? [{ dataMatch: dataMatches[1], toUtc: toUtcTomorrow, label: 'tomorrow' }]
        : [])
    ];

    for (const { dataMatch, toUtc, label } of datasets) {
      ...
      if (times.length === 0 || prices.length === 0) {
        this.log(`kWhPrice: no valid data for ${label}`);
        continue;
      }

      const count = Math.min(times.length, prices.length);
      this.log(`kWhPrice layout: Chart.js 15-min intervals [${count} slots ${label}]`);
  ...
  _process15MinSlots(slots) {
    return slots.map(slot => {
      ...
      const cetMinute = parseInt(_parts.minute);
  ...
  async fetchPrices(force = false) {
    ...
    try {
      ...
      // Page-change detect: if HTML > 2KB but 0 slots parsed, parser likely broken (selector changed).
      // Throw so the stale-cache fallback in catch{} engages instead of returning [].
      if (slots.length === 0) {
        if (html.length > 2000) {
          throw new Error(`kWhPrice parse failure: ${html.length}B HTML but 0 slots — page structure may have changed`);
        }
  ...
  getCurrentRate() {
    ...
    if (!currentPrice) {
      const currentHourCET = parseInt(_amsHourMinuteFormatter.formatToParts(now).find(p => p.type === 'hour').value);
