 ...
 *
 * Drop-in replacement for EnergyZeroProvider.
 * Scrapes https://kwhprice.eu/en/netherlands which publishes EPEX spot
 * prices for the Netherlands in 15-minute intervals (Europe/Amsterdam).
 *
 * 15-minute slots are averaged per hour to produce the same hourly
 * data format that the rest of the app expects.
 *
 * Pricing formula (same as EnergyZero):
 *   finalPrice = (spot + markup) × 1.21
 */
class KwhPriceProvider {
  ...
  async _loadCache() {
    try {
      ...
      if (cached && cached.expiry > Date.now()) {
        ...
        this.log(
          `Loaded ${this.cache.length} cached prices from storage ` +
  ...
  async _saveCache() {
    try {
      await this.homey.settings.set('kwhprice_cache', {
        prices: this.cache,
        expiry: this.cacheExpiry,
        savedAt: Date.now()
      });
    } catch (err) {
      this.error('Failed to save price cache:', err.message);
    }
  }
  ...
  /** Average 15-min slots into hourly buckets. */
  ...
  async fetchPrices(force = false) {
    ...
    try {
      ...
      if (slots.length === 0) {
        throw new Error('No price slots parsed from kWhPrice page — HTML structure may have changed');
      }
      ...
      this.log(`Fetched ${slots.length} 15-min slots (${days}) → ${this.cache.length} hourly prices from kWhPrice`);

      if (this.cache.length > 0) {
        const first = this.cache[0];
        const last = this.cache[this.cache.length - 1];
        this.log(
          `Price calc sample: (€${first.originalPrice.toFixed(5)} spot + €${this.markup} markup) × 1.21 = €${first.price.toFixed(5)}`
        );
        this.log(
          `Range: ${first.timestamp.toISOString()} (${first.hour}:00 AMS, €${first.price.toFixed(4)}) ` +
