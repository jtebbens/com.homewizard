...
class LearningEngine {
  ...
  async initialize() {
    ...
    if (stored) {
      ...
      // Observe-only: reset the warmup-inflated ensemble-GTI EMA so it re-seeds from
      // pv_accuracy_om (started at 1.0, looked artificially good). One-time.
      if (!this.data.pv_accuracy_ens_reset_v1) {
        this.data.pv_accuracy_ens = null;
        this.data.pv_accuracy_ens_reset_v1 = true;
        await this._saveData();
      }
   ...
   * provider already gives the Amsterdam-local hour/minute directly (KwhPrice,
   * Xadi item.hour), avoiding any UTC/local timestamp parsing ambiguity.
   ...
   */
  async recordPvAccuracy(predictedW, actualW, omW = null, scW = null, perModelW = null, ensW = null) {
    const now = Date.now();
    ...
    this.data.pv_predictions.push({
      ...
      sc:        scW ?? null,
      error:     error,
      mf:        perModelW?.['meteofrance_arpege_europe']              ?? null,
      gfs:       perModelW?.['gfs_seamless']                         ?? null,
      icon:      perModelW?.['icon_seamless']                         ?? null,
      knmi:      perModelW?.['knmi_harmonie_arome_netherlands']       ?? null,
    });
    ...
    // Observe-only: ensemble-GTI accuracy, to compare against best_match GTI (= pv_accuracy_om).
    // Seed from pv_accuracy_om (not 1.0) so both EMAs start equal and diverge purely on merit —
    // a fresh 1.0 start would make ensemble look artificially good for ~a day of warmup.
    if (ensW != null && ensW > 50) {
      if (this.data.pv_accuracy_ens == null) this.data.pv_accuracy_ens = this.data.pv_accuracy_om ?? 1.0;
      const ensErr = Math.abs(actualW - ensW) / Math.max(actualW, ensW, 1);
      this.data.pv_accuracy_ens = (alpha * (1 - ensErr)) + ((1 - alpha) * this.data.pv_accuracy_ens);
      // Observe-only verdict: surface the head-to-head so the compare is actually readable.
      const bm = this.data.pv_accuracy_om ?? 0;
      this.log(`[GTI ens] accuracy bm=${bm.toFixed(3)} ens=${this.data.pv_accuracy_ens.toFixed(3)} (Δ=${((this.data.pv_accuracy_ens - bm) * 100).toFixed(1)}pp, +=ens beter)`);
    }
   ...
   */
  getPvBlendWeights() {
    const accOM = this.data.pv_accuracy_om ?? null;
    const accSC = this.data.pv_accuracy_sc ?? null;
    if (accOM == null || accSC == null) return { wOM: 0.5, wSC: 0.5 };
    const total = accOM + accSC;
    if (total <= 0) return { wOM: 0.5, wSC: 0.5 };
    const raw = accSC / total;
    const wSC = Math.min(0.8, Math.max(0.2, raw));
    return { wOM: 1 - wSC, wSC };
  }
   ...
   */
  async recordDailyPvBiasFromPredictions(yDateStr, avgCloudPct = null, knmiKt = null) {
   ...
   */
  async saveForecastSnapshot(dateStr, forecastAvgWm2, perModel = null) {
    this.data.forecast_snapshots = this.data.forecast_snapshots || {};
    this.data.forecast_snapshots[dateStr] = { forecastAvgWm2, perModel, savedAt: Date.now() };
  ...
  /**
   * Update per-model accuracy EMA from yesterday's per-model forecast snapshot vs actual radiation.
   * @param {{ [model]: number }} perModelForecast - avg W/m² per model from yesterday's snapshot
   * @param {number} actualAvgWm2 - measured avg W/m² for yesterday's daylight hours
   */
  async recordModelAccuracy(perModelForecast, actualAvgWm2, dateStr) {
    if (!perModelForecast || actualAvgWm2 <= 0) return;
    // Guard: process each date only once (weather fetch runs hourly, same data every time)
    if (dateStr && this.data.pv_model_accuracy_date === dateStr) return;
    if (dateStr) this.data.pv_model_accuracy_date = dateStr;
    // Skip cloudy days — low absolute radiation means large relative errors from noise, not model skill
    if (actualAvgWm2 < 150) {
      this.homey.log(`[ModelAccuracy] skipped ${dateStr} — overcast (actual=${actualAvgWm2.toFixed(0)}W/m² < 150)`);
      return;
    }
    this.data.pv_model_accuracy = this.data.pv_model_accuracy || {};
    const alpha = 0.15;
    const log = [];
    for (const [m, fc] of Object.entries(perModelForecast)) {
      if (typeof fc !== 'number' || fc < 30) continue;
      const err = Math.abs(actualAvgWm2 - fc) / Math.max(actualAvgWm2, fc, 1);
      const acc = 1 - Math.min(err, 1);
      if (this.data.pv_model_accuracy[m] == null) this.data.pv_model_accuracy[m] = this._modelPrior(m);
      this.data.pv_model_accuracy[m] = alpha * acc + (1 - alpha) * this.data.pv_model_accuracy[m];
      log.push(`${m.replace('_seamless','').replace('knmi_harmonie_arome_netherlands','knmi')}=${this.data.pv_model_accuracy[m].toFixed(2)}`);
    }
    if (log.length) this.homey.log(`[ModelAccuracy] ${log.join(' ')} (actual=${actualAvgWm2.toFixed(0)}W/m²)`);
    await this._saveData();
  }
  ...
  /**
   * Persist an observe-only Cabauw-vs-local-PV clearness decorrelation sample. The matching
   * log line is wiped by log rotation / app redeploys, so the spatial-decorrelation measurement
   * week loses days; storing here makes it survive. No policy effect — diagnostic only.
   * @param {{ date:string, hour:number, cabauwKt:number, pvKt:number, delta:number, n:number }} s
   */
  recordDecorrelationSample(s) {
    if (!s || typeof s.delta !== 'number' || !isFinite(s.delta)) return;
    this.data.decorrelation_samples = this.data.decorrelation_samples || [];
    this.data.decorrelation_samples.push({
      date: s.date, hour: s.hour,
      cabauwKt: +s.cabauwKt.toFixed(2), pvKt: +s.pvKt.toFixed(2),
      delta: +s.delta.toFixed(2), n: s.n, ts: Date.now(),
    });
    // Keep ~14 days of hourly daylight samples (14 × ~10h ≈ 140)
    if (this.data.decorrelation_samples.length > 200) {
      this.data.decorrelation_samples = this.data.decorrelation_samples.slice(-200);
    }
  }

  /**
   * Per-day mean Δ (local-PV-kt − Cabauw-kt) from stored decorrelation samples, for the
   * nowcast cross-day sign-stability decision. @returns {{[date]:{meanDelta:number,n:number}}}
   */
  getDecorrelationDailyMeans() {
    const out = {};
    for (const s of this.data.decorrelation_samples || []) {
      (out[s.date] = out[s.date] || []).push(s.delta);
    }
    return Object.fromEntries(Object.entries(out).map(([d, arr]) =>
      [d, { meanDelta: +(arr.reduce((a, b) => a + b, 0) / arr.length).toFixed(3), n: arr.length }]));
  }
  ...
  getModelWeights() {
    ...
    const acc = MODELS.map(m => ({ m, acc: this.data.pv_model_accuracy?.[m] ?? this._modelPrior(m) }));
    // Rank-based: robust to single bad days; rank 1 (best) = weight 4, rank 4 = weight 1
