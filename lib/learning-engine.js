...
class LearningEngine {
  ...
  async recordDailyPvBiasFromPredictions(yDateStr, avgCloudPct = null) {
    ...
    if (isClear) {
      ...
      this.log(`Daily PV bias (clear, cloud=${avgCloudPct.toFixed(0)}%) for ${yDateStr}: actual=${sumActual.toFixed(0)}W, predicted=${sumPredicted.toFixed(0)}W, ratio=${ratio.toFixed(3)}, bias=${this.data.pv_daily_bias_clear.toFixed(3)} (n=${this.data.pv_daily_bias_clear_samples})`);
    } else {
      ...
      this.log(`Daily PV bias (mixed, ${cloudLabel}) for ${yDateStr}: actual=${sumActual.toFixed(0)}W, predicted=${sumPredicted.toFixed(0)}W, ratio=${ratio.toFixed(3)}, bias=${this.data.pv_daily_bias.toFixed(3)} (n=${this.data.pv_daily_bias_samples})`);
    }
