...
class LearningEngine {
  ...
  getDailyPvBiasFactor(avgCloudPct = null, knmiKt = null) {
    // Once yield factors are learned (≥10 slots), they already absorb systematic
    // forecast bias (learned against forecast radiation). Applying the daily-bias
    // EMA on top double-counts the correction — same guard as getRadiationBiasFactor.
    if (this.getSolarLearnedSlotCount() >= 10) return 1.0;
  ...
  getModelWeights() {
    ...
    // Rank-based: robust to single bad days; rank 1 (best) = weight 6, rank 4 = weight 0
    const RANK_WEIGHTS = [6, 3, 1, 0];
