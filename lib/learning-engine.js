...
class LearningEngine {
  ...
  getPredictedConsumption(targetTime = new Date()) {
    ...
    // Enough samples for this specific day — use it directly
    if (pattern.count >= 4) return pattern.avg;

    // Fallback: average over same day-group (weekend vs weekday) for this hour
    // Weekend = Sunday (0) + Saturday (6); weekday = Mon–Fri (1–5)
    const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
    const groupDays = isWeekend ? [0, 6] : [1, 2, 3, 4, 5];

    let groupSum = 0, groupCount = 0;
    for (const d of groupDays) {
      const p = this.data.consumption_patterns[d][hour];
  ...
  /**
   * Get number of recorded samples for a specific day+hour combination.
  ...
  getConsumptionSampleCount(targetTime = new Date()) {
    const dayOfWeek = targetTime.getDay();
    const hour      = targetTime.getHours();
    return this.data.consumption_patterns[dayOfWeek][hour]?.count ?? 0;
  }
  ...
  getConsumptionConfidence(targetTime = new Date()) {
    ...
    // Per-day confidence (0-10: 0-50%, 10-50: 50-90%, 50+: 90-100%)
    const dayConf = (count) => {
      if (count === 0) return 0;
    ...
    if (pattern.count >= 4) return dayConf(pattern.count);

    // Fallback to group confidence, capped at 60% (less precise than per-day data)
    const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
    const groupDays = isWeekend ? [0, 6] : [1, 2, 3, 4, 5];
    const groupTotal = groupDays.reduce((sum, d) => sum + this.data.consumption_patterns[d][hour].count, 0);
    return Math.min(60, dayConf(groupTotal));
  }
