...
class LearningEngine {
  ...
  /**
   * Store an hourly measured sunshine duration (minutes within the 10-min obs window, 0-10) from
   * the nearest station. Same shape as knmi_hourly_cloud — no comparator reads it yet.
   * Keeps last 2 calendar days (UTC) — pruned on write.
   * @param {number} minutes  Sunshine duration, 0-10
   * @param {string} dateStr  UTC date 'YYYY-MM-DD'
   * @param {number} hour     UTC hour 0-23
   */
  recordKnmiHourlySunshine(minutes, dateStr, hour) {
    if (typeof minutes !== 'number' || !isFinite(minutes)) return;
    if (!this.data.knmi_hourly_sunshine) this.data.knmi_hourly_sunshine = {};
    if (!this.data.knmi_hourly_sunshine[dateStr]) this.data.knmi_hourly_sunshine[dateStr] = {};
    this.data.knmi_hourly_sunshine[dateStr][hour] = minutes;
    // Prune dates older than 2 days
    const cutoff = new Date(dateStr);
    cutoff.setUTCDate(cutoff.getUTCDate() - 2);
    for (const d of Object.keys(this.data.knmi_hourly_sunshine)) {
      if (new Date(d) < cutoff) delete this.data.knmi_hourly_sunshine[d];
    }
  }

  /**
   * Store an hourly measured dewpoint depression (ta - dewpoint, °C) from the upwind station.
   * Same shape as knmi_hourly_sunshine — no comparator reads it yet.
   * Keeps last 2 calendar days (UTC) — pruned on write.
   * @param {number} depressionC  ta - dewpoint, °C
   * @param {string} dateStr      UTC date 'YYYY-MM-DD'
   * @param {number} hour         UTC hour 0-23
   */
  recordKnmiHourlyDewDepression(depressionC, dateStr, hour) {
    if (typeof depressionC !== 'number' || !isFinite(depressionC)) return;
    if (!this.data.knmi_hourly_dew_depression) this.data.knmi_hourly_dew_depression = {};
    if (!this.data.knmi_hourly_dew_depression[dateStr]) this.data.knmi_hourly_dew_depression[dateStr] = {};
    this.data.knmi_hourly_dew_depression[dateStr][hour] = depressionC;
    // Prune dates older than 2 days
    const cutoff = new Date(dateStr);
    cutoff.setUTCDate(cutoff.getUTCDate() - 2);
    for (const d of Object.keys(this.data.knmi_hourly_dew_depression)) {
      if (new Date(d) < cutoff) delete this.data.knmi_hourly_dew_depression[d];
    }
  }
