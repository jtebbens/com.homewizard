...
class LearningEngine {
   ...
   */
  async recordPvAccuracy(predictedW, actualW, omW = null, scW = null, perModelW = null, satW = null) {
    const now = Date.now();
   ...
   */
  getPvBlendWeights() {
    // Lever A — unbiased blend: a fixed 50/50 of raw OM & Solcast cancels their opposite
    // biases (OM under-, SC over-forecasts) and measured ~10% lower MAE than these
    // accuracy-derived weights on the 300-sample buffer (2026-06-15). Toggle off to revert.
    if (this.device?.getSetting?.('pv_unbiased_blend') !== false) return { wOM: 0.5, wSC: 0.5 };
