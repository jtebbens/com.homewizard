...
class LearningEngine {
   ...
   * a second buffer would duplicate it. What it cannot answer is kept instead: a score
   * that outlives the cap, and per-hour structure.
   *
  ...
  recordConsumptionAccuracy(predictedW, actualW, when = new Date()) {
    ...
    // Per-hour buckets: 24 of them fill 24x slower than the global score, so they use a
    // slower alpha. Amsterdam local hour — getHours() is UTC on Homey.
