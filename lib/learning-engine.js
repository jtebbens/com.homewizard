...
class LearningEngine {
   ...
   * then update the daily level bias EMA.
   * @param {string} yDateStr - ISO date string 'YYYY-MM-DD' (UTC)
   */
  async recordDailyPvBiasFromPredictions(yDateStr) {
    ...
    this.log(`Daily PV bias for ${yDateStr}: actual=${sumActual.toFixed(0)}W, predicted=${sumPredicted.toFixed(0)}W, ratio=${ratio.toFixed(3)}, bias=${this.data.pv_daily_bias.toFixed(3)} (n=${this.data.pv_daily_bias_samples})`);
   ...
   * Applied as a multiplier on the full pvForecast to correct systematic over/under-prediction.
   * Returns 1.0 until at least 5 days of data are collected.
   * @returns {number} Correction factor clamped to 0.3–2.0
   */
  getDailyPvBiasFactor() {
