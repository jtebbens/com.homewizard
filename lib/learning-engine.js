...
class LearningEngine {
  ...
  async recordPvAccuracy(predictedW, actualW, omW = null, scW = null, perModelW = null) {
    ...
    this.data.pv_predictions.push({
      ...
      sc:        scW ?? null,
      error:     error,
      ecmwf:     perModelW?.['ecmwf_ifs04']                          ?? null,
   ...
   */
  async recordDailyPvBiasFromPredictions(yDateStr, avgCloudPct = null) {
    ...
    const alpha = 0.15;
    const isClear    = avgCloudPct != null && avgCloudPct <= 40;
    const isOvercast = avgCloudPct != null && avgCloudPct >  75;
    ...
    if (isClear) {
      ...
      this.log(`Daily PV bias (clear, cloud=${avgCloudPct.toFixed(0)}%) for ${yDateStr}: ratio=${ratio.toFixed(3)}, bias=${this.data.pv_daily_bias_clear.toFixed(3)} (n=${this.data.pv_daily_bias_clear_samples})`);
    } else if (isOvercast) {
      ...
      this.log(`Daily PV bias (overcast, cloud=${avgCloudPct.toFixed(0)}%) for ${yDateStr}: ratio=${ratio.toFixed(3)}, bias=${this.data.pv_daily_bias_overcast.toFixed(3)} (n=${this.data.pv_daily_bias_overcast_samples})`);
    } else {
      ...
      const cloudLabel = avgCloudPct != null ? `cloud=${avgCloudPct.toFixed(0)}%` : 'cloud=unknown';
      this.log(`Daily PV bias (mixed, ${cloudLabel}) for ${yDateStr}: ratio=${ratio.toFixed(3)}, bias=${this.data.pv_daily_bias.toFixed(3)} (n=${this.data.pv_daily_bias_samples})`);
    }
   ...
   * On clear days (avg cloud cover ≤ 40%) returns the clear-sky EMA when available;
   * otherwise returns the mixed/overcast EMA. Returns 1.0 until ≥5 days of data.
   * @param {number|null} avgCloudPct - Today's average forecast cloud cover (0–100), or null
   * @returns {number} Correction factor clamped to 0.3–2.0
   */
  getDailyPvBiasFactor(avgCloudPct = null) {
    const isClear    = avgCloudPct != null && avgCloudPct <= 40;
    const isOvercast = avgCloudPct != null && avgCloudPct >  75;
    ...
    // Fallback: mixed EMA (covers 41-75% and any class with < 5 samples)
  ...
  async recordModelAccuracy(perModelForecast, actualAvgWm2, dateStr) {
    ...
    if (log.length) this.log(`[ModelAccuracy] ${log.join(' ')} (actual=${actualAvgWm2.toFixed(0)}W/m²)`);
    await this._saveData();
  }
   ...
   */
  getModelWeights() {
    const MODELS = ['ecmwf_ifs04', 'gfs_seamless', 'icon_seamless', 'knmi_harmonie_arome_netherlands'];
    const acc = MODELS.map(m => this.data.pv_model_accuracy?.[m] ?? this._modelPrior(m));
    const total = acc.reduce((a, b) => a + b, 0);
    return Object.fromEntries(MODELS.map((m, i) => [m, acc[i] / total]));
  }
