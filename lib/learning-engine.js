...
class LearningEngine {
   ...
   */
  async recordPvAccuracy(predictedW, actualW, omW = null, scW = null, perModelW = null, satW = null, chartW = null) {
    const now = Date.now();
    ...
    this.log(`PV accuracy: predicted=${predictedW}W, actual=${actualW}W, om=${omW ?? '-'}W, sc=${scW ?? '-'}W, sat=${satW ?? '-'}W, error=${(error*100).toFixed(1)}%, score=${this.data.pv_accuracy_score.toFixed(2)}`);
   ...
   */
  getModelWeights() {
    const MODELS = ['meteofrance_arpege_europe', 'gfs_seamless', 'icon_seamless', 'knmi_harmonie_arome_netherlands', 'ecmwf_ifs'];
  ...
  recordSatYield(utcHour, satGhiWm2, actualW) {
    ...
    this.data.solar_sat_yield_factors = this.data.solar_sat_yield_factors || {};
    const old = this.data.solar_sat_yield_factors[utcHour];
    if (actualW < 30 && old != null && old > 0 && old * satGhiWm2 > 200) return;
    // Ceiling/floor scoped to this hour's own EMA, not a cross-hour max — yield factor
    // varies ~12x across hours by sun-angle geometry alone (hardcoded table: 0.498-5.924),
    // so a global cap let whichever hour converged lowest permanently throttle every other
    // hour's ability to correct upward. Confirmed 2026-07-10: h=6/7/8 stuck ~1.0-1.1 while
    // hardcoded baseline for those hours is 4-6, and a same-morning live sample (~2.6) was
    // being rejected against the global ceiling despite being a legitimate same-hour reading.
    //
    // A single out-of-band sample is discarded outright — correct for one-off noise (satellite
    // glint, cloud edge) but wrong once the EMA itself has converged to a badly wrong value:
    // every legitimate correction then looks like an outlier forever and never gets in, so the
    // EMA can never recover (confirmed same day: even after scoping the cap per-hour, h=6's
    // stuck-low EMA kept rejecting real high samples). Escape hatch: 3 consecutive same-direction
    // rejections are a recalibration signal, not noise — let the 3rd one through.
    this.data.solar_sat_yield_reject_streak = this.data.solar_sat_yield_reject_streak || {};
    const outOfBand = old != null && old > 0 && (yf > old * 1.25 || yf < old * 0.2);
    if (outOfBand) {
      const dir = yf > old ? 'up' : 'down';
      const prev = this.data.solar_sat_yield_reject_streak[utcHour];
      const streak = (prev && prev.dir === dir) ? prev.n + 1 : 1;
      if (streak < 3) {
        this.data.solar_sat_yield_reject_streak[utcHour] = { dir, n: streak };
        return;
      }
    }
    delete this.data.solar_sat_yield_reject_streak[utcHour];
    const alpha = old == null ? 1.0 : 0.10;
    const newYf = alpha * yf + (1 - alpha) * (old ?? yf);
    this.data.solar_sat_yield_factors[utcHour] = newYf;
    this.homey.log(`[SAT YF] h=${utcHour} sample=${yf.toFixed(3)} → ema=${newYf.toFixed(3)}`);
  }

  getSatYieldFactor(utcHour) {
    return this.data.solar_sat_yield_factors?.[utcHour] ?? null;
  }
