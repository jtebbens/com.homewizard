  ...
  _initializeConsumptionPatterns() {
    ...
    for (let day = 0; day < 7; day++) {
      ...
      for (let hour = 0; hour < 24; hour++) {
        patterns[day][hour] = {};
        for (let q = 0; q < 4; q++) {
          patterns[day][hour][q] = { sum: 0, count: 0, avg: 0 };
        }
      }
    }
    return patterns;
  ...
  getDailyProfile(dayOfWeek) {
    ...
    for (let hour = 0; hour < 24; hour++) {
      for (let q = 0; q < 4; q++) {
        ...
        const count = this.data.consumption_patterns[dayOfWeek][hour][q]?.count ?? 0;
        slots.push({ slot: hour * 4 + q, hour, quarter: q, avgW, count });
      }
    }
    return slots;
  }
