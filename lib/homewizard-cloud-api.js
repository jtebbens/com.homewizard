...
class HomeWizardCloudAPI extends EventEmitter {
  ...
  return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (message.type === 'response' && message.details && message.details.includes('Stream ID')) {
          ...
          if (!this._lastStreamId) {
            ...
            this.log(`[DEBUG] Stream ID reused: ${streamId}`);
          }
        }
        // ---------------------------------------------------------------------------
        ...
        if (message.type === 'json_patch') {
          ...
          if (this._fastModeStart && now - this._fastModeStart > 5000) {
            ...
            this._lastFastModeRecovery = Date.now(); // NEW: timestamp for lockout
            ...
            const cooldown = 12000 + jitter;

            this.log(`[FAST-MODE] Waiting ${cooldown}ms before reconnect...`);

            setTimeout(() => {
              this.connectMainWebSocket().catch(err => {
                this.error('Fast-mode reconnect failed:', err);
              });
            }, cooldown);
    ...
    this.mainWs.on('close', () => {
      this.log('Main WebSocket closed, scheduling delayed reconnect to avoid session overlap...');
      this.emit('mainws_closed');

      authenticated = false;

      // Skip normal reconnect if fast-mode recovery triggered this close
      if (this._fastModeRecovery) {
        this.log('Main WebSocket closed due to fast-mode recovery, skipping normal reconnect');
        this._fastModeRecovery = false;
        return;
      }

      // --- 2-MINUTE LOCKOUT AFTER FAST-MODE --------------------------------------
      const sinceRecovery = Date.now() - (this._lastFastModeRecovery || 0);
      const lockout = 2 * 60 * 1000; // 2 minutes

      if (sinceRecovery < lockout) {
        const wait = lockout - sinceRecovery;
