...
class HomeWizardCloudAPI extends EventEmitter {
  ...
  // Re-arm after disconnect() (no-op while already running)
  this._startWatchdog();

  return new Promise((resolve, reject) => {
    const ws = new WebSocket('wss://energy-app-ws.homewizard.com/ws/');
    this.mainWs = ws;
    ...
    ws.on('open', () => {
      ...
      // Guard: if this socket was superseded by a reconnect, ignore it
      if (this.mainWs !== ws) return;
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (message.type === 'json_patch') {
          ...
          // Tijdens lockout géén fast-mode detectie meer
    ...
    ws.on('close', () => {
      ...
      // Superseded socket (a reconnect already replaced it) or intentional
      // disconnect (mainWs nulled): don't touch state or schedule a reconnect.
      if (this.mainWs !== ws) return;

      // Fast-mode state resetten bij elke close
      this._fastModeStart = null;
      this._lastPatchTs = null;
      this._lastPatchReceived = null;
      ...
      if (this._fastModeRecovery) {
        ...
        // recovery-vlag pas na het plannen van reconnect uit
        this._fastModeRecovery = false;
  ...
  async connectRealtimeWebSocket(deviceId, threePhases = false) {
    ...
    return new Promise((resolve, reject) => {
      const ws = new WebSocket('wss://tsdb-reader.homewizard.com/devices/date/now');
      this.realtimeWs = ws;
      ...
      ws.on('close', () => {
        ...
        // Superseded socket or intentional disconnect: no reconnect.
  ...
  this._watchdogInterval = setInterval(() => {
    ...
    setTimeout(() => {
      ...
      // Reconnect runs via the close-handler; calling connectMainWebSocket()
      // here too races it and orphans an open socket.
  ...
  disconnect() {
    ...
    // Null before close: the close-handlers use `this.mainWs !== ws` /
    // `this.realtimeWs !== ws` to detect intentional disconnect and skip reconnect.
    if (this.mainWs) {
  ...
  log(...args) {
    ...
    console.log(`${ts} [log] [HomeWizardCloudAPI]`, ...args);
  }

  error(...args) {
    const ts = new Date().toISOString();
    console.error(`${ts} [err] [HomeWizardCloudAPI]`, ...args);
  }
}
