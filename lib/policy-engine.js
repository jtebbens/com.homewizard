...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  buildPlanningSchedule(slots, pvForecast, minDischargePriceOverride = null, battChargePowerW = 800, maxChargePriceOverride = null, batteryCapacityKwhOverride = null) {
    ...
    const minSoc            = this.settings.min_soc            ?? 0;
    const maxSoc            = this.settings.max_soc            ?? 95;
    const rte               = this.settings.battery_efficiency ?? 0.75;
    ...
    for (let i = 0; i < enriched.length; i++) {
      ...
      if (hwMode === 'zero_charge_only' || hwMode === 'pv_trickle') {
        ...
        const chargeKwh = Math.min(battChargePowerW, surplus) * slotHours / 1000 * rte;
