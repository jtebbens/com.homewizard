...
class PolicyEngine {
  ...
  _applyDayAheadStrategy(scores, tariff, battery, time, inputs) {
    ...
    // FIX 6 (improved): Pre-peak charging only if price is cheap or spread is profitable
    if (currentHour >= 12 && currentHour < 16 && soc < 70) {

      const remaining = next24
        .slice(0, 12) // next 12 hours (12:00–24:00)
        .map(h => h.price)
      ...
      if (remaining.length > 0) {
        ...
        const cheapThreshold = remaining[Math.floor(remaining.length * 0.3)];

        // highest future price
        const futureMax = remaining[remaining.length - 1];

        // profitable spread?
        const profitableSpread =
          (futureMax * this.BATTERY_EFFICIENCY) - currentPrice > this.MIN_PROFIT_MARGIN;

        if (currentPrice <= cheapThreshold || profitableSpread) {
          scores.charge += 30;
          scores.preserve -= 5;
          this.log(
            `DayAhead: pre-peak charge (price €${currentPrice.toFixed(3)} <= cheapThreshold €${cheapThreshold.toFixed(3)} OR profitable spread)`
          );
        } else {
          scores.preserve += 20;
          this.log(
            `DayAhead: pre-peak skipped (price €${currentPrice.toFixed(3)} not cheap, no profitable spread)`
          );
        }
      }
    }
