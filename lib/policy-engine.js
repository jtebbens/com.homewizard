...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  buildPlanningSchedule(slots, pvForecast) {
    ...
    return slots.map(slot => {
      ...
      return {
        ...
        action:       slot.action,        // 'charge' | 'discharge' | 'preserve'
        hwMode,
  ...
  _mapActionToHwModeForPlanning(action, { price, soc, pvW, tariffType, userPolicyMode, maxChargePrice, minDischargePrice, minSoc, maxSoc }) {
    ...
    if (action === 'discharge') return profitableToDischarge ? 'zero_discharge_only' : 'standby';
    ...
    // preserve
    return pvPresent ? 'zero_charge_only' : 'standby';
  }
