...
class PolicyEngine {
  ...
  calculatePolicy(inputs) {
    ...
    // Pre-compute effective minimum discharge price for use across all scoring functions.
    // respect_minmax=true  → honour the user-configured min_discharge_price (strict mode)
    // respect_minmax=false → opportunistic: only the economic break-even is the floor,
    //                        so the battery discharges whenever it's actually profitable.
    // balanced-dynamic always behaves opportunistically (post-saldering design).
    {
      ...
      if (_respectMinMax) {
        ...
        // Pre-compute break-even from cost model if available, else derive from settings.
        // Full break-even is recomputed (and may be overwritten) in the Arbitrage block,
        // but we need a value here so DayAhead/Weather/BatteryScore can use it.
        ...
        const _settingsFloor = _cycleCost / _configuredEff; // ~€0.10 — no purchase price, just cycle cost
    ...
    if (_pvSurplusDetected && soc < maxSoc && _isDaylight) {
      ...
      if (_toChargeKwhCheck <= 0) {
        // Battery already at target SoC — nothing to delay-charge. Let discharge scoring decide.
        ...
        // Delay-charge only worthwhile at high export prices (≥ maxCharge + €0.08).
        // At low/moderate prices the net benefit is marginal (~€0.15) and the risk of
        // staying empty outweighs it. With maxCharge=€0.14 this threshold is €0.22.
        ...
        if (_pvCurrentPrice <= _dynamicMax && _excellentArbitrage) {
          ...
          // 24h window: catches tonight's cheap grid slots (e.g. 22:00 when current time is 09:00)
          const _cheapLaterHours = _pvPricesArray.filter(p =>
            typeof p.index === 'number' &&
            p.index > 2 * _sph && p.index <= 24 * _sph &&
            typeof p.price === 'number' &&
            p.price < _pvCurrentPrice * 0.70  // At least 30% cheaper
          );
          ...
          if (_cheaperLater) {
            ...
            // Guard: pvKwhRemaining must cover what the battery still needs.
            // Counts as "enough" only when remaining PV comfortably exceeds the charge need
            // (1.2× margin for forecast uncertainty and household consumption losses).
            // Without this, delay-charge can trap us: we export now, sun fades,
            // and the battery can never fill before the evening peak.
            const _pvKwhRemaining = inputs.weather?.pvKwhRemaining ?? null;
            const _pvCanFill = _pvKwhRemaining === null  // no data → trust sun-hour count
              || _pvKwhRemaining >= _toChargeKwh * 1.2;
            ...
            if (_enoughSun && _pvCanSupport && _pvCanFill) {
              ...
              this.log(`PV OVERSCHOT: delay-charge possible — ${_sunSource} (need ${_hoursNeeded.toFixed(1)}h for ${_toChargeKwh.toFixed(1)}kWh @ ${_battChargePowerW}W)${_pvRemainingStr}, ${_cheapLaterHours.length} cheap slot(s) (${(_cheapLaterHours.length * 60 / _sph).toFixed(0)}min) coming → export now, charge later`);
            } else if (_enoughSun && _pvCanSupport && !_pvCanFill) {
              this.log(`PV OVERSCHOT: delay-charge SKIPPED — PV remaining ${_pvKwhRemaining?.toFixed(1)}kWh < ${(_toChargeKwh * 1.2).toFixed(1)}kWh needed (${_toChargeKwh.toFixed(1)}kWh × 1.2 margin) → charge now before sun fades`);
            } else {
      ...
      if (_canDelayCharge) {
        ...
        // Export is nominally more profitable — but require a wider margin at mid/low SoC.
        // Below 50% SoC a thin export premium (< €0.02/kWh) is not worth keeping the
        // battery half-empty; the RTE-adjusted future discharge value is more valuable
        // than marginal export revenue when a high peak is still to come.
        ...
        if (_exportMargin < _lowSocMarginRequired) {
          ...
          inputs._pvStoreWins = false;
          scores.preserve += 20;
          this.log(`PV OVERSCHOT: export now more profitable (€${_pvCurrentPrice.toFixed(3)} > max €${_pvMaxFuture.toFixed(3)} × ${this.BATTERY_EFFICIENCY} = €${_pvStoreValue.toFixed(3)}, margin €${_exportMargin.toFixed(3)} >= threshold €${_lowSocMarginRequired.toFixed(3)}) → standby, PV to grid`);
        }
      }
    }
    ...
    if (inputs.batteryCost?.avgCost > 0) {
      ...
      // Re-use the pre-computed effective floor; update now that we have the full break-even.
      inputs.effectiveMinDischarge = inputs.respectMinMax
        ? (this.settings.min_discharge_price || 0.22)
        : breakEven;
      const minDischargePrice = inputs.effectiveMinDischarge;
      ...
      if (price !== null) {
        if (price > breakEven + 0.01 && price >= minDischargePrice) {
          if (soc <= _arbitrageMinSoc) {
            ...
            // Scale arbitrage score by load coverage when DP says preserve —
            // low house consumption means low effective discharge value.
            const dpSaysPreserve = inputs.optimizer?.getSlot(new Date()) === 'preserve';
            const coverageRatio  = this.maxDischarge > 0
              ? Math.min(this.currentLoad / this.maxDischarge, 1.0) : 1.0;
            const arbitrageScore = dpSaysPreserve
              ? Math.round(80 * coverageRatio)
              : 80;
            scores.discharge += arbitrageScore;
            scores.preserve  -= 20;
            this.log(`Arbitrage: price €${price.toFixed(3)} > break-even €${breakEven.toFixed(3)} AND >= min_discharge €${minDischargePrice.toFixed(3)} → discharge +${arbitrageScore}${dpSaysPreserve ? ` (DP=preserve, load-coverage ${Math.round(coverageRatio*100)}%)` : ''}${inputs._delayCharge ? ' (delay-charge)' : ''}`);
          }
          ...
          if (inputs.optimizer?.getSlot(new Date()) === 'preserve') {
            scores.preserve += 20;
            this.log(`Arbitrage: price €${price.toFixed(3)} cheap but optimizer says preserve (cheap slot/PV beyond 8h) → skip charge`);
          } else {
          scores.charge   += 80;
          scores.preserve -= 20;
    ...
    // PV covers gap: suppress GRID charging only.
    // When PV surplus is already being stored (pvStoreWins), that's PV charging — not wasteful.
    // Only apply the grid-charge penalty when pvStoreWins is NOT active.
    if (inputs._pvCoversGap && !inputs._pvStoreWins) {
      scores.charge   = Math.max(0, scores.charge   - 100);
      scores.preserve = Math.max(scores.preserve, scores.charge + 20);
    }
    ...
    // Build compact diagnostic snapshot for the settings page.
    // Users can copy this to share with support without needing developer log access.
    const optimizerAction = inputs.optimizer ? inputs.optimizer.getSlot(new Date()) : null;
    const _diagPrices12h = (() => {
      const all = inputs.tariff?.effectivePrices || inputs.tariff?.allPrices || inputs.tariff?.next24Hours || [];
  ...
  // In opportunistic mode use the pre-computed economic break-even as the floor (no hardcoded price).
  const oppDischargeFloor = respectMinMax
    ? (this.settings.min_discharge_price ?? 0.22)
    : (inputs.effectiveMinDischarge ?? (this.settings.cycle_cost_per_kwh ?? 0.075) / (this.settings.battery_efficiency || 0.75));
  ...
  if (respectMinMax) {
    ...
    if (price >= minDischargePrice && minDischargePrice > 0) {
      ...
      if (!isPricePeak && inputs._pvExporting && inputs._pvStoreWins === true) {
        ...
        // When DP says preserve, use actual load coverage (no floor) so that
        // low-load situations don't trigger discharge against DP advice.
        // At a clear price peak, always use full score.
        const dpSaysPreserve = !isPricePeak && inputs.optimizer?.getSlot(new Date()) === 'preserve';
        const effectiveCoverage = dpSaysPreserve ? coverageRatio : Math.max(0.5, coverageRatio);
        const baseScore = Math.round(100 * priceRatio * effectiveCoverage);

        scores.discharge += baseScore;
        scores.preserve   = 0;

        this.log(`Tariff [STRICT]: discharge hour €${price.toFixed(3)} >= min €${minDischargePrice.toFixed(3)} → discharge +${baseScore}${isPricePeak ? ' (🔴 PRICE PEAK)' : ''}${dpSaysPreserve ? ` (DP=preserve, load-coverage ${Math.round(coverageRatio*100)}%)` : ''}${inputs._delayCharge ? ' (delay-charge: PV exports + battery discharges)' : ''}`);
        return;
      }
    }
  } else {
    ...
    if (price >= minDischargePrice && minDischargePrice > 0) {
      ...
      if (inputs._pvExporting && inputs._pvStoreWins === true) {
        ...
        this.log(`Tariff [DYNAMIC]: discharge hour €${price.toFixed(3)} >= min €${minDischargePrice.toFixed(3)} → discharge +${baseScore}${dpSaysPreserve ? ` (DP=preserve, load-coverage ${Math.round(coverageRatio*100)}%)` : ''}${inputs._delayCharge ? ' (delay-charge: PV exports + battery discharges)' : ''}`);
        return;
      }
  ...
  if (respectMinMax) {
    ...
    if (price <= maxChargePrice && maxChargePrice > 0) {
      ...
      if (sunExpected.source === 'actual_pv') {
        scores.charge += 300;
        if (!inputs._delayCharge) scores.preserve = 0;
      ...
      // Optimizer 24h-DP may know about a cheaper slot beyond the 8h window (e.g. midday PV dip)
      if (inputs.optimizer?.getSlot(new Date()) === 'preserve') {
        scores.preserve += 80;
        scores.charge     = 0;
        this.log(`Tariff [STRICT]: optimizer says preserve (cheap slot/PV beyond 8h window) → defer charge`);
        return;
      }
    ...
    if (price <= maxChargePrice && maxChargePrice > 0) {
      ...
      if (inputs.optimizer?.getSlot(new Date()) === 'preserve') {
        scores.preserve += 80;
        scores.charge     = 0;
        this.log(`Tariff [DYNAMIC]: optimizer says preserve (cheap slot/PV beyond 8h window) → defer charge`);
        return;
      }
  ...
  if (nextExpensiveHour && hoursUntilExpensive !== null) {
    ...
    if (hoursUntilExpensive <= 4 && soc < 80) {
      if (alreadyAtPeakPrice) {
        ...
        if (hoursUntilExpensive <= 0.5 && soc < maxSoc - 5) {
          inputs._chargeUrgent = true; // signal mapping: grid charge before imminent peak
        }
      }
    }
  ...
  const battCapKwh     = inputs.battery?.totalCapacityKwh ?? 3;
  // Strong sun: Open-Meteo says >= 4h sunshine OR learned PV forecast covers ≥ 80% of battery
  const strongSunTomorrow = sunTomorrow >= 4 || pvKwhTomorrow >= battCapKwh * 0.8;

  if (strongSunTomorrow && soc >= 40 && currentHour >= 20) {
    scores.preserve += 15;
    this.log(`DayAhead: strong sun tomorrow (${sunTomorrow}h sunshine, ${pvKwhTomorrow}kWh PV forecast) → preserve`);
  }

  const postPeak = currentHour >= 21 || currentHour < 6;

  if (postPeak && soc < 80 && !strongSunTomorrow) {
  ...
  } else if (postPeak && soc < 80 && strongSunTomorrow) {
    this.log(`DayAhead: post-peak cheap hours skipped — strong sun tomorrow (${sunTomorrow}h / ${pvKwhTomorrow}kWh PV) will charge for free`);
  }
}
  ...
  // Sunset guard: after astronomical sunset + 30 min buffer, pvEstimate is unreliable
  // (EMA smoothing causes stale high values). Only hard signals (grid export, battery
  // charging without grid) can prove PV is still active.
  const sunset = inputs.weather?.todaySunset;
...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  _getPvWForTimestamp(timestamp, pvForecast) {
    ...
    for (const f of pvForecast) {
      const dist = Math.abs(new Date(f.timestamp).getTime() - tsMs);
      if (dist < bestDist) { bestDist = dist; best = f; }
    }
    // Only use forecast if within 35 minutes — no spillover from distant slots.
    return (best && bestDist <= 35 * 60 * 1000) ? (best.pvPowerW ?? 0) : 0;
  }
  ...
  _mapActionToHwModeForPlanning(action, { price, soc, pvW, tariffType, userPolicyMode, maxChargePrice, minDischargePrice, minSoc, maxSoc }) {
    ...
    if (action === 'discharge') {
      // If PV is producing and battery isn't full, capturing free solar beats discharging.
      // The real-time policy would detect PV and override to zero_charge_only anyway.
      if (pvStrong && soc < maxSoc) return 'zero_charge_only';
    ...
    // preserve: only show zero_charge_only when PV is meaningfully strong (≥400W).
    // Weak evening sun (e.g. 100W) is not worth a mode switch — show standby instead.
  ...
  _applyPeakTimingGuard(scores, inputs) {
    ...
    // If the DP optimizer already recommends discharge for this slot, trust it —
    // it has already accounted for the future peak price in its backward induction.
    // PeakTiming is a safety heuristic; the DP is the authoritative decision-maker.
    const optimizerAction = inputs.optimizer?.getSlot?.(new Date()) ?? null;
    if (optimizerAction === 'discharge') {
      this.log(`PeakTiming: skipped — DP optimizer already factored in future prices → discharge`);
      return;
    }

    // When remaining PV today will refill the battery for free, the RTE efficiency
    // factor is irrelevant — recharge costs nothing.  Discharging now and absorbing
    // free solar later is economically equivalent to a zero-cost cycle.
    // Skip PeakTiming suppression; the floor gate (_mapPolicyToHwMode) still applies.
    const _pvKwhRemaining = inputs.weather?.pvKwhRemaining ?? null;
    if (_pvKwhRemaining !== null) {
      const _battCapPT = inputs.battery?.totalCapacityKwh ?? (this.maxDischarge / 800) * 2.688;
      const _socPT     = inputs.battery?.stateOfCharge ?? 50;
      const _minSocPT  = this.settings.min_soc ?? 0;
      const _storedKwh = _battCapPT * Math.max(0, _socPT - _minSocPT) / 100;
      if (_pvKwhRemaining >= _storedKwh) {
        this.log(`PeakTiming: skipped — PV remaining ${_pvKwhRemaining.toFixed(1)}kWh ≥ stored ${_storedKwh.toFixed(1)}kWh → free-cycle, discharge not suppressed`);
        return;
      }
    }
    ...
    // Use actual house load as effective discharge rate — in zero_discharge_only the battery
    // only discharges as fast as the house consumes, not at max inverter power.
    // A 4-unit battery (10.752 kWh) at 291W load takes 37 h to drain, not 3.4 h at 3200W.
    // Capped at 12 to match the price window; min 1 to avoid division-by-zero.
    const maxDischargeW = this.maxDischarge || 800;
    const capacityKwh = (maxDischargeW / 800) * 2.688;
    const effectiveDischargeW = this.currentLoad > 50
      ? Math.min(maxDischargeW, this.currentLoad)
      : maxDischargeW;
    const dischargeHours = Math.min(12, Math.max(1, Math.ceil(capacityKwh / (effectiveDischargeW / 1000))));
  ...
  _selectMode(scores, ctx) {
    ...
    // charging is not executable. Re-evaluate using only discharge vs preserve
    // so confidence reflects the actual executable choice.
    ...
    // When charge is not the winner but not executable (price too high, no PV),
    // exclude charge score from denominator so it doesn't dilute confidence.
