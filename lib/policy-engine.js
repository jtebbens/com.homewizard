...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  buildPlanningSchedule(slots, pvForecast, minDischargePriceOverride = null, battChargePowerW = 800, maxChargePriceOverride = null, batteryCapacityKwhOverride = null) {
    ...
    for (let i = 0; i < enriched.length; i++) {
      ...
      } else if (hwMode === 'zero_discharge_only' || hwMode === 'to_full') {
  ...
  _mapActionToHwModeForPlanning(action, { price, soc, pvW, consumptionW = 0, tariffType, userPolicyMode, maxChargePrice, minDischargePrice, minSoc, maxSoc, futurePrices, battChargePowerW = 800, pvStoreValue = null }) {
    ...
    if (action === 'standby') return r('standby', 'dp:pv_export_wins');
    ...
    if (action === 'charge') {
      ...
      return r('standby', `dp:charge blocked price>${maxChargePrice?.toFixed(3)} no_pv`);
    }
