...
class PolicyEngine {
  ...
  _computePvFlags(inputs) {
    ...
    // cheaper export slot is coming within 4h, that slot has PV production, and enough
    ...
    if (!_canDelayCharge && _pvCurrentPrice !== null && _pvCurrentPrice > 0 && _toChargeKwhCheck > 0) {
      ...
      const _pvCanFill = _pvKwhRem === null || _pvKwhRem >= _toChargeKwhCheck * 1.2;
      const _nowMs     = Date.now();
      const _next4h    = _pvPricesArray.filter(p => {
      ...
      if (_pvCanFill && _next4h.length > 0) {
        ...
        const _sortedPrices = [..._next4h].sort((a, b) => a.price - b.price);
        ...
        if (_pvCurrentPrice > _avgCheapest * 1.30 && _pvCurrentPrice - _avgCheapest >= 0.04) {
          ...
          if (_cheapHasPv) {
            _canDelayCharge = true;
            this.log(`PV OVERSCHOT: PV opportunity delay — current €${_pvCurrentPrice.toFixed(3)} >> avg €${_avgCheapest.toFixed(3)} (${_slotsNeeded} goedkoopste slots), PV remaining ${_pvKwhRem?.toFixed(1) ?? '?'}kWh → export now, charge when cheaper`);
          }
        }
      }
    }
...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  buildPlanningSchedule(slots, pvForecast, minDischargePriceOverride = null, battChargePowerW = 800) {
    ...
    for (let i = 0; i < enriched.length; i++) {
      ...
      const { hwMode, reason } = this._mapActionToHwModeForPlanning(slot.action, {
        ...
        pvW: slot.pvW, tariffType, userPolicyMode,
   ...
   */
  _mapActionToHwModeForPlanning(action, { price, soc, pvW, tariffType, userPolicyMode, maxChargePrice, minDischargePrice, minSoc, maxSoc, futurePrices, battChargePowerW = 800 }) {
