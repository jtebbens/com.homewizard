    ...
    if (_pvSurplusDetected && soc < maxSoc && _isDaylight) {
      ...
      if (_pvCurrentPrice !== null && _pvCurrentPrice > 0) {
        ...
        if (_pvCurrentPrice <= _dynamicMax && _excellentArbitrage) {
          ...
          const _cheaperLater = _cheapLaterHours.length > 0 && _cheapLaterHours.length >= Math.ceil(_hoursNeeded);
          ...
          if (_cheaperLater) {
            ...
            if (_enoughSun && _pvCanSupport) {
              ...
              const _sunSource = _hasForecast
                ...
                : `score ${_sunScore}`;
              this.log(`PV OVERSCHOT: delay-charge possible — ${_sunSource} (need ${_hoursNeeded.toFixed(1)}h for ${_toChargeKwh.toFixed(1)}kWh @ ${_battChargePowerW}W), ${_cheapLaterHours.length} cheap hour(s) coming → export now, charge later`);
            } else {
