...
_mapPolicyToHwMode(policyMode, ctx) {
   ...
   */
  _mapActionToHwModeForPlanning(action, { price, soc, pvW, consumptionW = 0, tariffType, userPolicyMode, maxChargePrice, minDischargePrice, minSoc, maxSoc, futurePrices, battChargePowerW = 800, pvStoreValue = null }) {
    ...
    // Export beats storing this surplus (store value ≤ price) → hold and export, matching the
    // runtime standby. Without this the chart maps to zero_charge_only / pv_trickle and projects
    // the battery filling while the runtime actually exports — plan diverges from reality.
    if (pvStrong && netSurplusW >= 400 && pvStoreValue != null && pvStoreValue <= price) {
      return r('standby', `preserve:export_wins(store€${pvStoreValue.toFixed(3)}≤€${price.toFixed(3)})`);
    }
