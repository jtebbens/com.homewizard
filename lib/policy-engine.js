...
class PolicyEngine {
  ...
  if (respectMinMax) {
    ...
    if (price >= minDischargePrice && minDischargePrice > 0) {
      // NOTE: delay-charge no longer blocks discharge — stored energy discharge is
      // independent of PV export strategy. zero_discharge_only handles both:
      // PV exports to grid + battery discharges to cover house load.
      if (inputs._pvExporting && inputs._pvStoreWins === true) {
        ...
        this.log(`Tariff [STRICT]: discharge hour €${price.toFixed(3)} >= min €${minDischargePrice.toFixed(3)} → discharge +${baseScore}${inputs._delayCharge ? ' (delay-charge: PV exports + battery discharges)' : ''}`);
        return;
      }
    }
  } else {
