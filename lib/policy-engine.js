...
class PolicyEngine {
  ...
  calculatePolicy(inputs) {
    ...
    this.maxDischarge = (inputs.battery?.maxDischargePowerW || 0) ||
                        (inputs.battery?.battery_group_max_discharge_power_w || 0) ||
    ...
    if (_pvSurplusDetected && soc < maxSoc && _isDaylight) {
      ...
      if (_pvCurrentPrice !== null && _pvCurrentPrice > 0) {
        // Check: are there significantly cheaper hours coming?
        const _cheaperLater = _pvPricesArray.some(p =>
          typeof p.index === 'number' &&
          p.index > 2 && p.index <= 12 &&
          typeof p.price === 'number' &&
          p.price < _pvCurrentPrice * 0.80  // At least 20% cheaper
        );

        if (_cheaperLater) {
          // Check: is there enough sun + capacity to charge later?
          const _battChargePowerW = inputs.battery?.battery_group_max_discharge_power_w ??
          ...
          const _estCapKwh = (_battChargePowerW / 800) * 2.7;
          const _toChargeKwh = ((_targetSoc - soc) / 100) * _estCapKwh;
          const _hoursNeeded = _toChargeKwh / (_battChargePowerW / 1000);

          // Use sun forecast (weather + sun-multisource)
          const _sunHoursLeft = (inputs.weather?.sunshineTodayRemaining ?? 0);
          const _sunScore = inputs.sun?.gfs ?? inputs.sun?.harmonie ?? 0;
          const _enoughSun = _sunHoursLeft >= _hoursNeeded || _sunScore >= 50;
          const _pvCanSupport = _pvCapW >= _battChargePowerW * 0.8;

          if (_enoughSun && _pvCanSupport) {
            _canDelayCharge = true;
            this.log(`PV OVERSCHOT: delay-charge possible — ${_sunHoursLeft.toFixed(1)}h sun left (need ${_hoursNeeded.toFixed(1)}h for ${_toChargeKwh.toFixed(1)}kWh), cheaper prices coming → export now, charge later`);
          }
        }
      }
      ...
      if (_canDelayCharge) {
        ...
        scores.charge = 0;
        scores.discharge = 0;
        this.log(`PV OVERSCHOT: delay-charge wins → STANDBY (export PV to grid at €${_pvCurrentPrice?.toFixed(3)}/kWh, charge battery later at cheaper hours)`);
      ...
      if (price !== null) {
        if (price > breakEven + 0.01 && price >= minDischargePrice) {
          if (soc <= _arbitrageMinSoc) {
            ...
            // Delay-charge is active: PV exporting to grid is more profitable than discharging
            this.log(`Arbitrage: price €${price.toFixed(3)} > break-even €${breakEven.toFixed(3)} but delay-charge active → standby (PV exporting)`);
            scores.preserve += 20;
          } else {
            scores.discharge += 80;
            scores.preserve  -= 20;
  ...
  if (respectMinMax) {
    ...
    if (price >= minDischargePrice && minDischargePrice > 0) {
      if (inputs._delayCharge) {
        this.log(`Tariff [STRICT]: discharge skipped — delay-charge active (PV exporting to grid at €${price.toFixed(3)})`);
    ...
    if (price >= minDischargePrice && minDischargePrice > 0) {
      if (inputs._delayCharge) {
        this.log(`Tariff [DYNAMIC]: discharge skipped — delay-charge active (PV exporting to grid at €${price.toFixed(3)})`);
        // fall through to charge logic
      } else if (inputs._pvExporting && inputs._pvStoreWins === true) {
        // fall through to charge logic
      } else {
  ...
  if (batteryPower > 50) {
    ...
    if (delayChargeActive) {
      this.log(`💡 PV detected via batteryPower (${batteryPower}W) → sticky 5 min (delay-charge: no charge bonus)`);
    } else {
...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  if (ctx.policyMode === 'balanced' && tariffType === 'dynamic') {
    ...
    // When delay-charge is active, battery should do nothing → PV exports to grid.
    // This applies at ANY SoC. The battery firmware handles all safety/protection.
    if (ctx._delayCharge) {
      this.log(`[MAPPING] delay-charge active → standby (PV exports to grid at high price)`);
      return 'standby';
    }
  ...
  _applyPeakTimingGuard(scores, inputs) {
    ...
    const allPrices = inputs.tariff?.allPrices || inputs.tariff?.next24Hours || [];
    const now = new Date();
    ...
    const capacityKwh = (maxDischargeW / 800) * 2.7;
    const dischargeHours = Math.ceil(capacityKwh / (maxDischargeW / 1000)); // e.g. 2.7/0.8 = 3.375 → 4
