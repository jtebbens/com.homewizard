    ...
    if (!_canDelayCharge && _pvCurrentPrice !== null && _pvCurrentPrice > 0 && _toChargeKwhCheck > 0) {
      ...
      if (_pvCanFill && _next8h.length > 0) {
        ...
        if (_pvCurrentPrice > _avgCheapest * 1.30 && _pvCurrentPrice - _avgCheapest >= 0.03) {
          ...
          if (_cheapHasPv) {
            _canDelayCharge = true;
            this.log(`PV OVERSCHOT: PV opportunity delay — current €${_pvCurrentPrice.toFixed(3)} >> avg €${_avgCheapest.toFixed(3)} (${_slotsNeeded} goedkoopste slots), PV surplus ${_pvSurplusRem?.toFixed(1) ?? '?'}kWh → export now, charge when cheaper`);
          }
        }
      }
    }
...
_mapPolicyToHwMode(policyMode, ctx, exception = null) {
  ...
  buildPlanningSchedule(slots, pvForecast, minDischargePriceOverride = null, battChargePowerW = 800, maxChargePriceOverride = null, batteryCapacityKwhOverride = null, pvKwhTomorrowOverride = 0, refillConfidenceOverride = 1) {
    ...
    for (let i = 0; i < enriched.length; i++) {
      ...
      const minDischargePrice = isPerSlotOverride
        ...
        : scalarMinDischarge;
      const { hwMode, reason } = this._mapActionToHwModeForPlanning(slot.action, {
      ...
      if (hwMode === 'zero_charge_only' || hwMode === 'pv_trickle') {
        ...
        const dpDelta = (enriched[i + 1]?.socProjected ?? slot.socProjected) - slot.socProjected;
        simSoc = Math.max(minSoc, Math.min(maxSoc, simSoc + dpDelta));
  ...
  _mapActionToHwModeForPlanning(action, { price, soc, pvW, consumptionW = 0, tariffType, userPolicyMode, maxChargePrice, minDischargePrice, minSoc, maxSoc, futurePrices, battChargePowerW = 800, pvStoreValue = null, exportPrice = null, battCapKwh = 2.688, pvKwhTomorrow = 0, refillConfidence = 1 }) {
    ...
    const lowSocGridTopUp = price !== null
      ...
      && !pvStrong
      && !_pvHeadroomOpen;
    const r = (hwMode, reason) => ({ hwMode, reason });
    ...
    if (userPolicyMode === 'zero') {
      if (soc <= minSoc) return r('zero_charge_only', 'zero:soc_min');
      if (soc >= maxSoc) return r('zero_discharge_only', 'zero:soc_max');
      return r('zero', 'zero:peak_shaving');
    }

    if (action === 'trickle') {
      if (soc >= maxSoc) return r('standby', 'trickle:soc_full');
      if (netSurplusW >= 50) return r('pv_trickle', `trickle:pv_weak(${pvW}W)`);
      return r('standby', 'trickle:no_surplus');
    }
    if (action === 'standby') {
      if (lowSocGridTopUp) return r('to_full', `dp:topup soc${Math.round(soc)}% €${price?.toFixed(3)}`);
      return r('standby', 'dp:pv_export_wins');
    }

    if (action === 'discharge') {
      if (soc <= minSoc) return r('standby', `dp:discharge blocked soc≤min`);
      return profitableToDischarge
        ? r('zero_discharge_only', `dp:discharge price≥min(${minDischargePrice?.toFixed(3)})`)
        : r('standby', `dp:discharge blocked price<min(${minDischargePrice?.toFixed(3)})`);
    }
    ...
    if (action === 'charge') {
      ...
      if (pvCanSustainCharge)                        return r('zero_charge_only', `dp:charge pv_surplus(${netSurplusW}W)`);
      if (pvPresent && netSurplusW > 0)             return r('zero_charge_only', `dp:charge pv_present(${netSurplusW}W)`);
      return r('standby', `dp:charge_blocked price>${maxChargePrice?.toFixed(3)} no_pv`);
    }
