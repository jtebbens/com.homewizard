...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  if ((ctx.policyMode === 'balanced' || ctx.policyMode === 'balanced-dynamic') && tariffType === 'dynamic') {
    ...
    if (policyMode === 'charge') {
      ...
      // Use 'zero' so firmware charges from PV surplus AND discharges if consumption
      // exceeds PV — grid stays at ~0W in both directions.
      // Fall back to zero_charge_only when SoC is at or below minSoc to avoid
      // discharging below the user floor. Skip when _chargeUrgent (grid charging needed).
      if (ctx._pvStoreWins && actualPvNow && !ctx._chargeUrgent) {
        const hwMode = soc <= minSoc ? 'zero_charge_only' : 'zero';
        this.log(`[MAPPING][CHARGE] _pvStoreWins + PV active (${pvEstimate}W), soc=${soc}% → ${hwMode} (charge from surplus, discharge covers load peaks)`);
        return hwMode;
      }
