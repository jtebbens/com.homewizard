...
class PolicyEngine {
    ...
    // ── CHARGE ──
      ...
      if (spreadProfit > this.MIN_PROFIT_MARGIN) {
        if (sunExpected.source === 'actual_pv') {
          ...
          // FIX 3: only block charge, never block discharge
          scores.preserve += 200;
          scores.charge    = 0;
  ...
  _applyDayAheadStrategy(scores, tariff, battery, time, inputs) {
    ...
    // Pre-peak charge window: 12:00-16:00, SoC < 70%, price reasonable
    ...
    // Sun tomorrow penalty — only if strong sun (>=4h)
    ...
    // Post-peak: cheap hours coming → signal charge
  ...
  _mapPolicyToHwMode(policyMode, ctx) {
    ...
    if (ctx.policyMode === 'balanced' && tariffType === 'dynamic') {
      ...
      // CHARGE — only when price <= max_charge_price
