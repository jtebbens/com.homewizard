  ...
  _computePvFlags(inputs) {
    ...
    if (!_canDelayCharge && _pvCurrentPrice !== null && _pvCurrentPrice > 0 && _toChargeKwhCheck > 0) {
      ...
      if (_pvCanFill && _next8h.length > 0) {
        ...
        if (_pvCurrentPrice > _avgCheapest * 1.30 && _pvCurrentPrice - _avgCheapest >= 0.04) {
          const _minNext      = _sortedPrices[0].price;
          const _cheapSlot    = _next4h.find(p => p.price === _minNext);
...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  _mapActionToHwModeForPlanning(action, { price, soc, pvW, consumptionW = 0, tariffType, userPolicyMode, maxChargePrice, minDischargePrice, minSoc, maxSoc, futurePrices, battChargePowerW = 800 }) {
    ...
    const pvDelayMin = (price !== null && pvStrong && Array.isArray(futurePrices) && futurePrices.length > 0) ? (() => {
      ...
      if (!(price > _avg * 1.30 && price - _avg >= 0.04)) return null;
