...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  this.log(`[MAPPING] policyMode=${policyMode}, soc=${soc}, PV=${actualPvNow} (sticky=${stickyPvActive}, pvEst=${pvEstimate}W, pvP1=${pvFromP1}W), price=${price?.toFixed(3)}, maxCharge=€${maxChargePrice}`);
   ...
   */
  buildPlanningSchedule(slots, pvForecast, minDischargePriceOverride = null, battChargePowerW = 800) {
  ...
  _mapActionToHwModeForPlanning(action, { price, soc, pvW, consumptionW = 0, tariffType, userPolicyMode, maxChargePrice, minDischargePrice, minSoc, maxSoc, futurePrices, battChargePowerW = 800, pvStoreValue = null }) {
    ...
    if (action === 'charge') {
      ...
      if (pvStrong) {
        if (pvDelayMin !== null) return r('standby', `charge:pv_delay(min€${pvDelayMin.toFixed(3)})`);
        return r('zero_charge_only', `dp:charge pv_strong(${pvW}W)`);
      }
      if (price !== null && price <= maxChargePrice) return r('to_full', `dp:charge price≤max(${maxChargePrice?.toFixed(3)})`);
      if (pvPresent)                                 return r('zero_charge_only', `dp:charge pv_present(${pvW}W)`);
    ...
    const netSurplusW = pvW - consumptionW;
