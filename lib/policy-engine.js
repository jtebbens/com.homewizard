...
class PolicyEngine {
  ...
  calculatePolicy(inputs) {
    ...
    if (inputs.batteryCost?.avgCost > 0) {
      ...
      const minDischargePrice = this.settings.min_discharge_price ?? 0.22;
...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  // balanced-dynamic uses opportunistic floor (no strict min_discharge_price),
  // consistent with the DP optimizer and buildPlanningSchedule.
  ...
  const minDischarge = respectMinMax
    ...
    : (this.settings.opportunistic_discharge_floor ?? 0.20);
  ...
  buildPlanningSchedule(slots, pvForecast, minDischargePriceOverride = null) {
    ...
    // Mirror the DP floor logic: in dynamic mode use opportunistic_discharge_floor
    // so the planning display is consistent with what the optimizer actually scheduled.
    ...
    const baseMinDischargePrice = respectMinMax
      ? (this.settings.min_discharge_price || 0)
      : (this.settings.opportunistic_discharge_floor ?? 0.20);
    // Use override when the DP used a lower floor (e.g. PV headroom mode)
    const minDischargePrice = (minDischargePriceOverride !== null && minDischargePriceOverride < baseMinDischargePrice)
