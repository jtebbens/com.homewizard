    ...
    if (tariffType === 'dynamic') {
      ...
      if (sunTomorrow >= 4.0 && soc > 50) {
        ...
        if (currentHour >= eveningStartHour || currentHour < 1) {
          effectiveMinDischarge = minDischarge * 0.75; // 25% more aggressive
      ...
      const isTop3Cheap = Array.isArray(ctx.tariff?.top3Lowest) &&
        ctx.tariff.top3Lowest.some(p => Math.abs(p.price - price) < 0.00001);
      const isTop3Expensive = Array.isArray(ctx.tariff?.top3Highest) &&
        ctx.tariff.top3Highest.some(p => Math.abs(p.price - price) < 0.00001);

      // 1. Discharge mode → use battery for household load
      if (policyMode === 'discharge' || isTop3Expensive || isExpensiveWeatherAware) {
        const reason = isTop3Expensive ? 'TOP-3 EXPENSIVE' : 
                      isExpensiveWeatherAware && !isExpensive ? `WEATHER-AWARE (${sunTomorrow}h sun tomorrow)` :
                      'DISCHARGE';
        this.log(`Mode: ${reason} (€${price?.toFixed(3)}) → zero_discharge_only (cover household load)`);
