...
class PolicyEngine {
  ...
  calculatePolicy(inputs) {
    ...
    if (finalAction === 'discharge') {
      ...
      } else if (inputs._pvStoreWins) {
        this.log(`Exception: discharge blocked — PV store wins (future €${inputs._pvStoreValue?.toFixed(3)} > current €${inputs._pvCurrentPrice?.toFixed(3)})`);
        finalAction = 'charge';
        exception   = 'pv_store_wins';
      }
    }
