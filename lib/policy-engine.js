...
class PolicyEngine {
  ...
  calculatePolicy(inputs) {
    ...
    if (grid < -100 && soc < maxSoc) {
      scores.charge += 500;   // PV is free energy, always prioritize
    ...
    // 4. PV Reality (now pricing-aware)
  ...
  _getFutureExpensiveHours(tariff) {
    if (!tariff || !Array.isArray(tariff.next24Hours)) {
      return null;
    }

    const minDischargePrice = this.settings.min_discharge_price || 0.25;
    
    // Find hours in next 24h that are above discharge threshold
    const expensiveHours = tariff.next24Hours
      .filter((hour, index) => {
        return index > 0 && // Skip current hour
               index <= 12 && // Only look 12 hours ahead (realistic battery planning)
               hour.price >= minDischargePrice;
      });
  ...
  _applyPVReality(scores, p1, batteryMode) {
    ...
    this.log(`PV Reality: PV surplus detected (${pvSurplus > 0 ? pvSurplus + 'W' : 'yes'}) → charge allowed (no conversion loss)`);
    return true;
  }
  ...
  _applyWeatherForecast(scores, weather) {
    ...
    if (sun4h >= 2.0) {
      scores.charge -= 25;
      scores.preserve += 15;
      this.log('Weather: sun4h >= 2 → preserve (PV coming, no grid charge losses needed)');
    }
  ...
  _mapPolicyToHwMode(policyMode, ctx) {
    ...
    if (zeroModeActive) {
      ...
      // Net‑0 logic
      if (grid < -deadband && chargeAllowed) return 'zero_charge_only';
      if (grid >  deadband && dischargeAllowed) return 'zero_discharge_only';
    ...
    if (tariffType === 'dynamic') {
      ...
      // Check if we're in top-3 hours
      const isTop3Cheap = Array.isArray(ctx.tariff?.top3Lowest) &&
        ctx.tariff.top3Lowest.some(p => Math.abs(p - price) < 0.00001);
      const isTop3Expensive = Array.isArray(ctx.tariff?.top3Highest) &&
        ctx.tariff.top3Highest.some(p => Math.abs(p - price) < 0.00001);

      // 1. Discharge mode → use battery for household load
      if (policyMode === 'discharge' || isTop3Expensive) {
        this.log(`Mode: ${isTop3Expensive ? 'TOP-3 EXPENSIVE' : 'DISCHARGE'} → zero_discharge_only (cover household load)`);
        return 'zero_discharge_only';
      }

      // 2. Cheap hours → charge from grid OR PV
      if (policyMode === 'charge' || isCheap || isTop3Cheap) {
        ...
        if (isTop3Cheap && soc < maxSoc) {
          ...
          const avgExpensive = top3High.length > 0 
            ? top3High.reduce((sum, p) => sum + p, 0) / top3High.length 
        ...
        // Charge mode but neutral conditions → PV standby
        return 'zero_charge_only';
      }
      ...
      // 4. Preserve → wait for PV (no grid charging losses)
