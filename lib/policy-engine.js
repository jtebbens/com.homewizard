...
class PolicyEngine {
  ...
  calculatePolicy(inputs) {
    ...
    // Only force-charge on genuine PV surplus during daylight hours
    // Threshold -200W (not -100W) to avoid battery discharge fluctuations triggering this
    ...
    if (inputs.batteryCost?.avgCost > 0) {
      ...
      if (price !== null) {
        ...
        } else if (price < breakEven - 0.01 && soc < maxSoc) {
          const boost = 80;
          scores.charge += boost;
          scores.preserve -= 20;
  ...
  _applyTariffScore(scores, tariff, battery, inputs) {
    ...
    // Build 48h future price array — use allPrices (full 48h) if available
    ...
    const minFuturePrice = futurePrices48h.length ? Math.min(...futurePrices48h) : null;

    // Is there a cheaper hour coming in the next 8 hours?
    const cheaperHourComing = allPrices.some(p =>
      typeof p.index === 'number' && p.index > 0 && p.index <= 8 &&
      typeof p.price === 'number' && p.price < price - 0.005
    );

    // Spread: what profit per kWh if we charge now and discharge at max future price
    ...
    // ── DISCHARGE: any hour at or above min_discharge_price ──
    if (price >= minDischargePrice && minDischargePrice > 0) {
      ...
      // Scale score by how expensive this hour is relative to max future price
    ...
    // ── CHARGE: price at or below dynamic max AND spread is profitable ──
    else if (price <= maxChargePrice && maxChargePrice > 0) {
      if (spreadProfit > this.MIN_PROFIT_MARGIN) {
        if (sunExpected.source === 'actual_pv') {
          scores.charge += 500;
          scores.preserve = 0;
          this.log(`Tariff: cheap €${price.toFixed(3)} + actual PV → CHARGE +500`);
        } else if (cheaperHourComing) {
          scores.preserve += 120;   // veel zwaarder
          scores.charge -= 40;      // actief ontmoedigen
          this.log(`Tariff: cheaper hours coming → STRONG preserve (waiting for cheaper window)`);
        } else if (sunExpected.goodSunComing && sunExpected.hours <= 6) {
          scores.preserve += 30;
          scores.charge   -= 20;
          this.log(`Tariff: cheap €${price.toFixed(3)} but PV in ${sunExpected.hours}h → preserve`);
        } else {
          const boost = spreadProfit >= 0.10 ? 100 : 60;
          scores.charge  += boost;
          scores.preserve = 0;
        ...
        scores.preserve += 20;
        this.log(`Tariff: cheap €${price.toFixed(3)} but spread €${spreadProfit.toFixed(3)}/kWh too small → preserve`);
      }
    }

    // ── NORMAL: between charge and discharge thresholds ──
    else {
      scores.preserve += 5;
      this.log(`Tariff: normal price €${price.toFixed(3)} → preserve +5`);
    }

    // ── EXTREME prices: absolute overrides ──
    if (price <= 0.05) {
      scores.charge  += 50;
      scores.preserve -= 10;
  ...
  _applyDayAheadStrategy(scores, tariff, battery, time, inputs) {
    ...
    // Post-peak: if evening peak is over, SoC is low, and cheap hours are coming → charge
    const postPeak = currentHour >= 21 || currentHour < 6;
    if (postPeak && soc < 80) {
      const cheapSoon = next24.findIndex(p =>
        typeof p.index === 'number' && p.index > 0 && p.index <= 8 &&
        typeof p.price === 'number' && p.price <= dynamicMax
      );
      if (cheapSoon >= 0) {
        scores.charge  += 60;
        scores.preserve -= 10;
        this.log(`DayAhead: post-peak, cheap hours in ${cheapSoon}h, SoC ${soc}% → charge +60`);
      }
    }
  }
  ...
  _mapPolicyToHwMode(policyMode, ctx) {
    ...
    // Use min_discharge_price directly — battery cost model unreliable when avgCost=0
    // Only use cost model breakeven when it has tracked meaningful energy (>0.1 kWh)
    const costModelActive = ctx.batteryCost?.avgCost > 0 && ctx.batteryCost?.energyKwh > 0.1;
    const profitableToDischarge = price !== null && (
      costModelActive
        ? price >= ctx.batteryCost.breakEven && price >= minDischarge
        : price >= minDischarge
    );
    ...
    // Arbitrage override (battery cost model) — only when model has real data
    if (ctx.batteryCost?.avgCost > 0 && ctx.batteryCost?.energyKwh > 0.1 && price !== null) {
      ...
      if (price < breakEven - margin && soc < maxSoc) {
        const allPrices = ctx.tariff?.allPrices || ctx.tariff?.next24Hours || [];
        const cheaperHourComing = allPrices.some(p =>
          typeof p.index === 'number' && p.index > 0 && p.index <= 8 &&
          typeof p.price === 'number' && p.price < price - 0.005
        );
        ...
        this.log(`[MAPPING][ARBITRAGE] price €${price.toFixed(3)} < break-even but cheaper hours coming → standby`);
      }
    }
    ...
    if (ctx.policyMode === 'balanced' && tariffType === 'dynamic') {
      ...
      if (price !== null) {
        ...
        if (futureExpensiveHours && futureExpensiveHours.length > 0) {
          ...
          // Don't charge now if cheaper hours are coming within 8 hours
          const allPrices = ctx.tariff?.allPrices || ctx.tariff?.next24Hours || [];
          const cheaperHourComing = allPrices.some(p =>
            typeof p.index === 'number' && p.index > 0 && p.index <= 8 &&
            typeof p.price === 'number' && p.price < price - 0.005
          ...
          if (profit > this.MIN_PROFIT_MARGIN && soc < maxSoc - 10 && !cheaperHourComing) {
            this.log(`[MAPPING] preserve → to_full (spread profitable, no cheaper hours in 8h)`);
            return 'to_full';
          }
          if (cheaperHourComing) {
            this.log(`[MAPPING] preserve → standby (cheaper hours coming, waiting)`);
          }
        }
      }
