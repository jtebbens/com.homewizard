...
class PolicyEngine {
  ...
  _computePvFlags(inputs) {
    ...
    const _pvSurplusDetected = grid < -200 || batt > 100 || (inputs.p1?.pv_power_estimated ?? 0) >= 100;
  ...
  _buildDpResult(action, hwMode, confidence, exception, inputs, dpAction) {
    ...
    const _futDischargePrices = _schedSlots
      ...
      .map(s => s.price);
    const _allFuturePrices   = allPrices
    ...
    const maxFuturePrice = _futDischargePrices.length
      ...
      : (_allFuturePrices.length ? Math.max(..._allFuturePrices) : null);
