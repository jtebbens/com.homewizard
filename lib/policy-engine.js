...
class PolicyEngine {
  constructor(homey, settings) {
    ...
    this.BATTERY_EFFICIENCY = settings.battery_efficiency || 0.75; // HomeWizard actual RTE: 75% (25% loss per cycle)
    // FIX B: Derived from actual efficiency (not hardcoded 1.25 which assumed 80% RTE)
    // 75% RTE → 1/0.75 = 1.333 → need 33% higher future price to break even
    // 80% RTE → 1/0.80 = 1.250 → need 25% higher future price to break even
    this.BREAKEVEN_MULTIPLIER = 1 / this.BATTERY_EFFICIENCY; // Auto-derived from actual RTE
    // Minimum profit per kWh after losses before grid arbitrage triggers.
    // Exposed as 'min_profit_margin' setting so users can tune via the app UI.
    // Default €0.02: works on typical NL days (spread €0.04-0.08), skips flat days.
    // €0.00 = always arbitrage; €0.05+ = only volatile days.
  ...
  _applyTariffScore(scores, tariff, battery, inputs) {
    ...
    if (price <= maxChargePrice && maxChargePrice > 0) {
      ...
      // FIX 2: Merged into else-if - prevents double-scoring with previous block
      if (this.currentLoad > 0) {
        const coverableLoad = Math.min(this.currentLoad, this.maxDischarge);
        const savingsPerHour = (coverableLoad / 1000) * price;
      ...
      // Normal price range (above maxChargePrice, below minDischargePrice) - mild preserve
      scores.preserve += 5;
      this.log(`Tariff: normal price (€${price.toFixed(3)}) → preserve +5`);
    }

    // Fallback: only when truly nothing scored at all
  ...
  _getFutureExpensiveHours(tariff, currentPrice) {
    ...
    // FIX 4: Use 25% breakeven rule instead of min_discharge_price setting.
    // min_discharge_price (€0.30) is a user threshold that may be unreachable today.
    // The real question for PV storage is: will future price > currentPrice × 1.25?
    const breakeven = (currentPrice != null)
      ? currentPrice * this.BREAKEVEN_MULTIPLIER   // e.g. €0.263 × 1.333 = €0.351 (75% RTE)
      : (this.settings.min_discharge_price || 0.25); // fallback when price unknown
    const minDischargePrice = breakeven; // use breakeven as the filter
    ...
    // FIX: Use timestamp comparison, not index.
    // index is UNRELIABLE with 15-min Xadi data: index 12 = only 3h ahead, not 12h!
    // Timestamp-based filter correctly covers the full 24h window.
    const now = new Date();
    const lookAheadMs = 24 * 60 * 60 * 1000; // 24 hours
    ...
    const expensiveHours = pricesArray
      .filter((hour, idx) => {
        ...
        // Fallback for hourly data without timestamps
        const hourIndex = hour.index ?? idx;
        return hourIndex >= 1 && hourIndex <= 24 && (hour.price ?? 0) >= minDischargePrice;
      });

    // Deduplicate: if 15-min slots, keep only on-the-hour entries for clean averages
    // (prevents Wed 07:00, 07:15, 07:30, 07:45 all inflating the avg 4x)
  ...
  _applyPVReality(scores, p1, batteryMode, inputs) {
    ...
    if (pvEstimate > 0) {
      // FIX: Grid reading is ground truth. If importing (grid > 0), no PV surplus.
      // pvEstimate is configured panel capacity, not actual current output.
      // The P1 meter already accounts for all consumption and production.
      if (gridPower < -100) {
        // Net exporting → real PV surplus confirmed
        pvSurplus = Math.abs(gridPower); // actual export = surplus
        pvDetected = true;
      } else if (gridPower <= 0 && batteryPower > 50) {
        // Net zero or slight export + battery charging → PV covering load + battery
        pvSurplus = Math.abs(gridPower) + batteryPower;
        pvDetected = pvSurplus >= 75;
      } else {
        // Net importing → no PV surplus (pvEstimate is just panel capacity)
        pvSurplus = 0;
        pvDetected = false;
      }
    }
  ...
  _applyWeatherForecast(scores, weather, tariff, battery) {
    ...
    // FIX 5: Removed duplicate sunToday >= 4 block (was applying -30 charge / +20 preserve)
  ...
  _mapPolicyToHwMode(policyMode, ctx) {
    ...
    if (ctx.policyMode === 'balanced' && tariffType === 'dynamic') {
      ...
      if (sunTomorrow >= 4.0 && soc > 50) {
        ...
        let eveningStartHour = 17;
        let preDawnEndHour = 8;
        
        if (todaySunset) {
          // FIX: Start discharge window 1h BEFORE sunset to catch evening peak.
          // Previously rounded UP (sunset=18:56 → 19:00) which delayed discharge
          // past the peak hours. Now: sunset-1h = 17:56 → floor = 17:00.
          eveningStartHour = Math.max(0, todaySunset.getHours() - 1);
        }
        ...
        if (currentHour >= eveningStartHour || currentHour < 1) {
          effectiveMinDischarge = minDischarge * 0.50;
          this.log(`Weather-aware discharge: sunny tomorrow (${sunTomorrow}h), evening, threshold €${effectiveMinDischarge.toFixed(3)}`);
        } 
        else if (currentHour >= 1 && currentHour < preDawnEndHour) {
          effectiveMinDischarge = minDischarge * 0.9;
          this.log(`Weather-aware discharge: sunny tomorrow (${sunTomorrow}h), pre-dawn, threshold €${effectiveMinDischarge.toFixed(3)}`);
        }
      }
      
      const isExpensiveWeatherAware = price !== null && price >= effectiveMinDischarge && effectiveMinDischarge > 0;

      // pvSurplus: grid is the only ground truth. pvEstimate is static panel capacity,
      // not live output. When grid > 0 we are importing → no surplus regardless of pvEstimate.
      // When grid < 0 we are exporting → surplus = export amount.
      const pvSurplus = grid < 0 ? Math.abs(grid) : 0;
      ...
      // FIX: Use grid reading as ground truth for PV surplus.
      // pvEstimate is panel CAPACITY (3500W), not actual output.
      // If grid > 0 we are net IMPORTING → no PV surplus, regardless of pvEstimate.
      // If grid < -100 we are net EXPORTING → PV surplus confirmed by meter.
      // pvSurplus calculation above is informational only.
      const hasPvSurplus = grid < -100; // ONLY trust the actual grid export reading

      this.log(`[PV-CHECK] grid=${grid}W, pvSurplus=${pvSurplus}W (grid-derived), hasPvSurplus=${hasPvSurplus}`);
      ...
      if (hasPvSurplus) {
        ...
        if (futureExpensiveHours && futureExpensiveHours.length > 0) {
          ...
          const profitMargin = this.MIN_PROFIT_MARGIN;
      ...
      this.log(`[MAPPING] Discharge check: policyMode=${policyMode}, isTop3Expensive=${isTop3Expensive}, isExpensiveWeatherAware=${isExpensiveWeatherAware}`);

      if ((policyMode === 'discharge' || isTop3Expensive || isExpensiveWeatherAware) && !hasPvSurplus) {
        this.log(`[MAPPING] !! DISCHARGE: zero_discharge_only`);
        return 'zero_discharge_only';
      }

      if ((policyMode === 'charge' || isCheap || isTop3Cheap) && policyMode !== 'discharge') {
        if (hasPvSurplus) { // grid < -100
          return 'zero_charge_only';
        }
        ...
        if (isTop3Cheap && soc < maxSoc) {
          ...
          const avgExpensive = top3High.length > 0 
            ? top3High.reduce((sum, p) => sum + (typeof p === 'number' ? p : p.price), 0) / top3High.length 
            : (this.settings.min_discharge_price || 0.30);
          const profit = (avgExpensive * this.BATTERY_EFFICIENCY) - price;
          
          if (profit > 0) {
            this.log(`Mode: TOP-3 CHEAP, profit €${profit.toFixed(3)}/kWh → to_full`);
            return 'to_full';
          } else {
            this.log(`Mode: TOP-3 CHEAP but unprofitable → zero_charge_only`);
            return 'zero_charge_only';
          }
        }
      ...
      if (isExpensive) {
        this.log(`Mode: Expensive → zero_discharge_only`);
        return 'zero_discharge_only';
      }
      
      // "No PV system" = pv_estimation_enabled is off (pvEstimate=0 in that case)
      // When pv_estimation_enabled=true, pvEstimate is static capacity (e.g. 3500W)
      // and is non-zero even at night — so use settings flag instead.
      const noPvSystem = !this.settings.pv_estimation_enabled || pvEstimate === 0;
      if (noPvSystem && soc > minSoc + 10 && !isCheap) {
        this.log(`Mode: No PV system configured, battery at ${soc}% → zero_discharge_only`);
        return 'zero_discharge_only';
      }

      // PRESERVE MODE: evaluate if grid charging is economically viable
      // Don't blindly return zero_charge_only — at night with cheap prices and
      // expensive hours tomorrow, we should charge from grid.
      if (policyMode === 'preserve') {
        // Check if current price makes grid charging profitable
        const futureExpensiveHours = this._getFutureExpensiveHours(ctx.tariff, price);
        ...
        if (futureExpensiveHours && futureExpensiveHours.length > 0) {
          ...
          if (profitPerKwh > this.MIN_PROFIT_MARGIN && soc < maxSoc - 10) {
            this.log(`Mode: PRESERVE + profitable arbitrage (€${profitPerKwh.toFixed(3)}/kWh) → to_full`);
            this.log(`  → Current €${price?.toFixed(3)}, future avg €${avgFuturePrice.toFixed(3)}`);
            return 'to_full';
          }
        }
        
        // No profitable arbitrage, or battery nearly full
        this.log('Mode: PRESERVE → zero_charge_only (wait for PV or better prices)');
        return 'zero_charge_only';
      }
    ...
    if (ctx.policyMode === 'balanced-fixed' || (ctx.policyMode === 'balanced' && tariffType === 'fixed')) {
      ...
      if (policyMode === 'charge') {
        if (grid < -100) return 'zero_charge_only';
