...
class PolicyEngine {
  ...
  // ✅ FIX: raised from Math.max(min_soc, 1) to Math.max(min_soc, 5)
  // At 1-4% SoC the available energy (< 100Wh) barely covers RTE losses —
  // discharging is energetically pointless regardless of price.
  // min_soc=0 users still get protection via this floor.
  const zeroModeThreshold = Math.max(minSocBattery, 5);
  ...
  const pvTrickleHysteresis = Math.max(minSocBattery + 20, 20);
...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  if (ctx.policyMode === 'balanced' && tariffType === 'dynamic') {
    ...
    if (policyMode === 'charge') {
      ...
      // Price takes priority: if grid charging is affordable, always use to_full.
      // zero_charge_only only makes sense when grid charging is NOT price-justified
      // (price above maxChargePrice), in which case we fall back to capturing free PV.
      if (price !== null && price <= maxChargePrice) {
        this.log(`[MAPPING][CHARGE] price €${price.toFixed(3)} <= max_charge_price €${maxChargePrice} → to_full (grid charging wins over PV-only)`);
        return 'to_full';
      }

      // Price above ceiling — capture PV if available, else standby
      if (actualPvNow) {
        this.log(`[MAPPING] price €${price?.toFixed(3)} > maxChargePrice, PV active → zero_charge_only`);
