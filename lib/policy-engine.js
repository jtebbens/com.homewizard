...
// 2. _applySmartLowSocRule: skipped during active discharge window
// 3. _applyTariffScore: 48h threshold logic, cheaperHourComing only blocks charge
// 4. _applyWeatherForecast: sunTomorrow charge penalty reduced
// 5. _getDynamicChargePrice: uses max future price instead of top-half avg
// 6. _applyDayAheadStrategy: post-peak charge + pre-peak window
// 7. _mapPolicyToHwMode: simplified, charge only when price <= max_charge_price
// 8. Cost model reset: only when soc <= min_soc AND actively discharging
// 9. Arbitrage charge boost: only when price <= max_charge_price
// ======================================================
...
class PolicyEngine {
  ...
  calculatePolicy(inputs) {
    ...
    // FIX 8: Reset cost model only when at operational floor AND actively discharging
    // This prevents price creep across cycles without spurious resets during charge
    ...
    // PV surplus override — only during daylight
    ...
    // FIX 9: Arbitrage charge boost only when price <= max_charge_price
    // Prevents charging at €0.23 just because it's below a stale breakeven
  ...
  _applySmartLowSocRule(scores, inputs) {
    ...
    // Don't interfere during active peak discharge window
    const price = inputs.tariff?.currentPrice ?? null;
  ...
  // ------------------------------------------------------------
  // 1. ZERO MODE — Battery empty → charging is the only valid action
  // ------------------------------------------------------------
  if (soc === 0) {
    this.log('BatteryScore: SoC = 0 → ZERO MODE (charge-only)');
    scores.discharge = 0;   // never discharge at 0%
    scores.preserve  = 0;   // preserving makes no sense at 0%
    scores.charge   += 120; // strong push toward charging
    return;
  }

  // ------------------------------------------------------------
  // 2. MAX SOC — Battery full → prefer discharging
  // ------------------------------------------------------------
  if (soc >= maxSoc) {
    this.log('BatteryScore: max SoC reached → discharge');
    scores.discharge += 40;
    scores.charge     = 0;  // no charging allowed
    scores.preserve  += 10;
    return;
  }

  // ------------------------------------------------------------
  // 3. PV DETECTED — Encourage charging when sun is available
  // ------------------------------------------------------------
  if (pvDetected) {
    this.log('BatteryScore: PV detected → prefer charging');
    scores.charge   += 40;  // mild push toward charging
    scores.preserve += 5;
    return;
  }

  // ------------------------------------------------------------
  // 4. NORMAL RANGE — No special conditions
  // ------------------------------------------------------------
  ...
  // Dynamic pricing must be enabled and price must be known
  ...
  // Collect future prices (48h window)
  ...
  // Check if a cheaper hour exists in the next 8 hours
  ...
  // Profit from charging now and discharging at the next expensive hour
  ...
  if (price >= minDischargePrice && minDischargePrice > 0) {
    ...
    // Balanced discharge score
    const baseScore = Math.round(100 * priceRatio * Math.max(0.5, coverageRatio));

    scores.discharge += baseScore;
    scores.preserve   = 0;

    this.log(`Tariff: discharge hour €${price.toFixed(3)} >= min €${minDischargePrice.toFixed(3)} → discharge +${baseScore}`);
    return;
  }
  ...
  if (price <= maxChargePrice && maxChargePrice > 0) {
    ...
    // Strongest signal: cheap price + actual PV
    if (sunExpected.source === 'actual_pv') {
      scores.charge += 300; // reduced from 500 to avoid overpowering
      scores.preserve = 0;
      this.log(`Tariff: cheap €${price.toFixed(3)} + actual PV → CHARGE +300`);
      return;
    }

    // If a cheaper hour is coming soon, wait — but do not block discharge
    if (cheaperHourComing) {
      scores.preserve += 80; // reduced from 200
      scores.charge     = 0;
      this.log(`Tariff: cheaper hours coming → preserve (waiting for cheaper window)`);
      return;
    }

    // If strong PV is expected soon, avoid grid charging
    if (sunExpected.goodSunComing && sunExpected.hours <= 6) {
      scores.preserve += 20;
      scores.charge   -= 10;
      this.log(`Tariff: cheap €${price.toFixed(3)} but PV in ${sunExpected.hours}h → preserve`);
      return;
    }

    // Normal cheap-hour charging
    const boost = spreadProfit >= 0.10 ? 80 : 50;
    scores.charge  += boost;
    scores.preserve = 0;

    this.log(`Tariff: cheap €${price.toFixed(3)}, spread €${spreadProfit.toFixed(3)}/kWh → charge +${boost}`);
    return;
  }
  ...
  // ------------------------------------------------------------
  // 0. ZERO MODE PROTECTION
  //    If SoC = 0%, do NOT let day-ahead logic block charging.
  // ------------------------------------------------------------
  if (soc === 0) {
  ...
  // ------------------------------------------------------------
  // 1. Identify next profitable discharge hour
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // 2. If an expensive hour is coming soon, prepare battery
  // ------------------------------------------------------------
  if (nextExpensiveHour && hoursUntilExpensive !== null) {

    // Case A: Expensive hour soon, battery not ready
    if (hoursUntilExpensive <= 4 && soc < 80) {
      // If PV is coming before the expensive hour → wait
  ...
  // ------------------------------------------------------------
  // 3. Pre-peak charge window (12:00–16:00)
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // 4. Sun tomorrow (only strong sun)
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // 5. Post-peak cheap hours (21:00–06:00)
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // 1. Sticky PV detection
  //    Once PV is detected, keep it active for 5 minutes.
  //    This prevents PV from dropping out during short load spikes
  //    (e.g. kettle, oven, heat pump).
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // 2. Primary PV detection: battery charging power
  //    If the battery is charging with >50W, there MUST be PV surplus.
  //    This is the most reliable indicator and not affected by load spikes.
  // ------------------------------------------------------------
  if (batteryPower > 50) {
    this._pvStickyUntil = now + 5 * 60 * 1000; // 5 minutes
  ...
  // ------------------------------------------------------------
  // 3. Secondary PV detection: PV estimate from P1 or inverter
  //    Detect PV even when there is no export (PV > load).
  // ------------------------------------------------------------
  if (pvEstimate >= 100) {
  ...
  // ------------------------------------------------------------
  // 4. Fallback PV detection: export to the grid
  //    Only used when batteryPower and pvEstimate are not available.
  // ------------------------------------------------------------
  if (gridPower < -100) {
  ...
  // ------------------------------------------------------------
  // 5. ZERO MODE fallback
  //    If the battery is in zero_charge_only mode, assume PV is present.
  //    This prevents the battery from dropping out of PV mode too early.
  // ------------------------------------------------------------
  if (batteryMode === 'zero_charge_only') {
    this._pvStickyUntil = now + 2 * 60 * 1000; // shorter sticky window
  ...
  // ------------------------------------------------------------
  // 6. No PV detected
  //    Block charging in fixed tariff mode.
  //    In dynamic mode, allow tariff logic to decide.
  // ------------------------------------------------------------
  if (this.settings.tariff_type === 'dynamic') {
  ...
  _applyWeatherForecast(scores, weather, tariff, battery, inputs) {
    ...
    // Only penalize for sunTomorrow >= 4h (weak winter sun doesn't count)
    if (sunTomorrow >= 4.0) {
      scores.charge -= 10;
...
_mapPolicyToHwMode(policyMode, ctx) {
  ...
  // ------------------------------------------------------------
  // 0. HARD SAFETY RULE — Never discharge below min_soc
  // ------------------------------------------------------------
  if (soc < minSoc) {
  ...
  // ------------------------------------------------------------
  // 1. COST MODEL — Calculate safe break-even price
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // 2. PV DETECTION (fast + robust)
  //    actualPvNow = true means: PV charging is allowed
  // ------------------------------------------------------------
  const gridPower    = ctx.p1?.resolved_gridPower ?? 0;
  const batteryPower = ctx.p1?.battery_power ?? 0;

  // PV is considered active if:
  // - exporting to grid, OR
  // - battery is charging (>50W), OR
  // - gridPower <= 0 AND batteryPower > 50
  const actualPvNow =
    gridPower < -100 ||
    batteryPower > 50 ||
    (gridPower <= 0 && batteryPower > 50);

  this.log(`[MAPPING] policyMode=${policyMode}, soc=${soc}, PV=${actualPvNow}, price=${price?.toFixed(3)}, maxCharge=€${maxChargePrice}`);

  // ------------------------------------------------------------
  // 3. ZERO MODE — Battery is empty, only allow charging
  // ------------------------------------------------------------
  if (ctx.policyMode === 'zero') {
    // If empty → charge to full
    if (soc <= minSoc) return 'to_full';

    // If full → only allow discharge
    if (soc >= maxSoc) return 'zero_discharge_only';

    // Otherwise → stay in zero mode
    return 'zero';
  }

  // ------------------------------------------------------------
  // 4. DYNAMIC TARIFF MODE (BALANCED)
  // ------------------------------------------------------------
  if (ctx.policyMode === 'balanced' && tariffType === 'dynamic') {
    ...
    if (policyMode === 'charge') {
      ...
      // PV always wins — even if price is high
      if (actualPvNow) {
        this.log(`[MAPPING] PV detected → zero_charge_only`);
        return 'zero_charge_only';
      }

      // Grid charging only when price is cheap enough
