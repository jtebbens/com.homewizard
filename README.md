...
* **Fixed a case where the planner could get stuck aiming for a much lower state of charge than it should, after the battery briefly recalibrated its maximum charge rate.** Batteries occasionally report a temporary, very low charge-rate ceiling while calibrating internally. Normally this self-corrects within moments, but on some systems the reported value could stay stuck at that low figure for the rest of the day even though the battery kept charging normally at full rate. Because the planner trusted that reported ceiling, it believed far less solar could be stored than actually could, and planned around a much emptier battery than necessary. The app now cross-checks the reported ceiling against what the battery is actually doing at that moment — if the battery is clearly charging faster than its own reported maximum allows, the reported value is treated as unreliable and a sensible fallback is used instead, until the two agree again.
