...
* **The planner is blind to a small solar surplus, and the decision about it grew up outside the planner. This release measures what fixing that would be worth, without changing a thing yet.** When solar produces only a little more than the house uses, the planner assigns that surplus no value at all, so holding the battery and sitting idle look identical to it and there is nothing to choose between. The choice — store it or send it to the grid — was therefore rebuilt in two other places on top of the plan, which is how the plan, the chart and the explanation can end up saying different things. The planner can now price that surplus itself, from the real hardware floor of 75 W rather than the 400 W tuning figure used until now, and the live explanation follows whatever it decides instead of re-deciding on its own. It ships **off**. While it is off, every run plans the day a second time with it switched on, scores both plans through one and the same calculation, and writes down the difference in euros — one log line per run plus the last 96 runs kept for later, including how much of the input was real measured data rather than a default. Night runs are skipped, since there is no surplus to weigh. Two notes on reading those numbers: the surplus itself is derived from *forecast* house consumption, so the measurement can exceed the forecast but never contradict it; and a run where both plans move the same energy is recorded as exactly €0.000 rather than quietly counted as agreement.
...
### Fixed a Solar-Storage Value That Reset to Zero Too Easily (v3.19.4)

* **Fixed the app undervaluing stored solar power whenever recent solar surplus had been strong, even if the sun was about to fade.** The optimizer prices storing solar against exporting it by comparing the price the stored energy would fetch later against the current export price. There was already a safeguard for the case where that stored value came out at zero — check whether the sun will fill the remaining battery room anyway before trusting the zero — but a single quarter-hour with strong surplus (roughly 400 W or more) applied a positive-but-wrong cap without that same check, dropping the evening peak price entirely from the comparison. Seen live on 9 September: at 11:00 the app valued storage at €0.110/kWh and exported cheap solar instead, while the real value — driven by that evening's €0.459 peak, net of round-trip loss and cycle wear — was €0.261/kWh, a margin of €0.047/kWh given away. The same safeguard now applies to both cases. Confirmed live the next day: the same situation now stores instead of exporting (valued at €0.261, holding through rising export prices until export genuinely overtook it at €0.269).
